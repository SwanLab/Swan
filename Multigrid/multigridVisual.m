function [x,res,ii,it,grids] = multigridVisual(A,b,pre,post,cycle,smooth,grids,maxit,tol,x0,it,dat)
...
if grids==0
    ...
    for ii = 1:maxit
        ...
        plot([it it+1],[grids grids-1],'k-o')
        % coarse grids recursion
        [eh,~,~,it,grids] = multigridVisual(dat.RAI{n-grids+1},rh,...
            pre,post,cycle,smooth,grids-1,1,tol,eh,it+1,dat);
        plot([it-1 it],[grids-1 grids],'k-o')
        ...
        if cycle == 3 && grids ~= 1 % F cycle
            ...
            [eh,~,~,it,grids] = multigridVisual(dat.RAI{n-grids+1},rh,...
                pre,post,1,smooth,grids-1,1,tol,eh,it+1,dat);
            plot([it-1 it],[grids-1 grids],'k-o')
