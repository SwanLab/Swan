classdef FetiDPPoissonParallel < handle
    ...
    methods (Access = public)
        ...
        function obj = FetiDPPoissonParallel(globalMesh, subMeshes, stiffness, forces, tol, dirDofs)
        ...
        function [fMat, dBar] = assembleProblem(obj)
            ...
            numSub = obj.numSubdomains;
            
            % =========================================================
            % 1. PRE-CÁLCULO SECUENCIAL (Para dependencias de interfase)
            % =========================================================
            visitedDuals       = zeros(max(allDuals), 1); 
            obj.dualIdxLocal   = cell(numSub, 1);
            obj.dualSignsLocal = cell(numSub, 1);
            
            for subId = 1:numSub
                dGlobal       = obj.dualDofsGlobal{subId};
                [~, dualRows] = ismember(dGlobal, allDuals);
                
                % Calculamos los signos (+1 / -1) de forma secuencial
                isFirst             = ~visitedDuals(dGlobal);
                dualSigns           = ones(length(dGlobal), 1);
                dualSigns(~isFirst) = -1;
                visitedDuals(dGlobal) = true;
                
                % Lo guardamos en el objeto para poder usarlo después
                obj.dualIdxLocal{subId}   = dualRows;
                obj.dualSignsLocal{subId} = dualSigns;
            end
            
            % =========================================================
            % 2. BUCLE PARALELO (Map - Operaciones pesadas)
            % =========================================================
            sppLocCell  = cell(numSub, 1);
            ApCell      = cell(numSub, 1);
            termDualMat = cell(numSub, 1);
            termDBar    = cell(numSub, 1);
            termBr      = cell(numSub, 1);
            termRhsP    = cell(numSub, 1);
            
            % SLICING: Extraemos a variables locales para evitar sobrecargar el parfor
            dSignsCell = obj.dualSignsLocal;
            
            parfor subId = 1:numSub
                % Cálculos puramente locales
                ...
                % Usamos la variable local rebanada (sliced)
                dualSigns = dSignsCell{subId};
                
                % Schur primario
                sppLocCell{subId} = Kpp - Kpr * Urp;
                
                % Términos duales
                if ~isempty(dualSigns)
                    termDualMat{subId} = (dualSigns) .* (Urd'*Tdr') .* dualSigns';
                    termDBar{subId}    = dualSigns .* (Urd' * fR);
                    termBr{subId}      = dualSigns .* (Tdr * Urp) * Ap';
                end
                
                termRhsP{subId} = Ap * (fP - Urp' * fR);
            end
            
            % =========================================================
            % 3. FASE DE REDUCCIÓN SECUENCIAL (Reduce - Ensamblaje global)
            % =========================================================
            for subId = 1:numSub
                Ap = ApCell{subId};
                SPP = SPP + Ap * sppLocCell{subId} * Ap';
                
                dualRows = obj.dualIdxLocal{subId};
                if ~isempty(dualRows)
                    fDual(dualRows, dualRows) = fDual(dualRows, dualRows) + termDualMat{subId};
                    dBar(dualRows)            = dBar(dualRows) + termDBar{subId};
                    BrKrrInvKrp(dualRows, :)  = BrKrrInvKrp(dualRows, :) + termBr{subId};
                end
                
                rhsPrimal = rhsPrimal + termRhsP{subId};
            end
                        
            % Reducción final de los grados de libertad primales
            ...
            % Configuramos el precondicionador (también está paralelizado)
    ...
    methods (Access = private)
        ...
        function setupPreconditioner(obj)
            ...
            numSub = obj.numSubdomains;
            schurCell = cell(numSub, 1);
            parfor subId = 1:numSub
                schurCell{subId} = obj.computeLocalSchur(subId);
            end
            obj.localSchurBlocks = schurCell;
