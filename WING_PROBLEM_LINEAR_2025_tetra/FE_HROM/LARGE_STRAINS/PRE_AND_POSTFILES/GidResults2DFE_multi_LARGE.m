function GidResults2DFE_multi_LARGE(NameFile_res,ndime,NODAL_VECTOR,NODAL_SCALAR,MESH,DATA,DATAinpGID,CLUSTER_SEQUENCE)
% ---------------------------------------------------------------------------------------------------
% FUNCTION: GidResults2DFE_multi_LARGE
% ---------------------------------------------------------------------------------------------------
% PURPOSE:
%   This function generates a GiD `.res` file for visualizing simulation results (nodal and Gauss-point
%   based) associated with finite element analyses. It supports multiple meshes, multiple types of
%   variables (scalars, vectors, matrices), and reconstructs reduced-order results if applicable.
%
% USAGE:
%   GidResults2DFE_multi_LARGE(NameFile_res, ndime, NODAL_VECTOR, NODAL_SCALAR, MESH, DATA, DATAinpGID, CLUSTER_SEQUENCE)
%
% INPUTS:
%   - NameFile_res     : String with the name (and path) of the output GiD `.res` file.
%   - ndime            : Problem dimension (2 for 2D, 3 for 3D).
%   - NODAL_VECTOR     : Struct array with nodal vector quantities, including their values and metadata.
%   - NODAL_SCALAR     : Struct array with nodal scalar quantities.
%   - MESH             : Array of mesh structures containing Gauss-point data and metadata.
%   - DATA             : General configuration and postprocessing parameters, including time steps.
%   - DATAinpGID       : Optional reconstruction and enrichment data (e.g., ROM coefficients and additions).
%   - CLUSTER_SEQUENCE : (Optional) Time clustering structure used in reduced-order modeling.
%
% OUTPUT:
%   - No returned output. The function writes a `.res` file for GiD postprocessing.
%
% FUNCTIONALITY:
%   - Writes Gauss point definitions for all submeshes.
%   - Iterates over time steps and writes:
%       - Nodal scalar and vector fields.
%       - Gauss-point scalar, vector, and matrix fields.
%   - Applies optional reconstruction and correction to reduced variables before exporting.
%   - Supports flexible component definition and internal coordinate system for Gauss points.
%
% NOTES:
%   - The reconstruction logic assumes reduced basis methods, where variables may be written
%     in compressed form and reconstructed on-the-fly using `.coeff`, `.BASIS`, and optional `.ADDITION_VARIABLE`.
%   - Each Gauss-point variable is reshaped and assigned correctly based on the number of integration points.
%
% AUTHOR:
%   Joaquín A. Hernández, UPC – CIMNE
%   Date: 11-Dec-2020 (last update)
%   Comments generated by ChatGPT on 12-May-2025
% DEPENDENCIES:
%   - GidPostPlotVectNodesNEW
%   - GidPostPlotScalarNodesNEW
%   - GidPostPlotScalarGaussNEW
%   - GidPostPlotVectorGaussNEW
%   - GidPostPlotMatGaussNEWlarge
%   - DefaultField (for optional field handling)
%
% ---------------------------------------------------------------------------------------------------
...
for istep = 1:length(DATA.TIME_DISCRETIZATION)
    ...
    for ivar = 1:length(NODAL_VECTOR)
        ...
        if ~isempty( DATAinpGID.OPERreconstr.(NODAL_VECTOR(ivar).NAME_SNAP));
            VAR = DATAinpGID.OPERreconstr.(NODAL_VECTOR(ivar).NAME_SNAP).coeff*VAR;
    ...
    for imesh = 1:length(MESH)
        ...
        for ivar = 1:length(MESHloc.GAUSS_MATRIX)
            if  ~isempty(MESHloc.GAUSS_MATRIX(ivar).VAR)
