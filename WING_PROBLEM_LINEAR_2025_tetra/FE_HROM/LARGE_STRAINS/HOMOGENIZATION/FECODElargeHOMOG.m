function [MESH,MATPRO,OPERFE,DATA,TIME_COMPUT,CONVERGED,DISP_CONDITIONS,OTHER_output] = ...
    FECODElargeHOMOG(DATAFILE,PARAMETERS)
% FINITE ELEMENT CODE, large strains, homogenization 
% JAHO, 26-Nov-2020
if nargin ==0
    load('tmp.mat')
end
...
[MESH,MATPRO,OPERFE,DISP_CONDITIONS,DATA,OTHER_output] = ...
...
% snapshots )
[DATA,VAR,SNAP] = INITIALIZATIONvarHOMOG(DATA)   ;
%
...
if PARAMETERS.ONLY_PRINT_GID == 0
   [DATA,CONVERGED]=NONLINEARhomogLARGE(DATA,DISP_CONDITIONS,VAR,OPERFE,SNAP,MATPRO) ;     
else
...
% BASE_FOLDER = fileparts(DATA.PRINT.NAME_FILE_MSH) ; 
