function [DATA,VAR_n,SNAP] = INITIALIZATIONvarHOMOG(DATA,INICOND)
if nargin ==1
    INICOND = [] ; 
end
...
INICOND.FLUCT = zeros(ndof,1) ;    

 
VAR_n.DISP = repmat(INICOND.DISP,1,nstoreKINEM) ;  % displacements, initial conditions in
VAR_n.FLUCT = repmat(INICOND.DISP,1,nstoreKINEM) ;  % displacements, initial conditions in
...
VAR_n.PK1STRESS_homog = zeros(DATA.MESH.ndim^2,1,nstoreSTRESS) ;
...
NODESV_PROP.FLUCT.TYPE = 'Vector' ;  % For printing in GID
NODESV_PROP.FLUCT.LEGEND = 'DISP. FLUCT.' ;  % GID's printing
if DATA.MESH.ndim == 2
    NODESV_PROP.FLUCT.COMP = {'u-x','u-y'} ;
else
    NODESV_PROP.FLUCT.COMP = {'u-x','u-y','u-z'} ;
end
