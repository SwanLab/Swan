function [DATA_regress_w_der, nREDcoor] = BsplinesLeastSquares_MAW_ECMdmg(DATA_interp, qNONrel,qLIN,qNON, w)
% BsplinesLeastSquares_MAW_ECMdmg is a modification of
% BsplinesLeastSquares_MAW_ECM, described below
% It is adaptation to damage univariate problems
% JAHO, 28-Oct-2025, Tuesday, HGs, Pedralbes Barcelona
% See /home/joaquin/Desktop/CURRENT_TASKS/MATLAB_CODES/TESTING_PROBLEMS_FEHROM/112_NonLIN_ROM_RBF/17_DAMAGE.mlx
% -----------------------------------------------------------------------------------%
%--------------------------------------------------------------------------
% BsplinesLeastSquares_MAW_ECM is in turn a modification of BsplinesLeastSquares_fastECM (a function described thoroughly below)
...
spGLO = spap2(knots_number, order_Bslines, qNONrel, w);
...
for ipoint = 1:size(w,1)
    figure(200)
    hold on
    plot(qNONrel, w(ipoint,:)/VOL,'DisplayName',['wREL',num2str(ipoint),' e = ',num2str(DATA_interp.setElements(ipoint))],'Color',colors(ipoint,:)) ;
