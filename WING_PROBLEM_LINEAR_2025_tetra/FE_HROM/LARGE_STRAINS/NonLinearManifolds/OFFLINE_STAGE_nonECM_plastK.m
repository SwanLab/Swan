function OFFLINE_STAGE_nonECM_plastK(DATAoffline,DATA_interp,...
    NAME_BASE,DATA_GENGAUSS,InputDataForForces)
% =========================================================================
% OFFLINE STAGE — NONLINEAR MANIFOLD HROM (J2 PLASTICITY) WITH Kll METRIC
% =========================================================================
% PURPOSE
...
%     1) Splits displacement snapshots into Kll-orthogonal ELASTIC / PLASTIC
%        subspaces and extracts bases with a strain-energy inner product.
%     2) Chooses the FIRST PLASTIC SVD MODE as the master latent variable
%        q_PLAST and fits a B-spline decoder for plastic “slave” coords τ(q).
%     3) Constructs an ENCODER (displacements → latent) needed for stress
%        recovery and to assemble hyperreduction operators.
%     4) Reconstructs PK2/PK1 stresses per snapshot and assembles internal
%        force snapshots A_fint(qk).
%     5) Selects Gauss points/weights by Discrete ECM (or CECM-based strategy).
%
% WHY Kll?
%   Using the constrained stiffness metric (Kll) to define inner products
%   stabilizes the elastic/plastic split and yields more accurate stresses.
%
% MODEL / DECODER
%     d_L(q) = Φ_ELAST*q_ELAST  + Φ_PLAST_master*q_PLAST
%              + Φ_PLAST_slave * f(q_PLAST)    ,  f(·) = LSQ B-splines
%
% WHEN TO USE
%   • Manifold ROM with one plastic master coordinate (dominant plastic SVD mode).
%   • Stress-driven hyperreduction (A_fint assembled from PK1 stresses).
%
% -----------------------------------------------------------------------------
% INPUTS
% -----------------------------------------------------------------------------
% DATAoffline  : struct with tolerances, switches, and method choices
%   .UseEncoderToDetermineStresses  (0/1) default: 1
%   .errorDISP                      (scalar)  SVD tol for displacement basis
%   .nmodes_PLASTIC                 (int)     cap on plastic modes
%   .errorFINT                      (scalar)  ECM tolerance on internal forces
%   .errorSTRESS                    (scalar)  PK2 reconstr. relative error check
%   .errorPK2stress_basis           (scalar)  tol to build PK2 basis (if used)
%   .Hyperreduction_METHOD_projection   'STANDARD' | 'CECM_BASED_STRATEGY'
%   .ECMforReactionForces           (logical) include reaction-force block in A
%   .errorFINT_linearpart           (scalar)  tol to compress linear part
%   .USE_ELEMENT_BASED_APPROACH     (0/1)     point vs element selection
%   .DATA_interp_ECM                (struct)  optional override for CECM step
%
% DATA_interp  : struct controlling manifold fit and ECM assembly
%   .METHOD_SELECT_REFERENCE_MODE   'FIRST_SVD_MODE' | 'STANDARD_ROM'
%   .METHOD_INTERP                  'BSPLINES_LEAST_SQUARES'
%   .NSAMPLES, .ratio_NSAMPLES_knots, .order_Bslines, .PortionExtrapolation_plot
%   .MAKE_SVD_AMATRIX_per_project   (0/1)  SVD-compress A_fint per project
%   .LocalToleranceECMnonlinear_masterPOINTS (scalar) (CECM path)
%   .IndexLatentPlasticVariable     (int) index of plastic master (default: 2)
%
% NAME_BASE     : char. Base name to locate training snapshots in ./SNAPSHOTS.
% DATA_GENGAUSS : struct for continuous ECM (kept for compatibility; typically off).
% InputDataForForces : function handle returning the training load cases.
%
% -----------------------------------------------------------------------------
% OUTPUTS (SAVED to ./SNAPSHOTS/NAME_BASE/OFFLINE.mat)
% -----------------------------------------------------------------------------
% ECMdata         : selected Gauss points and weights; for CECM path includes
%                   additional manifold weights/auxiliary bases.
% BasisU          : displacement basis [Φ_ELAST, Φ_PLAST_master, Φ_PLAST_slave].
% BasisStwo       : PK2 stress basis (if constructed; left [] here by default).
% DATA            : enriched runtime data (DOFr, encoder/decoder handles, etc.).
%   .DATA_evaluateTAU_and_DER       : callable for τ(q), τ′(q), τ″(q).
%   .nREDcoor                       : # generalized coords (q_ELAST ⊕ q_PLAST …).
% Kll             : constrained stiffness matrix used as inner-product metric.
% BstRED_reactions: (optional) reaction-force operator on selected points.
%
% -----------------------------------------------------------------------------
% FILES / SIDE EFFECTS
% -----------------------------------------------------------------------------
% • Writes diary to ./OFFLINE.txt
% • Creates ./MODES/* plots for visual inspection (GiD .msh/.res)
% • Stores ./SNAPSHOTS/NAME_BASE/OFFLINE.mat with all offline artifacts
%
% -----------------------------------------------------------------------------
% WORKFLOW (high level)
% -----------------------------------------------------------------------------
...
%       → Kll-orthogonal ELAST/PLAST split; first plastic SVD mode = master;
%         build B-spline decoder and encoder.
% (C) Export modes for inspection (GiD).
% (D) For each snapshot: reconstruct d, compute PK2/PK1, assemble A_fint(qk).
% (E) Compress stress and/or A_fint (RSVD/SRSVD).
% (F) Run Discrete ECM to select Gauss points/weights (or CECM strategy).
% (G) Optional manifold-aware weighting (CECM-based strategy).
% (H) Sanity checks (PK2 error ≤ errorSTRESS), store results.
%
% -----------------------------------------------------------------------------
% IMPORTANT INTERACTIONS / GOTCHAS
% -----------------------------------------------------------------------------
% • Encoder REQUIRED: Set DATAoffline.UseEncoderToDetermineStresses = 1
%   so stresses come from encoded latent q, ensuring consistency with τ(·).
%
% • Master plastic coordinate MUST be the 1st plastic SVD mode
%   (DATA_interp.METHOD_SELECT_REFERENCE_MODE = 'FIRST_SVD_MODE').
%   Using “force-linear” masters breaks invertibility for stresses.
%
% • Reaction forces in A_fint:
%     If DATAoffline.ECMforReactionForces = true,
%     the code appends a normalized reaction block BstRED_r to BstRED_l*q
%     so ECM “sees” both internal and reaction contributions.
%
% • Element-based path:
%     DATAoffline.USE_ELEMENT_BASED_APPROACH = 1 converts point selection
%     to element selection (mutually exclusive with CECM path here).
%
% • CECM-based strategy (DATAoffline.Hyperreduction_METHOD_projection='CECM_BASED_STRATEGY'):
%     Uses continuous ECM ideas to adapt weights on the manifold; relies on
%     DATAoffline.DATA_interp_ECM (IndexLatentPlasticVariable is propagated).
%
% -----------------------------------------------------------------------------
% ERROR CHECKS
% -----------------------------------------------------------------------------
% • Aborts if PK2 reconstruction error exceeds DATAoffline.errorSTRESS.
% • If MAKE_SVD_AMATRIX_per_project=1, ensure memory headroom (A_fint becomes tall).
%
% -----------------------------------------------------------------------------
% PERFORMANCE TIPS
% -----------------------------------------------------------------------------
% • Increase nmodes_PLASTIC and/or tighten errorDISP if PK2 error is high.
% • MAKE_SVD_AMATRIX_per_project=1 saves memory downstream ECM at the cost of
%   an extra SVD per project.
% • For very long trainings, prefer point-based ECM (finer granularity).
%
% -----------------------------------------------------------------------------
% DEPENDENCIES (called internally)
% -----------------------------------------------------------------------------
% Determine_qinf_qsup_PLAST_1DK
% GidPostProcessModesDOML
% SnapStressFromDispLOC
% RSVDqp / RSVDT / SRSVD
% DiscreteECM_givenAmat
...
% -----------------------------------------------------------------------------
% J.A. Hernández (JAHO)
% Comments refreshed 07-NOV-2025 (Barcelona). Cleaned options and interactions.
% =========================================================================
