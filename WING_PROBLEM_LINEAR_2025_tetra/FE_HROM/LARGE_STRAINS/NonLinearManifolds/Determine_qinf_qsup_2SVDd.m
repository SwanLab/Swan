function [PhiLIN,PhiNON,qINF,qSUP,UU,SS,VV,MESH,DATA,DOFl,DOFr,OPERFE,MATPRO,DISP_CONDITIONS,OTHER_output]...
    = Determine_qinf_qsup_2SVDd(CASES,NAMEsnap_base,DATAoffline,NAME_BASE,MESH)
%==========================================================================
% Determine_qinf_qsup_2SVDd is a modification of Determine_qinf_qsup_1SVDd, described below,  
% It is intended to accomodate 2 SVD modes as master modes 
% JAHO, 10-Nov-2025, Monday, 00:43, Balmes 185, Barcelona 
...
for iproj = 1:length(CASES)
    ...
    for iloc = 1:length(NAME_SNAP_loc)
        ...
        % Or the whole matrix ....
        DISP_LOC{iloc} = bsxfun(@times,SNAP_cluster.DISP.U',SNAP_cluster.DISP.S)' ;
        DISP_LOC{iloc} =  DISP_LOC{iloc}*SNAP_cluster.DISP.V(3:end,:)' ;
...
subplot(2,1,1)
hold on 
title('First SVD mode amplitude')
xlabel('Time step')
ylabel('q_1')
plot(S(1)*V(:,1))
 subplot(2,1,2)
hold on 
title('Second SVD mode amplitude')
xlabel('Time step')
ylabel('q_2')
plot(S(2)*V(:,2))
...
xlabel('q_1')
ylabel('q_2')
title('Points projection in the plane q1-q2')
plot(S(1)*V(:,1),S(2)*V(:,2),'Marker','.')
...
PhiNON = Phi(:,3:end) ; 
 ...
 Phi_To_Plot = zeros(ndofs, size(PhiNON,2)+2) ; 
  Phi_To_Plot(DOFl,1:2) = PhiLIN ; 
     Phi_To_Plot(DOFl,3:end) = PhiNON ; 
