function  ECMdata = CECM_based_ManifAdWeights_PLaC(A,DATA,Wfe,DATA_interp,zINI,wINI,...
    qLATENT,LOCALdata,Uel,Qall_w,DATAoffline)
% CECM_based_ManifAdWeights_PLaC is a modification of CECM_based_ManifAdWeights_PLa
% It combines CECM_based_ManifAdWeights_PLa and CECM_based_ManifAdWeights_PL

% CECM_based_ManifAdWeights_PLa is a modification of CECM_based_ManifAdWeights_PL
% It incorporates actual enforcement of the condition that the weights must
% be positive 
% 20th Sept. 2025, Saturday, Balmes 185, Barcelona

% CECM_based_ManifAdWeights_PL is Adaptation of CECM_based_ManifAdWeights (described below) to plasticity problems
% JAHO, 19th September 2025, Friday, HGs Pedralbes, Barcelona
%
%======================================================================
...
% PLASTIC PART
% ---------------
% We are going to include only plastic snapshots
% Besides, the latent variable is the second component of qLATENT
iLAT = 2;
qLATENT = qLATENT(iLAT,LOCALdata.ind_plastic)  ;
% % Norm of A
% rA = zeros(size(A)) ;
% for iCL = 1:length(A)
...
while   length(iCAND) > max_nmodes    
    ...
    [wADAPT_tent,iCAND_tent,jELIM_tent,kLOC] = Loop_MAWecmNOENF_cl(U,ilocPOS,iCAND,wADAPT,b,icluster,indSORT,jELIM); 
    ...
    if kLOC > length(ilocPOS)
        ...
        [wADAPT_tent,iCAND_tent,jELIM_tent,kLOC] = Loop_MAWecmNOENF(U,ilocPOS,iCAND,wADAPT,b,icluster,indSORT,jELIM);
        if kLOC > length(ilocPOS)
    ...
    % Enforcing weights to be positive
   % [wADAPT,iCAND,jELIM,kLOC] = Loop_MAWecmNOENF(U,ilocPOS,iCAND,wADAPT,b,icluster,indSORT,jELIM);  
