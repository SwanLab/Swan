function [DATA_evaluateTAU_and_DER, nREDcoor] = BsplinesLeastSquares_PLAST_al(DATA_interp, qMASTER_nonl, VrightT, UU, SS,q_firstMODE)
%--------------------------------------------------------------------------
...
%  It also incorporate a new input: q_firstMODE
...
% INVERSE FUNCTION RELATING   qMASTER_nonl =   tau_1_INV(q_firstMODE)
tau_1_INV = spap2(knots_number, order_Bslines, q_firstMODE, qMASTER_nonl);
tau_1_INV_der1 =fnder(tau_1_INV) ;
tau_1_INV_der2 =fnder(tau_1_INV_der1) ;

x_min_inv = min(q_firstMODE);
x_max_inv = max(q_firstMODE) ;
%
L_inv = x_max_inv - x_min_inv;

x_plot_inv = linspace(x_min_inv - L_extrap * L_inv, x_max_inv + L_extrap* L_inv, 500);


figure(342)
hold on
title(['Plastic latent  variable as a function of the amplitude of the first mode'])
xlabel('qNON_1')
ylabel('qLATENT plastic')
plot(q_firstMODE,qMASTER_nonl)
[OUTPUT ] = evaluate_spline_with_extrapolation( tau_1_INV  , tau_1_INV_der1   ,  tau_1_INV_der2 , x_plot_inv) ;
  plot(x_plot_inv,OUTPUT.VALUE,'--','DisplayName','Bspline' ) ;
  grid on 
%%%%%%%%%%%%%%%%%%%
figure(343)
hold on
title(['First derivative Plastic latent  variable as a function of the amplitude of the first mode'])
xlabel('der qNON_1')
ylabel('qLATENT plastic')
    plot(x_plot_inv,OUTPUT.DER1,'--','DisplayName','Bspline' ) ;
%%%%%%%%%%%%%%%%%%%
...
DATA_evaluateTAU_and_DER.nameFunctionEvaluate = 'tauFUN_1paramELASTplastL'; % Name of the function
...
disp('Testing tauFUN_1paramELASTplastL')
qMASTER = [-0.2,0.2
           -0.2,0.2   ] ;
[tau,tauDER1,tauDER2] = tauFUN_1paramELASTplastL(qMASTER,DATA_evaluateTAU_and_DER)  ; 