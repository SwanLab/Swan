function [VAR,CONVERGED,DATA] = NewtonRapshonStaticLarge_manifoldMD(DATA,OPERFE,VAR,MATPRO,DOFl)
%--------------------------------------------------------------------------
...
while  kiter <=DATA.NEWTON_RAPHSON.NMAXiter
    ...
    if  ~isempty(OPERFE.wSTs_cluster)
        ...
        w  = feval(OPERFE.wSTs_cluster.DATA_regress.nameFunctionEvaluate,q_LAT,OPERFE.wSTs_cluster.DATA_regress) ;
        OPERFE.wSTs = w;
...
%      if kiter == 1 
%          tauNONallDER_q_FIXED= tauNONallDER_q ; 
%      elseif kiter >1 && DATA.FreezeLeftBasis == 0
%           tauNONallDER_q_FIXED= tauNONallDER_q ; 
%     end
...
%            tauNONder2_q = [] ;
            if ~isempty(tauNONder2_q) && DATA.USE_GEOMETRIC_term_K == 1
                K_geo =0 ;
