function [NEUMANN] = Neumann_OPPrb_generic(t0,tEND,AMPLITUDE,SURFACES)
% 
% 
...
NEUMANN(icond).GENERALIZED_POINT_FORCE(iloadstate).AMPLITUDE =zeros(size(AMPLITUDE(:)')) ;  % % Translation (centroid face)
...
%---------------------------------------------------------------
NEUMANN(icond).NUMBER_SURFACE = SURFACES(2) ;   % Number of SURFACE on which DISPLACEMENT  is prescribed
iloadstate = 1;  % Loading stage
