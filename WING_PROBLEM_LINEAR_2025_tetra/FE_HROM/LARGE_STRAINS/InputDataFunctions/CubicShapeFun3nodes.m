function  [Nshape,xi,IndPoints] =   CubicShapeFun3nodes(ElemBnd_loc,COORnodes,COORbnd,Nshape)
if nargin == 0
    load('tmp.mat')
end
x1 = COORnodes( ElemBnd_loc(1),:) ;  % First point segment (assuming it is straight)
x2 = COORnodes(ElemBnd_loc(4),:) ;
...
% local system in the midpoint    of the segment
...
imode = ElemBnd_loc(1) ;
Nshape(IndPoints,imode) = -9/16 .* (xi + 1/3) .* (xi - 1/3) .* (xi - 1);
% SHAPE FUNCTION node 2
imode = ElemBnd_loc(2) ;
Nshape(IndPoints,imode) =27/16 .* (xi + 1)   .* (xi - 1/3) .* (xi - 1);
% SHAPE FUNCTION node 3
imode = ElemBnd_loc(3) ;
Nshape(IndPoints,imode) =  -27/16 .* (xi + 1)  .* (xi + 1/3) .* (xi - 1); 
% SHAPE FUNCTION node 4
imode = ElemBnd_loc(4) ;
Nshape(IndPoints,imode) =  9/16 .* (xi + 1)    .* (xi + 1/3) .* (xi - 1/3); 
