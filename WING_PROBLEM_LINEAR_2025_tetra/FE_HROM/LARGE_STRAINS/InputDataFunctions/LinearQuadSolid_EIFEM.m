function [Nshape,COORall,OTHER_OUTPUT] =  LinearQuadSolid_EIFEM(INFO_INTERFACE_MESH,CENTROID,COORbnd)
...
COORall =  INFO_INTERFACE_MESH.COORquad;
...
for idim = 1:ndim
    COORall(:,idim) = COORall(:,idim) - CENTROID(idim) ;
end
...
M1_eta = 0.5 * eta.*(eta - 1);
M2_eta =1 - eta.^2;
M3_eta =0.5 * eta.*(eta + 1);
 

Nshape = zeros(length(xi),6);  
...
inode = 4; 
Nshape(:,inode) = L2_xi.*M3_eta ; 
inode = 5; 
Nshape(:,inode) = L1_xi.*M3_eta ; 
inode = 6; 
