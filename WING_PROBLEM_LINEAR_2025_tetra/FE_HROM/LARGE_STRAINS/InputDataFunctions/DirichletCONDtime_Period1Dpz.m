function [DOFr,dR,DISP_CONDITIONS,INFO_PERIODIC_CONDITIONS] = DirichletCONDtime_Period1Dpz(DIRICHLET,DATA,ndim,MESH,DATALOC)
% Goal. Determine DOFr and    dR(t) .
% Periodicity conditions in one direction `+ zero constratins 
...
% JAHO- 14-May-2O24, Terrassa,  UPC/ 31-MAy-2024, UPC, CAMPUS NORD 
%--------------------------------------------------------------------------------------
if nargin == 0
    load('tmp3.mat')
end
...
% COORbnd = MESH.COOR(rnodLOC,:) ;  % COORDINATES
 ...
 DOFr = [] ; dR= [] ; 
if ndim == 2    
    [ DISP_CONDITIONS,INFO_PERIODIC_CONDITIONS] = Periodic_1directpz(DIRICHLET,DATA,ndim,MESH,DATALOC);    
