function [DOFr,dR] = DirichletCONDtime_rigid_WARPING(DIRICHLET,DATA,ndim,MESH,GEOproperties,DATALOC)
...
%
% JAHO- 21-march-2O24,   Balmes 185,  Barcelona
%--------------------------------------------------------------------------------------
if nargin == 0
    load('tmp1.mat')
elseif nargin == 5
...
for  izone = 1:length(DIRICHLET) 
       ...
       LOCVAR=  DIRICHLET(izone) ; 
        [Uloc,aloc,DOFrLOCC] =    DirichletCONDtimeRIGIDbody_warp(LOCVAR,DATA,ndim,MESH,GEOproperties) ;
        U{izone} = sparse(Uloc) ;
        a{izone} = aloc ;
        DOFr{izone} = DOFrLOCC ; 
