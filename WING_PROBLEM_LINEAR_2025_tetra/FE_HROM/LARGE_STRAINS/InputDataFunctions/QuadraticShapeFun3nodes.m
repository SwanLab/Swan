function  [Nshape,xi,IndPoints] =     QuadraticShapeFun3nodes(ElemBnd_loc,COORnodes,COORbnd,Nshape) 
if nargin == 0
    load('tmp.mat')
end
x1 = COORnodes( ElemBnd_loc(1),:) ;  % First point segment (assuming it is straight)
x2 = COORnodes(ElemBnd_loc(3),:) ;
...
% LEt us use normalized quadratic functions. We place the origin of the
% local system in the second point of the segment
x3 =  COORnodes(ElemBnd_loc(2),:)  ;
COORbnd_rel_0 = 2*bsxfun(@minus,COORbnd',x3(:))'/norm_x1x2 ;
xi = u*COORbnd_rel_0(IndPoints,:)' ;
% SHAPE FUNCTION ELEMENT 1
imode = ElemBnd_loc(1) ;
Nshape(IndPoints,imode) = 0.5*xi.*(xi-1) ;
% SHAPE FUNCTION ELEMENT 2
imode = ElemBnd_loc(2) ;
Nshape(IndPoints,imode) = 1-xi.^2;
% SHAPE FUNCTION ELEMENT 3
imode = ElemBnd_loc(3) ;
Nshape(IndPoints,imode) =  0.5*xi.*(xi+1) ;
%
