function [DOFr,dR] = DirichletCONDtime_rigid2(DIRICHLET,DATA,ndim,MESH,GEOproperties,DATALOC)
...
for  izone = 1:length(DIRICHLET) 
       ...
       LOCVAR=  DIRICHLET(izone) ; 
        [Uloc,aloc,DOFrLOCC] =    DirichletCONDtimeRIGIDbody2(LOCVAR,DATA,ndim,MESH,GEOproperties) ;
        U{izone} = sparse(Uloc) ;
        a{izone} = aloc ;
        DOFr{izone} = DOFrLOCC ; 
