function  [DISP_CONDITIONS,INFO_PERIODIC_CONDITIONS] =...
    PeriodicQ4(DIRICHLET,DATA,ndim,MESH,GEOproperties,DATALOC,Nshape,rnodLOC,MESHcoar)
% This function  returngs the matrices and vectors defining affine boundary
...
DATALOC = DefaultField(DATALOC,'PRESCRIBED_DIRECTLY_FLUCTUATIONS_AT_END_POINTS_WHEN_NO_CORNERS',1) ; 
...
if isempty(DATALOC.Ufluct)
    ...
    if ~isempty(III) || DATALOC.PRESCRIBED_DIRECTLY_FLUCTUATIONS_AT_END_POINTS_WHEN_NO_CORNERS == 1
        ...
        [DISP_CONDITIONS,INFO_PERIODIC_CONDITIONS] = PeriodicQ4_corners(DIRICHLET,DATA,ndim,MESH,GEOproperties,DATALOC,Nshape,rnodLOC,COMB_FACES)  ;
    else
