function [DATAIN] = ECMpointsPRINT(DATAIN,MESH,HYPERREDUCED_VARIABLES) ;
...
%         mkdir(ROOTFOLDER) ;
%     end
    NameFile_msh = [DATAIN.NameFileMesh_ECM ,'' ,'.msh'] ;
    NameFile_res = [DATAIN.NameFileMesh_ECM ,'' ,'.res'] ;
    
    
    % Writing mesh file
    %MaterialType = ones(size(CN,1),1) ;
    CNred  = CNref(HYPERREDUCED_VARIABLES.setElements,:) ;
    MatRED = MESH.MaterialType(HYPERREDUCED_VARIABLES.setElements)  ;
...
%     end
    ...
    if DATAIN.PRINT_MESH_ECM_POINTS_AS_POINTS == 1
        ...
        [COOR,CNredASPOINTS]=  PrintMeshECMPointsAsPoints(COOR,CNred,DATAIN,...
            MESH,HYPERREDUCED_VARIABLES.setElements);
        disp('----------------------------------')
        else
             disp('Printing mesh for ECM points')
        ...
        MaterialTypegloPRINT = {MatRED,MESH.MaterialType} ;
        TypeElementPRINT = {MESH.TypeElement,MESH.TypeElement} ;
    ...
    %
    %     GidPostProcess_onlyECMpoints(COOR,CNref,MESH.TypeElement,BasisUdef,MESH.posgp,...
