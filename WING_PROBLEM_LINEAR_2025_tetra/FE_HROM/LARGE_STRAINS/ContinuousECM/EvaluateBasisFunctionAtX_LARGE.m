function  [PHIk_y,dPHIk_y,POLYINFO]=     EvaluateBasisFunctionAtX_LARGE(xNEW,DATA,VAR_SMOOTH_FE,POLYINFO)
...
% New alternative (direct fitting), 16-Dec-2021. See
% /home/joaquin/Desktop/CURRENT_TASKS/MATLAB_CODES/TESTING_PROBLEMS_FEHROM/ContinuousEmpiricalCubatureM/Paper_hernandez2021ecm/06_HROM_homog2D/DirectFittingIntegrand.mlx
if nargin == 0
    load('tmp.mat')
end


if VAR_SMOOTH_FE.DIRECT_POLYNOMIAL_FITTING_INTEGRAND == 1
     
    [PHIk_y,dPHIk_y,POLYINFO]=  EvaluateBasisFunctionDIRECTFIT(xNEW,DATA,VAR_SMOOTH_FE,POLYINFO) ;   
    POLYINFO.BdomRED_interp = [] ; 
else
    ...
    while inew <=npoints
        ...
        if DATA.OnlyCheckIfIsInside == 0
            ...
            derN = zeros(length(PevaluateDER),length(N)) ;
