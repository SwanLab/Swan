...
% *****************************************
LOWERBOUND_matrix = sparse(length(BasisU_cluster)) ;
UPPERBOUND_matrix = sparse(length(BasisU_cluster)) ;
...
TransMatrix = cell(length(BasisU_cluster)) ; % Matrix for changing from one cluster to   neighboring
% clusters
...
for i = 1:length(BasisU_cluster)
    ...
    for j = 1:length(BasisU_cluster)  % This is amenable simplifcation  (repeated operations)
        if i ~= j
            ...
            if ISneigh == 1
                ...
                if size(TransMatrix{i,j},2) > size(TransMatrix{i,j},1)
                    Upperbound = 1 ;
                else
