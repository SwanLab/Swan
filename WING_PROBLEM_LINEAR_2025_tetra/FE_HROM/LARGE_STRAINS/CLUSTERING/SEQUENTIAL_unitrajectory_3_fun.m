...
if nargin == 0
    load('tmp1.mat')
    DATAoffline.NCLUSTERS_BASIS_DISP = 2; 
end
...
if isempty(DATAoffline.NCLUSTERS_BASIS_DISP)
    % LIMITING CASE 
    CENT = SNAPdisp(:,1:end-1) ; % Each  snapshot is a centroid
    SNAPSHOTS_PER_CLUSTER_after_overlap = cell(size(CENT,2),1) ;
    
    for icluster = 1:length(SNAPSHOTS_PER_CLUSTER_after_overlap)
        if icluster == 1
            SNAPSHOTS_PER_CLUSTER_after_overlap{icluster} = [icluster,icluster+1,icluster+2] ;
        else
            SNAPSHOTS_PER_CLUSTER_after_overlap{icluster}   =[icluster-1,icluster,icluster+1] ;
        end
    end
    ...
    INTERVALS =  ceil(linspace(1,size(SNAPdisp,2),DATAoffline.NCLUSTERS_BASIS_DISP+1)) ; 
    
    CENT = zeros(size(SNAPdisp,1),DATAoffline.NCLUSTERS_BASIS_DISP) ; 
    SNAPSHOTS_PER_CLUSTER_after_overlap = cell(size(CENT,2),1) ;
    
      for icluster = 1:length(SNAPSHOTS_PER_CLUSTER_after_overlap)
          
           iniCLUST = INTERVALS(icluster) ; 
           finCLUST = INTERVALS(icluster+1) ;
           snapLOC = SNAPdisp(:,iniCLUST:finCLUST) ; 
           centLOC = sum(snapLOC,2)/size(snapLOC,2) ; 
           CENT(:,icluster) = centLOC ;  
          
        if icluster == 1
            SNAPSHOTS_PER_CLUSTER_after_overlap{icluster} = iniCLUST:(finCLUST+1) ;
        elseif icluster == length(SNAPSHOTS_PER_CLUSTER_after_overlap)
            SNAPSHOTS_PER_CLUSTER_after_overlap{icluster}   = (iniCLUST-1):(finCLUST) ;
        else
             SNAPSHOTS_PER_CLUSTER_after_overlap{icluster}   = (iniCLUST-1):(finCLUST+1) ;
        end
    end
...
%NEIGHBORING_CLUSTERS = ClusterInterConnectivities(SNAPSHOTS_PER_CLUSTER_after_overlap,CENT,DATAoffline) ; 
 ...
 NEIGHBORING_CLUSTERS = cell(length(SNAPSHOTS_PER_CLUSTER_after_overlap),1) ; 

for icluster = 1:length(NEIGHBORING_CLUSTERS)-1
     
    NEIGHBORING_CLUSTERS{icluster} = icluster+1; 
end



    NEIGHBORING_CLUSTERS{end} = length(NEIGHBORING_CLUSTERS)-1; 
