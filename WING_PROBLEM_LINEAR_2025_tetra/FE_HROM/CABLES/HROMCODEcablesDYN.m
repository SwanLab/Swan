function  [BasisUall,RECONS_TENSIONV,OTHER_output ]=  HROMCODEcablesDYN(DATAFILE,PARAMETERS)
% HROM   CODE, large strains, dynamic regime
% JAHO, 28-Dec-2020/15-May-2021 (adaptation of FECODElargeDYN)
if nargin ==0
    load('tmp.mat')
end
TIME_COMPUT=[] ;
...
[BasisUall,BasisTENSIONV,ECMdata,...
...
% snapshots )
[DATAHROM,VAR,SNAP] = INITIALIZATIONvarCABLE(DATAHROM,INICOND)   ;
%
...
if PARAMETERS.ONLY_PRINT_GID == 0
    if DATAHROM.ISDYNAMIC == 0
        [DATAHROM,CONVERGED]=NONLINEARstaticCABLES(DATAHROM,DISP_CONDITIONS,VAR,OPERHROM,SNAP,Fbody,Ftrac,MATPRO) ;
    else
        [DATAHROM,CONVERGED]=NONLINEARdynamicCABLES(DATAHROM,DISP_CONDITIONS,VAR,OPERHROM,SNAP,Fbody,Ftrac,MATPRO) ;
    end
else
...
% Reconstruction of TENSION VECTOR
[OTHER_output,DATAinpGID,RECONS_TENSIONV] = TensionReconstruction(DATAinpGID,OTHER_output,ECMdata,DATAHROM,BasisTENSIONV) ;
...
PARAMETERS = DefaultField(PARAMETERS,'PRINT_POSTPROCESS_GID',1) ;
