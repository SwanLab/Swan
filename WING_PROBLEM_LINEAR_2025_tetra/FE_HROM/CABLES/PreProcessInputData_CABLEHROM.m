function   [MESH,MATPRO,OPERHROM,Fbody,Ftrac,DISP_CONDITIONS,INICOND,DATAHROM,OTHER_data,BasisUall] ...
    =  PreProcessInputData_CABLEHROM(DATAHROM,DATAFE,DIRICHLET,NEUMANN,INITIAL_CONDITIONS,ECMdata,BasisU,BasisTENSIONV,OTHER_INPUTS)
% See
...
PROPMAT = MATPRO.PROPMAT ;
MATPRO = rmfield(MATPRO,'PROPMAT') ; 
[MATPRO,INICOND] = GetMaterialPropertiesHROM(ECMdata,DATAFE,MATPRO,DATAHROM,OTHER_output)  ; 
MATPRO.PROPMAT =PROPMAT ; 

DATAHROM.MESH1D.MaterialType = DATAHROM.MESH1D.MaterialType(ECMdata.setElements)  ; 
...
% else
    % Standard
    OTHER_INPUTS = [] ; 
    [DOFrFE,dRfe] = DirichletCONDtimeCABLE(DIRICHLET,DATAHROM,DATAFE.MESH.ndim,MESH,OTHER_output.GEOproperties,OTHER_INPUTS) ; 
%end

DATAHROM = DefaultField(DATAHROM,'DO_NOT_APPLY_DIRICHLET_CONDITION_FIRST_STEP',0) ; 
if DATAHROM.DO_NOT_APPLY_DIRICHLET_CONDITION_FIRST_STEP == 1     
    for itraj = 1:size(dRfe.a,1) 
        istepZER0=  2; 
        dRfe.a(:,istepZER0) = 0 ; 
    end    
end
...
%     end
 ...
 %ECMdata = DefaultField(ECMdata,'wRED')

 if isfield(ECMdata,'wCECM')
OPERHROM.wSTs =  ECMdata.wCECM;
 else
...
OTHER_data.FbodyDOFr.U = Fbody.U(DOFrFE,:) ; 
...
Ftrac.U =  zeros(size(OTHER_output.Fbody_U)) ;
Ftrac.a = zeros(size(DATAHROM.STEPS)) ;  
 

OTHER_data.FtracDOFr.a = Ftrac.a  ;       % For computing reactions 
OTHER_data.FtracDOFr.U = Ftrac.U(DOFrFE,:) ; 
