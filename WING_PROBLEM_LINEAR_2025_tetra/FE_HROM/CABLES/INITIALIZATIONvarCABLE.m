function [DATA,VAR_n,SNAP] = INITIALIZATIONvarCABLE(DATA,INICOND)
if nargin ==1
    INICOND = [] ;
end
...
% Gauss variables
%------------------------
VAR_n.TENSION = zeros(DATA.MESH.ndofSTRESS,1,nstoreSTRESS) ;

ndofPK1stress = DATA.MESH.ngausT*DATA.MESH.ndim ;

%if DATA.SMALL_STRAIN_KINEMATICS ==1 && DATA.NO_USE_Deformation_gradient_in_Small_Strains == 1
%else
    VAR_n.TENSIONV = zeros(ndofPK1stress,1,nstoreSTRESS) ;
%end

VAR_n.STRAIN = zeros(DATA.MESH.ndofSTRESS,1,nstoreSTRESS) ;
...
if DATA.SKIP_PART_STORE == 0
    ...
    if DATA.MESH.ndim == 2
        ...
        GAUSSV_PROP.TENSIONV.TYPE = 'Vector' ;  % For printing in GID
    GAUSSV_PROP.TENSIONV.LEGEND = 'TENSIONv' ;  % GID's printing
    if DATA.MESH.ndim == 2
        GAUSSV_PROP.TENSIONV.COMP = {'T-x','T-y'} ;
    else
        GAUSSV_PROP.TENSIONV.COMP = {'T-x','T-y','T-z'} ;
    end
    ...
    GAUSSV_PROP.TENSION.LEGEND = 'TENSION' ;
    
    
     
        GAUSSV_PROP.TENSION.TYPE = 'Scalar' ;
     
    
     
    %%%%%%%%%%%%%%%%%%%%
    
    GAUSSV_PROP.STRAIN.LEGEND = 'STRAIN' ;
    GAUSSV_PROP.STRAIN.TYPE = 'Scalar' ;
