function  [xGAUSS,wGAUSS,Xf,W,DATAapprox]  =  TestingGeneralizedGauss3DFUN(NAMEDATA,M,P,DATA)
...
DATA = DefaultField(DATA,'RELOAD_SNAPSHOT_MATRIX',-1) ; 

if DATA.RELOAD_SNAPSHOT_MATRIX == 0 || DATA.RELOAD_SNAPSHOT_MATRIX == -1 
[Xf,W,mu,x,MPOINTS,XY] = GenerateFunExampleNDIM3d(M,P,DATA,DATAFUN);
if DATA.RELOAD_SNAPSHOT_MATRIX == 0
disp('Writting to disc Xf,W...')
save(DATA.DATAREMOVEPOINTS.NAMEWS,'Xf','W','x','mu','x','MPOINTS','XY','-v7.3')      
end
else
load(DATA.DATAREMOVEPOINTS.NAMEWS,'Xf','W','x','mu','x','MPOINTS','XY')
end
...
ExactIntegral = DATAOUT.INTexac ;  %Xf'*W ;
ApproximatedInt =DATAOUT.INTapprox ; % Xf(z,:)'*w ;
...
DATA = DefaultField(DATA,'IRREGULAR_SHAPE_PARAMETRIZATION_ALPHA',[]) ; % = 1;


DATALOC.IRREGULAR_SHAPE_PARAMETRIZATION_ALPHA=  DATA.IRREGULAR_SHAPE_PARAMETRIZATION_ALPHA ;

DATA = DefaultField(DATA,'PROCESS_MATRIX_SNAPSHOTS_BY_BLOCKS',0) ; % = 1;
 

DATALOC.PROCESS_MATRIX_SNAPSHOTS_BY_BLOCKS=  DATA.PROCESS_MATRIX_SNAPSHOTS_BY_BLOCKS ;
DATA = DefaultField(DATA,'TOLERANCE_FORCE_POINTS_TO_REMAIN_WITHIN_THE_DOMAIN',0) ; % = 1;

DATALOC.TOLERANCE_FORCE_POINTS_TO_REMAIN_WITHIN_THE_DOMAIN=  DATA.TOLERANCE_FORCE_POINTS_TO_REMAIN_WITHIN_THE_DOMAIN ;


 
%if DATA.USE_VERSION_REMOVE_POINTS_AT_END_FROM_FE_IMPLEMENTATION == 0
[xGAUSS,wGAUSS,DATAapprox] = GeneralizedGauss3D(DATAOUT.J,...
...
DATAapprox.ExactIntegral = ExactIntegral ; 
