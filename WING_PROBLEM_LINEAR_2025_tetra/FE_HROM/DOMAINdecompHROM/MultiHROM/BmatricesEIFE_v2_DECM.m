...
x = CECM_intforces.xDECM ; % Coordinate points
w = CECM_intforces.wDECM ; % Weights
index_elements = CECM_intforces.setElements ;  % Elements containint points (may be repeated)
index_points = CECM_intforces.DECM_indexes_points ;


%index_elements*DATA.MESH.ngaus_STRESS ;  % Gauss points of the elements defined above
DATA.MESH = DefaultField(DATA.MESH,'nstrain_F',DATA.MESH.nstrain);
index_strain = small2large(index_points,DATA.MESH.nstrain_F) ; % 25-mAY-2024, lARGE STRAINS
% Indexes entries corresponding to strain variables at the GAuss points
...
BmatRED  = BstFE(index_strain,:)*PhiDEF ;
...
if OTHER_IMPLE == 0
    ...
    % BasisStwo_z  =  InterpolationGaussVariablesECM(c,CECM_intforces,DATA.MESH.ngaus_STRESS,DATA.MESH.nstrain) ;
end
...
% If the above = 1, then BasisStwo is a basis matrix for what we call in
...
index_strain = small2large(index_points,DATA.MESH.nstrain) ; % 25-may-2024, LARGE STRAINS
