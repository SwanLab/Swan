function [MESH,MATPRO,OPERFE,Fbody,Ftrac,DISP_CONDITIONS,INICOND,DATA,OTHER_output] =  INPUTS_TRAIN_FEshapePERIOD(DATAINPUT)
% % Typical input data file for running structural problems corresponding
% to "training" cases, in a multiscale problem, with periodic conditions 
% Quadrilateral Q4 element 
% See comments in /home/joaquin/Desktop/CURRENT_TASKS/MATLAB_CODES/TESTING_PROBLEMS_FEHROM/104_EIFEM_plast2D/10_PeriodicQUADL.mlx
% Archetypical file defining the missing inputs: /home/joaquin/Desktop/CURRENT_TASKS/MATLAB_CODES/TESTING_PROBLEMS_FEHROM/104_EIFEM_plast2D/10_PERIODIC_Q4/DEFperiod.m
% JAHO, 17-Nov-2023, Balmes 185, Barcelona
% ----------------------------------------------------------------------
if nargin == 0
    load('tmp.mat')
end
...
% % ------------------------%
t0 = DATAINPUT.t0 ; tEND =  DATAINPUT.tEND ;
...
iloadstate = 1 ;
DIRICHLET(iloadstate).AMPLITUDE = DATAINPUT.PARAM ;  % % Translation (centroid face)
...
DATAINPUT = DefaultField(DATAINPUT,'TypeFunctionDisplacementTRAINING','PERIODICITY_ALL_FACES_WITH_CORNERS') ;
DATAINPUT = DefaultField(DATAINPUT,'TypeFunctionDisplacementInterfaces','QUADRILATERAL_LINEAR') ;
DATAINPUT = DefaultField(DATAINPUT,'PERIODICITY_FACES',{[1,3],[2,4]}) ;
...
OTHER_INPUTS.TypeFunctionDisplacementTRAINING  = DATAINPUT.TypeFunctionDisplacementTRAINING ;
OTHER_INPUTS.TypeFunctionDisplacementInterfaces  = DATAINPUT.TypeFunctionDisplacementInterfaces ;
OTHER_INPUTS.PERIODICITY_FACES  = DATAINPUT.PERIODICITY_FACES ;

DATAINPUT = DefaultField(DATAINPUT,'Ufluct',[]) ;
OTHER_INPUTS.Ufluct  = DATAINPUT.Ufluct ;
