function [MESH,MATPRO,OPERFE,DATA,TIME_COMPUT,DISP_CONDITIONS,OTHER_output] =  EIFECODElargeDYN(DATAFILE,PARAMETERS)
% EMPIRICAL INTERSCALE FINITE ELEMENT CODE,  dynamic regime
% JAHO, 20-March-2023, Clinica TEKNON, Barcelona
if nargin ==0
    load('tmp3.mat')
end
TIME_COMPUT=[] ;
...
if PARAMETERS.ONLY_PRINT_GID == 0
    ...
    [ OPERFE]= ComputeCini_B(DATA,OPERFE,MATPRO,VAR) ; 
...
if PARAMETERS.PRINT_RIGID_BODY == 1 && ~isempty(DISP_CONDITIONS)
 ...
 for icluster = 1:length(DATA.STORE.NSTEPS_CLUSTER)
    if  isempty(DATA.FOLLOWER_LOADS) ||  isempty(DATA.FOLLOWER_LOADS.HYDROSTATIC.NAME_MESH_WATERLINE)
       ...
       OTHER_output.strainCOARSE_history =     GidPostProcess_VECT_EIFEbub(MESH,DATA,icluster,DATAinpGID,OTHER_output.PROPMAT,PARAMETERS);
        else
