clc
clear all
load('CHECK_PERIODIC.mat')

load('SNAP_DISP_prueba.mat','SNAPdisp')
...
if  ~ischar(COMB_FACES)
    ...
    if ~isempty(DATAoffline.MESH_QUADRATIC_PARENT_DOMAIN)
...
DATAloc.NumberOfGaussPointsPerElement = [2,2] ;
...
FESHAPE_coarse_elem_transf_coord.COOR =COORquad ; %
FESHAPE_coarse_elem_transf_coord.CN =1:size(COORquad,1) ;
...
% ------------------
% ORTHOGONAL TO Vrb
PhiDEF = cell2mat(SNAPdisp) ; %(:,2:2:end) ; 
PhiDEF = PhiDEF(:,2:2:end) ; 

PhiDEF_f = PhiDEF(MESH.faceDOFSall,:) ;
PhiFLUC = PhiDEF_f ;%  PprojDEF_operator_loc(Vrb,Mintf,PhiDEF_f);
