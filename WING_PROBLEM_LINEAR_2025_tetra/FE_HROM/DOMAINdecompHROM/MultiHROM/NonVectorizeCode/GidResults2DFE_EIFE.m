function GidResults2DFE_EIFE(NameFile_res,ndime,NODAL_VECTOR,NODAL_SCALAR,MESH,DATAIN)
% INPUT DATA
%dbstop('5')
if nargin == 0
    load('tmp.mat')
...
for istepLOC = 1:length(stepsSH)
    ...
    for imesh = 1:length(MESH)
        ...
        for ivar = 1:length(MESHloc.GAUSS_SCALAR)
            ...
            VAR = MESHloc.GAUSS_SCALAR(ivar).VAR ;
        ...
        for ivar = 1:length(MESHloc.GAUSS_VECTOR)
            ...
            VAR = MESHloc.GAUSS_VECTOR(ivar).VAR ;
        ...
        for ivar = 1:length(MESHloc.GAUSS_MATRIX)
            ...
            VAR = MESHloc.GAUSS_MATRIX(ivar).VAR ;
            NAMEFIELDS.COMP = MESHloc.GAUSS_MATRIX(ivar).COMP ;
            ELEMENTS = MESHloc.GAUSS_MATRIX(ivar).ELEMENTS ;
            GidPostPlotMatGaussEIFE(VAR,nstrain,npg,time_step,NAMEFIELDS,fid_res,ndime,ELEMENTS,NAMEMESH_loc)
