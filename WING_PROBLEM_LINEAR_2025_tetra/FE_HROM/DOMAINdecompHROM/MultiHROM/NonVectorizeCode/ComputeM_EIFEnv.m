function M = ComputeM_EIFEnv(COOR,CN,Nmat,WEIGHTS,  PROPMAT,MaterialType,TRANSF_COORD) ;
%%%%
...
for e = 1:nelem
    %dens = densglo(e) ;  %  
    EIFEoper = PROPMAT(MaterialType(e)).EIFE_prop(TRANSF_COORD{e}.IndexParentDomain) ;
    CNloc = CN(e,:) ;   % Coordinates of the nodes of element "e"
    Xe = COOR(CNloc,:)' ;     % Computation of elemental stiffness matrix
    Me = ComputeMe_EIFEnv(EIFEoper,Nmat{e},WEIGHTS{e}) ;
    for anod=1:nnodeE
        a = Nod2DOF(anod,ndim) ;
        for bnod= 1:nnodeE
            b = Nod2DOF(bnod,ndim) ;
