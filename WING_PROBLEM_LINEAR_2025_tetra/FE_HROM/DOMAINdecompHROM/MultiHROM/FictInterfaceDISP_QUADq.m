function [Vall,Mintf,MESH,DATA,INFOPLOTMESHBND] = FictInterfaceDISP_QUADq(PhiDEF,PsiDEFf,MESH,DATAcommon,DATA,Mintf,PhiRB,PsiRBf,DATAoffline)
% See /home/joaquin/Desktop/CURRENT_TASKS/MATLAB_CODES/TESTING_PROBLEMS_FEHROM/101_MULTI2D_2023/03_HETEROG/02_ELLIP_QUAD.mlx
if nargin == 0
    load('tmp.mat')
end
...
%2.) Determination CORNERS 1, 2 3 AND 4

% CORNER 1: Intersection faces
COMB_FACES= {[1,4],[1,2],[2,3],[3,4]} ;
DATAoffline = DefaultField(DATAoffline,'CORNERS_DEFINING_MACRO_ELEMENT_AS_INTERSECTIONS_ENTITIES',COMB_FACES) ;
COMB_FACES = DATAoffline.CORNERS_DEFINING_MACRO_ELEMENT_AS_INTERSECTIONS_ENTITIES ;
ndim = 2;
ncorners = 4;
...
% -------------------------------------------------------------------------------------------------------------------
% . WE ARE DEALING NOW WITH A QUADRATIC
% QUADRILATERAL ELEMENTS. ACCORDINGLY, WE HAVE TO DETERMINE
% 1) CORNER NODES
% 2) MIDLINE NODES (THE CENTROID OF EACH OF THE BOUNDARY LINES)
% 3) CENTRAL NODE ---THIS ONE IS JUST AS A MEANS FOR DETERMINING THE
% BOUNDARY SHAPE FUNCTIONS

if  ~ischar(COMB_FACES)
    COORmid = zeros(4,2);
    for icorner = 1:length(COMB_FACES)
        iface = COMB_FACES{icorner}(1) ;  %
        jface = COMB_FACES{icorner}(2) ;
        % -----------------------------------------------------------------------------------------------
        CORNERS(icorner) =    intersect(MESH.BNDinterface(iface).NODES,MESH.BNDinterface(jface).NODES) ;
        COORmid(icorner,:) = MESH.BNDinterface(jface).CENTROID ;
    end
    COORcorners = MESH.COOR(CORNERS,:);
    COORquad = [COORcorners; COORmid; 0 0] ; % 0 0 is the centroid
else
    COORquad = [] ;
...
if ~isempty(COORquad)
COORquad = MidsideCoordintesQQ(COORquad,DATAoffline.MESH_QUADRATIC_PARENT_DOMAIN,MESH.GEOproperties.CENTROID) ;
else
    % COORquad read from file 
    COORquad = COORquadFromFileQ(DATAoffline.MESH_QUADRATIC_PARENT_DOMAIN,MESH.GEOproperties.CENTROID) ;
...
if PLOT_POINTS == 1
    ...
    for ipoints = 1:size(COORquad)
        plot(COORquad(ipoints,1),COORquad(ipoints,2),'k*')
        text(COORquad(ipoints,1),COORquad(ipoints,2),['  ',num2str(ipoints)])
    end
end
...
MESHLOC.COOR = COORquad  ;
MESHLOC.CN = [1,2,3,4,5,6,7,8,9];
DATAloc.MESH = MESHLOC ;
DATAloc.NumberOfGaussPointsPerElement = [3,3] ;
DATAloc.MESH.TypeElement = 'Quadrilateral' ;
...
if OLD_METHOD == 1
    ...
    [Nshape,~,~] = ShapeFunDer_inversemapping(COORquad',COORbnd,TypeElement,DATAinp) ;
    DATAlocSHAPE = []  ;
...
FESHAPE_coarse_elem_transf_coord.COOR =COORquad(1:end-1,:) ; %
FESHAPE_coarse_elem_transf_coord.CN =1:(size(COORquad,1)-1) ;
