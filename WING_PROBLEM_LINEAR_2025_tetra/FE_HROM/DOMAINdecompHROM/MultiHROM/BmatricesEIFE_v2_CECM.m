...
x = CECM_intforces.xCECM ; % Coordinate points
w = CECM_intforces.wCECM ; % Weights
index_elements = CECM_intforces.setElements ;  % Elements containint points (may be repeated)
index_points = index_elements*DATA.MESH.ngaus_STRESS ;  % Gauss points of the elements defined above
index_strain = small2large(index_points,DATA.MESH.nstrain) ; % Indexes entries corresponding to strain variables at the GAuss points
...
Bmat  =  InterpolationGaussVariablesECM(Bmat_all,CECM_intforces,DATA.MESH.ngaus_STRESS,DATA.MESH.nstrain_F) ;

%     % GENERAL IMPLEMENTATION
%     % We begin by defining the reduced Bmatrix
BmatRED_all =  BstFE*PhiDEF ;
%     % Now we interpolate this matrix
BmatRED  =  InterpolationGaussVariablesECM(BmatRED_all,CECM_intforces,DATA.MESH.ngaus_STRESS,DATA.MESH.nstrain_F) ;
% %
%
