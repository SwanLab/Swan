function [MESH,MATPRO,OPERFE,DATA,TIME_COMPUT,DISP_CONDITIONS,OTHER_output] =  EIFECODElargeDYN_COROT(DATAFILE,PARAMETERS)
% EMPIRICAL INTERSCALE FINITE ELEMENT CODE,  dynamic regime, co-rotational,
% large strains 
% Copy of EIFECODElargeDYN
% JAHO, 22-O4T-2023, Balmes 185,  Barcelona
if nargin ==0
    load('tmp3.mat')
end
TIME_COMPUT=[] ;
...
if PARAMETERS.ONLY_PRINT_GID == 0
    ...
    if PARAMETERS.ONLY_PRINT_GID == 0
        if DATA.ISDYNAMIC == 0
            [DATA,CONVERGED,QrotTIME,QrotINI]=NONLINEARstaticLARGE_COROT(DATA,DISP_CONDITIONS,VAR,OPERFE,SNAP,Fbody,Ftrac,MATPRO) ;
        else
