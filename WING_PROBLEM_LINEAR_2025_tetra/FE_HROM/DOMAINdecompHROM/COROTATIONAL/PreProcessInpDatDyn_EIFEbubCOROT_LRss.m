...
%--------------------------------------------------------------------------
%  PreProcessInpDatDyn_EIFEbubCOROT_LRss
%
%  Pre-processing function for dynamic simulations based on the
%  Empirical Interscale Finite Element Method (EIFEM) using a consistent
%  co-rotational formulation.
%
%  This version supports both small-strain/large-rotation and 
%  large-strain/small-rotation regimes, handled domainwise.
%
%  The underlying theory is detailed in the document:
%  "EIFEM_largeROTfinal.pdf", with specific reference to the chapters
%  on the variational principle (Sec. 4), geometric setup (Sec. 3),
%  rotation updates (Sec. 12.8), and mass/stiffness assembly 
%  (Secs. 8.1, 12.1–12.6).
%
%  Main tasks of this routine include:
%   - Generating or importing the coarse-scale mesh
%   - Computing geometric and material property matrices (e.g., OPERFE, MATPRO)
%   - Defining deformation kinematics based on the chosen regime
%   - Constructing mass, stiffness, and damping matrices
%   - Applying Dirichlet and Neumann boundary conditions
%   - Incorporating body forces, including gravitational effects
%   - Initializing state variables and dynamic conditions
%
%  Particular attention is given to the computation of the internal force 
%  vector and consistent tangent matrices (both material and geometric),
%  which require the rotation matrices Q and the appropriate
%  downscaling operators. These are computed via a dedicated set of
%  functions and operators introduced in the training phase.
%
%  Mass matrices are computed using diagonal weighting techniques,
%  and gravity-induced body forces are correctly rotated when rigid body
%  motion is involved. The rotation matrices are updated using 
%  Rodrigues’ exponential formula (Sec. 12.8).
%
%  Damping is optionally estimated from target natural frequencies or 
%  specified manually, and incorporated consistently into OPERFE.
%
%  Boundary conditions and follower loads (e.g., hydrostatic) are supported
%  in a modular fashion, allowing flexibility across applications.
%
%  The formulation includes amendments described in Section 8.1 to
%  ensure compatibility of rotations and correct residual definitions for 
%  the internal force vector in the small strain regime:
%
%     F_int_cmp = F_int + Ssp(F_int) * P̂_γr * d_rel^Q
%
%  Authors:
...
%
%  Related examples and testing scripts:
%    - 109_EIFEM_largeROT/05_COROT_SSLR_LSSR.mlx
%    - 109_EIFEM_largeROT/10_AUXETIC_2D.mlx
%
%--------------------------------------------------------------------------
...
% Adaptation of PreProcessInputDataDyn1_EIFEbub.m to  consistent method
% Small strains/Large rotations, or Large strains/Small rotation (domainwise)
% FORMULATION
...
% Change introduced on April 26th 2025, see 
...
if isempty(DATA.MESH_GIVEN_BY_THE_USER)
MESH = GeometryMeshUNC(DATA,OTHER_INPUTS) ;
else
    MESH = DATA.MESH_GIVEN_BY_THE_USER ; 
end
