function [D_BmatIst,Nmat,WEIGHTSinteg,TRANSF_COORD,CN,DATA,D_KcLINloc,CN_SUPPORT,QrotINI,lambdaLEN,D_Wecm,INDEXsparseROTmat,...
    LboolCall,XcALL,D_YcmpD,D_Zfict] = B_N_matricesEIFEbubCOROT2(COOR,CN, PROPMAT,MaterialType,DATA,TypeElement,...
    CN_SUPPORT)
...
% 2-dec-2O24, BALMES 185, BARCELONA
% --------------------------------------------------------------
if nargin == 0
    load('tmp1.mat')
end
...
D_YcmpD= blkdiag(D_YcmpD{:}) ;
D_Zfict  = blkdiag(D_Zfict{:}) ; ;
