function [Nmat,WEIGHTSinteg] = Nmat_weights_EIFEbubCOROT(DATA,TRANSF_COORD,EIFEoper_all)
if nargin == 0
    load('tmp1.mat')
end
...
% JAHO,11-March-2023/18-May-2023/13-Oct-2023 
if DATA.UNIFORM_SCALING_REFERENCE_ELEMENT == 1
    ...
    if  OLD_METHOD_SIMPLE == 1
        ...
        if ~isempty(EIFEoper_all(indCHOSEN).INFO.DOFsBUB)
            ...
            % LEFT
            nDOFSloc =  (size(Nmat,1)) ;  % Number of integration points times number of spatial dimensions
            nSD = length(TRANSF_COORD.CentCglo); % Number of spatial dimensions
            nnodesLOC = nDOFSloc/nSD ;  % Number of integration points 
            ROTglo = cell(1,nnodesLOC) ;
            ROTglo(:) = {sparse(TRANSF_COORD.QrotINI)} ;
            %  ROTglo = blkdiag(ROTglo{:}) ;
            Nmat = blkdiag(ROTglo{:})*Nmat; % diagonal matrix x Nmat 
            
            indB = 1:size(Nmat,2) ;  % All coarse-scale DOFs
            indBUB = EIFEoper_all(indCHOSEN).INFO.DOFsBUB ;   % Bubble DOFs
            indB(indBUB) = [] ;   % Boundary DOFs
            
            NmatB = Nmat(:,indB) ;  % Boundary term
            Nbub  =  Nmat(:,indBUB) ;   % Bubble term  ---Unaffected  
            % RIGHT
            nDOFSloc =  (size(NmatB,2)) ;
            nnodesLOC = nDOFSloc/nSD ;
            ROTglo = cell(1,nnodesLOC) ;
            ROTglo(:) = {sparse(TRANSF_COORD.QrotINI')} ;
            NmatB = NmatB*blkdiag(ROTglo{:});
          ...
          %   ndim = size(TRANSF_COORD.ROTATION,2) ;
            nBUB = length(EIFEoper_all(indCHOSEN).INFO.DOFsBUB) ;
            Nmat = ExtendedBmatrixBUB(NmatB,Nbub,nSD,nBUB) ;
