function [OPERFE,Nst,wSTs_RHS,NstT_W_N_boundaries,ngaus_RHS,GEOproperties,MESH,DATA] = ...
    GeometricMatricesFun_EIFEbubCOROT_LRss(MESH,nstrain,PROPMAT,DATA)
%--------------------------------------------------------------------------
%  GeometricMatricesFun_EIFEbubCOROT_LRss
%
%  Constructs geometric matrices and operators required by the EIFEM
%  formulation for dynamic simulations involving large rotations and small
%  (or large) strains. This routine is a corotational adaptation of 
%  `GeometricMatricesFun_EIFEbub.m`, consistent with the variational
%  framework described in the EIFEM_largeROTfinal.pdf (especially Sections
%  4, 6.3, and 12.6–12.8).
%
%  The function generates:
%   - Element-level B-matrices and transformation operators (Bmat, Qrot)
%   - Coarse-scale shape function matrices (Nst) for internal and boundary forces
%   - Centroid and rotational inertia data for the domain
%   - Operators for consistent internal force assembly, including 
%     the sparse index structures for fast assembly
%
%  The formulation uses:
%   - Corotational updates at the element level, with Qrot tensors
%     stored at initialization (QrotINI)
%   - A consistent mapping between global and local frames
%     via `D_PdownsRBlROT` and `D_KcLINloc`
%   - Bubble-enriched shape functions for internal variable approximation
%
%  Boundary integration is handled through `NstT_W_N_boundaries`, constructed
%  for each user-defined face using pre-processed face connectivity from GID.
%
%  The function also computes rigid body modes and geometric mass matrices
%  for both the volume and the boundary (see Sections 12.6.2 and 12.6.3).
%
%  Special care is taken to support uncoupled (multiphysics) FE types,
%  as well as extended connectivity structures for high-fidelity models.
%
%  Key output fields:
%   - OPERFE: structure collecting all geometric matrices and operators
%   - GEOproperties: centroid, volume, inertia, face normals/tangents
%   - MESH: enriched with coordinate transformations and auxiliary indexing
%
%  Author:
...
% Matrices depending on the finite element mesh (i.e., that can be pre-computed)
% Adaptation of GeometricMatricesFun_EIFEbub.m  for  EIFEM COROTATIONAL  Large strains/Small rotation (domainwise)
...
% JAHO, 05-Feb-2025, Balmes 185, Barcelona
% ----------------------------------------------------------------------------
...
[OPERFE.D_BmatIst,Nmat_allelem,WEIGHTSinteg,TRANSF_COORD,MESH.CN,DATA,OPERFE.D_KcLINloc,...
    MESH.CN_SUPPORT,OPERFE.QrotINI,OPERFE.lambdaLEN,OPERFE.D_Wecm,...
    OPERFE.INDEXsparseROTmat,OPERFE.LboolCall,OPERFE.XcALL,OPERFE.INDEXsparseFINT,...
    OPERFE.D_PdownsRBlROT,OPERFE.XcALLloc,OPERFE.D_AspMAT,OPERFE.INDEXsparseROTelem_byROWS  ]=...
    B_N_matricesEIFEbubCOROT_LRss(MESH.COOR,MESH.CN, PROPMAT,MESH.MaterialType,DATA,MESH.TypeElement,CN_SUPPORT ) ;
...
%     IDENTITY_F  = [] ;
    [ DATA.MESH.number_cecmpoints_INTERNAL_FORCES,~] = cellfun(@size,WEIGHTSinteg.INTforces) ;   % Number of 
...
GEOproperties.VOLUME = VOLUME ;
    ...
    GEOproperties.CENTROID = [] ;
     GEOproperties.INERTIA = [] ;
      GEOproperties.VOLUME = [] ;
