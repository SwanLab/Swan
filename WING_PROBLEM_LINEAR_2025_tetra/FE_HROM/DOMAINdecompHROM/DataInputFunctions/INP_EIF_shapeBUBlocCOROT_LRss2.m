function [MESH,MATPRO,OPERFE,Fbody,Ftrac,DISP_CONDITIONS,...
    INICOND,DATA,OTHER_output] =  INP_EIF_shapeBUBlocCOROT_LRss2(DATAINPUT)
%--------------------------------------------------------------------------
% function [MESH, MATPRO, OPERFE, Fbody, Ftrac, DISP_CONDITIONS, ...
%           INICOND, DATA, OTHER_output] = INP_EIF_shapeBUBlocCOROT_LRss2(DATAINPUT)
%
% PURPOSE:
%   Initializes all problem-specific data for an EIFEM-based simulation
%   involving large rotations and small/large strains using a corotational
%   formulation. This script serves as a flexible input driver for training
%   and testing parametric cases in the context of model reduction,
%   particularly in the EIFE (Empirical Interscale Finite Element) framework.
%
% FUNCTIONALITY:
%   - Defines time-stepping scheme and Newton-Raphson solver parameters
%   - Loads or generates mesh, material properties, and finite element operators
%   - Sets Dirichlet and Neumann boundary conditions, initial conditions
%   - Constructs input structures for the training phase of EIFEM
%   - Determines snapshot storage strategy and output file naming
%   - Prepares data for printing results in GID
%
% INPUT:
%   DATAINPUT : Struct containing user-defined input fields. Key subfields:
%       - DATAcommon.SMALL_STRAIN_KINEMATICS
%       - DATAcommon.INPUTS_PARAMETERS (parametric study values)
%       - DATAcommon.DirichletBoundaryConditions (function handle)
%       - DATAcommon.NeumannBoundaryConditions (optional)
%       - DATAcommon.t0 / tEND (time interval)
%       - FILE_MATERIAL_DATA (name of .m function defining PROPMAT)
%       - NameParamStudy (name to tag this simulation)
%
% OUTPUT:
%   MESH              : Struct containing coordinates, connectivities, and element info
%   MATPRO            : Material properties structure
%   OPERFE            : Finite element operator matrices
%   Fbody             : Global body forces vector
%   Ftrac             : Global traction forces vector
%   DISP_CONDITIONS   : Prescribed displacement conditions for the simulation
%   INICOND           : Initial condition structure (displacement/velocity)
%   DATA              : Struct with simulation configuration and flags
%   OTHER_output      : Struct containing supplementary information (e.g., PROPMAT)
%
% REFERENCE:
%   See usage in:
...
%
% AUTHOR:
%   J.A. Hernández Ortega, 5-Feb-2025, Balmes 185, Barcelona
%   Comments by ChatGPT4, 13-May-2205
%--------------------------------------------------------------------------
...
DATA.PRINT.NSTEPS =   DATAINPUT.DATAcommon.DATA_PRINT_NSTEPS ;
 ...
 DATAINPUT.DATAcommon= DefaultField(DATAINPUT.DATAcommon,'PROCEED_WITH_NEGATIVE_JACOBIANS',1) ; 
DATA.PROCEED_WITH_NEGATIVE_JACOBIANS = DATAINPUT.DATAcommon.PROCEED_WITH_NEGATIVE_JACOBIANS ; 

% PARAMETERS RELATED WITH TIME INTREGRATION SCHEME
% PARAMETERS RELATED WITH TIME INTREGRATION SCHEME
ISDYNAMIC = 0 ;
DATAINPUT.DATAcommon = DefaultField(DATAINPUT.DATAcommon,'ISDYNAMIC',ISDYNAMIC) ;



DATA.ISDYNAMIC = DATAINPUT.DATAcommon.ISDYNAMIC;
DATA.INTEGRATION_SCHEME.TYPE ='NEWMARKbossak' ;
DATA.INTEGRATION_SCHEME.NEWMARKbossak.aBOSSAK =-1/3;
DATA.INTEGRATION_SCHEME.steps_KINEM_VARIABLES_TO_STORE = 1; %tegrating systems
% (or for doing extrapolations using, say, the SVD...). By default = 1 )% figure(1)
% hold on
DATA.EstimateCoefficientsDampingFromNatFrequencies = 1;
DATAINPUT = DefaultField(DATAINPUT,'CoefficientsDampingAllNatFrequencies',0.1) ;
DATA.CoefficientsDampingAllNatFrequencies = DATAINPUT.CoefficientsDampingAllNatFrequencies;

DATA.INTEGRATION_SCHEME.steps_STRESS_VARIABLES_TO_STORE =1;


DATA.SMALL_STRAIN_KINEMATICS = DATAINPUT.DATAcommon.SMALL_STRAIN_KINEMATICS ;

DATAINPUT.DATAcommon = DefaultField(DATAINPUT.DATAcommon,'TOL_CONVERGENCE_angle_rads',1e-3) ;
DATAINPUT.DATAcommon = DefaultField(DATAINPUT.DATAcommon,'NITER_ROTATIONS_UPDATE',10) ;

DATA.TOL_CONVERGENCE_angle_rads =  DATAINPUT.DATAcommon.TOL_CONVERGENCE_angle_rads ;
DATA.NITER_ROTATIONS_UPDATE =  DATAINPUT.DATAcommon.NITER_ROTATIONS_UPDATE ;

% TOLERANCE NEWTON_RAPHSON
DATA.NEWTON_RAPHSON.TOL_FORCES_REL = 1e-6 ; %1e-8;
DATA.NEWTON_RAPHSON.TOL_DISPLAC_ABS = 1e-10;

DATAINPUT.DATAcommon = DefaultField(DATAINPUT.DATAcommon,'TOL_COMPATIBILITY_RESIDUAL',1e-10) ; 

DATA.NEWTON_RAPHSON.TOL_COMPATIBILITY_RESIDUAL = DATAINPUT.DATAcommon.TOL_COMPATIBILITY_RESIDUAL;
...
DATAINPUT  = DefaultField(DATAINPUT,'MESH_GIVEN_BY_THE_USER',[]); 
DATA.MESH_GIVEN_BY_THE_USER = DATAINPUT.MESH_GIVEN_BY_THE_USER ;
...
DATAINPUT = DefaultField(DATAINPUT,'FILE_MATERIAL_DATA','MATERIAL_EIFE') ;
...
% iloadstate = 1 ;
% DIRICHLET(iloadstate).AMPLITUDE = DATAINPUT.DATAcommon.INPUTS_PARAMETERS ;  % % Translation (centroid face)
% DIRICHLET(iloadstate).TIMEFUN =  @(t) t/tEND ;  % Function definining the temporal evolution
% DIRICHLET(iloadstate).INTERVAL =  [0,tEND];  % Outside this interval, the amplitude is assumed to be zero
...
DATAINPUT.DATAcommon = DefaultField(DATAINPUT.DATAcommon,'MESHcoarse',[]) ;
DATAINPUT.DATAcommon = DefaultField(DATAINPUT.DATAcommon,'LabelEntitiesDefiningBoundary',[]) ;

OTHER_INPUTS.MESHcoarse_FOR_DIRICHLET_BOUND_COND = DATAINPUT.DATAcommon.MESHcoarse  ;
OTHER_INPUTS.LabelEntitiesDefiningBoundary = DATAINPUT.DATAcommon.LabelEntitiesDefiningBoundary ; %  LabelEntitiesDefiningBoundary ;

DATAINPUT.DATAcommon = DefaultField(DATAINPUT.DATAcommon,'TypeFunctionDisplacementTRAINING','');



DATAINPUT.DATAcommon = DefaultField(DATAINPUT.DATAcommon,'ToleranceDeformationalPartTransformationParentDomain',1e-4);

DATA.ToleranceDeformationalPartTransformationParentDomain = DATAINPUT.DATAcommon.ToleranceDeformationalPartTransformationParentDomain ;
...
[MESH,MATPRO,OPERFE,Fbody,Ftrac,DISP_CONDITIONS,INICOND,DATA,OTHER_output] ...
    =  PreProcessInpDatDyn_EIFEbubCOROT_LRss(DATA,PROPMAT,DIRICHLET,NEUMANN,INITIAL_CONDITIONS,OTHER_INPUTS) ;
OTHER_output.PROPMAT = PROPMAT ;
...
%DATA.MESH.ndof = size(OPERFE.Bst,2) ; %
...
DATA.STORE.VAR.PK2STRESS_incre = 0;
