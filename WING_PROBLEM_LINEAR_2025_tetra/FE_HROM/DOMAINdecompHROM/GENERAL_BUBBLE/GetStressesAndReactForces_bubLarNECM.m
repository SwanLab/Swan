function [SNAPreactFORCESproj,SNAPstressSTWOproj,SNAPstressPonePROJ,NAME_BASE]  =...
    GetStressesAndReactForces_bubLarNECM(NAME_BASE,DATAcommon,NAMEsnap_base,DATAoffline,...
    INFO_RVE,BasisDEFdisp,DATA,OPERFE,MATPRO,Fbody,Ftrac,OTHER_output,PhiRB,Kstiff)
...
for iproj = 1:length(CASES)
    ...
    DATA  = INFO_SNAPSHOTS.EMPLOYED_INPUTS.DATA ; % 22-May-2024
    ...
    SNAPstressSTWOproj_LOC = cell2mat(SNAPstressSTWOproj_LOC) ;   % Approximate
        ...
        SNAPstressPonePROJ_LOC = cell2mat(SNAPstressPonePROJ_LOC) ;   % Approximate
...
% if  DATAoffline.CHECK_ACCURACY_PROJECTED_STRESSES ==1
%    warning(['RUN AGAIN  SETTING  DATAoffline.CHECK_ACCURACY_PROJECTED_STRESSES = 0  (or CHECK_ACCURACY_PROJECTED_STRESSES_BUBBLE)']) ;  
%    error('----')
%  
%  end
