function [COOR,CN,TypeElement,CONNECTb,TypeElementB,Materials,MATERIALNEW,DOMAINVAR]= ...
    MeshGenerationRepeat3D_faces(NAMEMSH,DATA,MATERIAL)
...
if ~isempty(DATA.angROTATION_FACE) && DATA.angROTATION_FACE ~= 0
    % Curved elements
    DATA3D = CurvedDomains_beamsGEN(DATA3D,DATA.angROTATION_FACE,DATA) ;
else
    % No curved elements
...
if nDOM== 1
    ...
    idir = 1;
    COOR(:,idir) = DATA3D.CENTRf1(idir) + eFACTOR*( COOR(:,idir) - DATA3D.CENTRf1(idir)) ;
    ...
    % Translation vector
    translationVECTOR =  DATA3D.CENTRf2- DATA3D.CENTRf1;  % This should be changed for curved elements
    ...
    eFACTOR =DATA.MakeMeshByRepetition.DILATATION_SLICES_x(1) ;
    idir = 1;
    ...
    if eFACTOR >1
        ...
        xINI  =  DATA3D.CENTRf1 +    translationVECTOR  ;
    end
