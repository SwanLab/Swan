...
a_A = INPUTS_LOC.GENERALIZED_FORCES_ENDS_BEAM{1}(:) ; % Rigid body amplitudes face 1
a_B = INPUTS_LOC.GENERALIZED_FORCES_ENDS_BEAM{2}(:) ; % Rigid body amplitudes face 2
if iscell(a_A)
    a_A = cell2mat(a_A) ; 
end
if iscell(a_B)
...
%if OKK ==1
[Gb,dR,DOFr,DOFm,AREA,R,Fpnt_Dirichlet] = PRESCRIBED_FLUCmixed_JOINTS_2faces_fun(a_A,a_B,DOMAINVAR,COOR,CONNECTb,TypeElementB,DATA) ; 
% else
