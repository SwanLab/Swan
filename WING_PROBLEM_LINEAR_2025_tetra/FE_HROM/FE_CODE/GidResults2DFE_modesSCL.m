function GidResults2DFE_modesSCL(NameFile,COOR,CONNECT,TypeElement,MODES,posgp,DATA)
% INPUT DATA
%dbstop('4')
if nargin == 0
    load('tmp.mat')
...
for imode = 1:size(MODES,2)
    d = MODES(:,imode) ; 
    var = reshape(d,1,nnod) ;
    time_step = TIMEVECTOR(istep) ;
    fprintf(fid_res, ['Result "Mode ',num2str(imode),'"  "Load Analysis" '  num2str(time_step) ' Scalar OnNodes  \n' ],[]);
%     if ndime==2
