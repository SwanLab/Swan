...
%dbstop('5')
%clear all
% load('tmp1.mat') ; warning('Borra estoooooooo')
%dbstop('7')
t = INPUTS_LOC.DISTLOAD.t ;
LIMITSglo =  INPUTS_LOC.DISTLOAD.LIMITS ;
FACES = fieldnames(t) ;
...
for iface = 1:length(FACES)
    ...
    if any(tLOC)
        ...
        %%% Now we search for the nodes whose complementary coordinate lies
        %%% between  LIMITS(1)  and LIMITS(2)
        LIMITSGLO = LIMITSglo.(FACES{iface}) ;
        dimCOMPglo = setdiff(1:ndim,dimREF) ;  % Complementary coordinates
        % Loop over complementary coordinates 
        % -----------------------------------
        for  icompl = 1:length(dimCOMPglo)
            dimCOMP = dimCOMPglo(icompl) ; 
            LIMITS = LIMITSGLO{icompl} ; 
            coorMIN = min(COOR(BoundaryNodes,dimCOMP)) ;  coorMAX = max(COOR(BoundaryNodes,dimCOMP)) ;
            L = coorMAX-coorMIN ;
            limMIN = coorMIN + LIMITS(1)*L ;  limMAX = coorMIN + LIMITS(2)*L ;
            % Points whose coordinate is greater than limMIN
            rnodBASEloc = find( COOR(NODESb,dimCOMP)>limMIN-TOL) ;
            NODESb = NODESb(rnodBASEloc) ;
            % Points whose coordinate is less than limMAX
            rnodBASEloc = find( COOR(NODESb,dimCOMP)<limMAX+TOL) ;
            NODESb = NODESb(rnodBASEloc) ;
        end
        %-----------------------------------------------------
