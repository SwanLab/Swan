function [COOR,CN,TypeElement,TypeElementB, celasglo,  DOFr,dR,...
    ...
    DATA,MaterialType] = INPUT_PERIODIC(INPUTS_LOC,DATA) ;
...
[dummy NameFileMesh_ave ]= fileparts(NameFileMesh) ;
nameWORKSPACE = ['DATAWS/',NameFileMesh_ave,'_WS.mat'] ; % To retrieve already read data
...
[PPP,~,~] = fileparts(DATA.nameWORKSPACE);
if ~exist(PPP,'file')
    mkdir(PPP);
end
...
if DATA.RECALCULATE_STIFFNESS == 1
    ...
    %   DATA.GID_VERSION_WITH_MESHES_DIFFERENT_SURFACES_SEPARATED =1 , 2024
    %   onwards
    % CHANGE INTRODUCED ON APRIL 12TH 2025
    ...
    if DATA.READ_MATERIAL_COLUMN == 1
        if DATA.GID_VERSION_WITH_MESHES_DIFFERENT_SURFACES_SEPARATED == 0
            [MESH]= ReadMeshFileStr(NameFileMesh,'READ_MATERIAL_COLUMN',1)  ;
        else
            [MESH]= ReadMeshFileStr_MULT(NameFileMesh,'READ_MATERIAL_COLUMN',1)  ;
        end
    ...
    save(DATA.nameWORKSPACE,'COOR','CN','TypeElement','CONNECTb','TypeElementB','MaterialType','-v7.3')
...
[DOFr DOFm Gb dR dispMACRO DATA] = Periodic_BoundaryCOND(COOR,CN,MACRODEF,CONNECTb,DATA) ;
