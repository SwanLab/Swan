function [G,uBAR,DOFs,DOFm] = BCs_PRESCRIBED_FLUCTUATIONS_withFORCES(DOFa,DOFb,R,U_n,U_b,gammaB)
...
%          \zero & \U_n & gamma_b \U_b \\
%          \R   & \U_n  &  \zero
%         \end{bmatrix} 
% \end{equation}
nrows = 2*size(R,1) ; 
ncols = size(R,2) + size(U_n,2) + size(U_b,2) ; 
...
iniCOL = finCOL+1; 
finCOL  = size(V,2) ;
V(iniROW:finROW,iniCOL:finCOL) = gammaB*U_b; 
  
 % V_23  = U_b
iniROW = size(U_n,1)+1; 
finROW = size(V,1) ; 
 
V(iniROW:finROW,iniCOL:finCOL) = U_b; 
