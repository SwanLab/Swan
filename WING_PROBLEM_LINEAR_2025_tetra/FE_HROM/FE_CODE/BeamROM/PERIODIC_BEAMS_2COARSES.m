function [G,uBAR,DOFr,DOFm,AREA,R,DOFA,DOFB] = PERIODIC_BEAMS_2COARSES(a_A,a_B,DOMAINVAR,COOR,CONNECTb,TypeElementB,...
...
%   COARSE MESHES  !
% ----------------
[z,~,~,zNODES,Mbar,AREA_coarse_A] = CoarseMeshVariable(NameFileMeshCoarse.A,CentroidFA,COORrelA,nodesfA,COOR_FACE,AREA) ; 
[y,~,~,yNODES,Mast,AREA_coarse_b] = CoarseMeshVariable(NameFileMeshCoarse.B,CentroidFA,COORrelA,nodesfA,COOR_FACE,AREA) ; 

 % Intersection of both sets 
[u, iYU, iZU ]= intersect(y,z) ; 
[zBAR, izBAR]= setdiff(z,u) ; 
[yBAR, iyBAR]= setdiff(y,u) ; 
h = 1:length(DOFA); 
h = setdiff(h,[z;y]) ; 

% Number of constraints 
nconstr = 6 + 6 + length(DOFA) ; 
% number of unknowns 
N = 2*length(DOFA) ; 
...
jini = 1 ;
jfin = length(zBAR) ;
A(iini:ifin,jini:jfin) =  R(z,:)'*Mbar(:,izBAR) ;
b(iini:ifin) =  (R(z,:)'*Mbar)*(R(z,:)*a_A) ;
% Third column 
jini = jfin + length(yBAR) +1 ;
jfin = jini + length(u)-1 ;
A(iini:ifin,jini:jfin) =  R(z,:)'*Mbar(:,iZU) ;
...
ifin = iini + 6 -1 ;
jini = length(zBAR)+1 ;
jfin = jini + length(yBAR)-1 ;
A(iini:ifin,jini:jfin) =  R(y,:)'*Mast(:,iyBAR) ;
b(iini:ifin) =  (R(y,:)'*Mast)*(R(y,:)*a_A) ;
% Third column 
jini = jfin +  1 ;
jfin = jini + length(u)-1 ;
A(iini:ifin,jini:jfin) =  R(y,:)'*Mast(:,iYU) ;

% IDENTITIES MATRICES 
% --------------------
DOFloc = [zBAR; yBAR; u ; h'] ;
iini = ifin+1; 
ifin = nconstr ; 
nrows = length(DOFloc) ; 
jini = 1 ;
