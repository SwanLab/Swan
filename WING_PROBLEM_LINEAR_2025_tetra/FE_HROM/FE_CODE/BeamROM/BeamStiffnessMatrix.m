function   [DATAOUT,MSG] = BeamStiffnessMatrix(BASES,DATA_REFMESH,DATAIN,nBASES_BEAM,DATAsnap,MSG)
...
DATAIN = DefaultField(DATAIN,'IncludeWarpingModesAsEssentialBeamModes',0) ;  % Option no longer used, Aug-2019
% if DATAIN.IncludeWarpingModesAsEssentialBeamModes == 1   OBSOLETE
%     for iii = 1:length(Vrb)
%         Vrb{iii} = [Vrb{iii},DATA_REFMESH.WarpingModesInterface{iii}] ;
%     end
%     DATAOUT.WarpingModesInterface = DATA_REFMESH.WarpingModesInterface ;
% else
DATAOUT.WarpingModesInterface = [] ;
%end
...
%DISP_BOUND = BASES.DISPLACEMENTS_INTERFACES_BOUNDARY ; OBSOLETE,
%22-MAY-2019
...
if isempty(nBASES_BEAM)
    ...
    if  DATAIN.UnifiedApproachForModes == 0
        % Old approach, before 10-Dec-2018
        % ---------------------------------
        DATAOUT.BasisInt = Vrb ;
        V = Vrb ;
        error('option no longer maintained')
    elseif DATAIN.UnifiedApproachForModes == 1
        iface= 1;
        ...
        if DATAIN.INTERFACE_MODES_REACTIONS_WORK.ACTIVE == 1
            % Method 15-May-2019
            error('This method was abandoned in favor of ReactionAndInterfaceLocalModes_UNIFIED, Aug 2019')
            [BasisRdef,V,MSG,BasisUdef,SinvVal_Udef] = ...
                ReactANDintf_WORKREACT(BasisUdef,BasisRdef,f1,f2,DATAIN.TOL_SINGULAR_VALUES_Hqr,...
                nBASES_BEAM,DATA_REFMESH,BasisINTbeam,M{iface},DATAIN,SinvVal_Udef,SinvVal_Rdef,BasisUrb,...
                DATA_REFMESH.M)  ;
        else
            if DATAIN.DoNotUseRigidBodyForInterface == 0
                [BasisRdef,V,MSG,BasisUdef,SinvVal_Udef,Vdef_Morth] = ReactionAndInterfaceLocalModes_UNIFIED(BasisUdef,BasisRdef,f1,f2,DATAIN.TOL_SINGULAR_VALUES_Hqr,...
                    nBASES_BEAM,DATA_REFMESH,BasisINTbeam,M{iface},DATAIN,SinvVal_Udef,SinvVal_Rdef,BasisUrb,...
                    DATA_REFMESH.M,MSG)  ;
            else
                error('No longer used, 28-May-2019')
                [BasisRdef,V] = ReactionAndInterfaceLocalModes_NORIGIDB(BasisUdef,BasisRdef,f1,f2,DATAIN.TOL_SINGULAR_VALUES_Hqr,...
                    nBASES_BEAM,DATA_REFMESH,BasisINTbeam,M{iface},DATAIN,SinvVal_Udef,SinvVal_Rdef)  ;
            end
        end
...
error('No longer used, 28-May-2019')
    ...
    BasisINTbeam = Vrb{iface} ;
    %else
    %    BasisINTbeam = BasisUdef(f2,1:nBASES_BEAM.DISPLACEMENTS) ;
    ...
    %          nBASES_BEAM,DATA_REFMESH,BasisINTbeam,M,DISP_BOUND,DATAIN)  ;
    ...
    [BasisRdef,V,nBOUNDARY_INTFMODES] = ReactionAndInterfaceLocalModes_new(BasisUdef,BasisRdef,f1,f2,DATAIN.TOL_SINGULAR_VALUES_Hqr,...
        nBASES_BEAM,DATA_REFMESH,BasisINTbeam,M{iface},DISP_BOUND,DATAIN,SinvVal_Udef,SinvVal_Rdef)  ;
    MSG{end+1} = ['Number of additional displacement modes =',num2str(size(BasisUdef,2)-nBASES_BEAM.DISPLACEMENTS)];
    MSG{end+1} = ['Number of additional reaction modes =',num2str(size(BasisRdef,2)-nBASES_BEAM.REACTIONS)];
    MSG{end+1} =   ['Number of additional interface modes =',num2str(size(V,2)-nrigid)];
...
DATAOUT.Vfluctuations = Vdef_Morth ; % 4-Nov-2023 
...
SHOW_ALSO_DEFORMATIONAL_MODES = 1;  % 4-Nov-2023
if SHOW_ALSO_DEFORMATIONAL_MODES ==1 
   Vprint =[V,Vdef_Morth]  ; 
else
    Vprint = V; 
end    
   MSG =  PrintInterfaceModes(Vprint, DATAIN, DATA_REFMESH,NODES_faces12,MSG);
MSG{end+1} = '------------------------------------' ;
