function [FtractionE,Rast,rRB,fextDOMred,FORCES_2_PRINT_local ]= ...
    TractionForcesDomain_curve(UPSILONforceTRAC,LOADS,...
    COOR,nnodeE,ndim,Q_tractions,...
    BETA_tractions,D_tractions,nmodes,ISLOCAL,ndimSP,ROTATIONS_DOMAIN,ROTATIONS_DOMAIN_transpose)
...
nelem = length(ROTATIONS_DOMAIN) ;
% Vector of external forces due to traction loads
% -----------------------------------------------
FtractionE = zeros(nnodeE*ndim,nelem) ;
nfaces = min(length(UPSILONforceTRAC.COOR), length(LOADS));
Rast = zeros(nmodes.REACTION,nelem) ;
nmodesRB  =6  ;
if ndimSP == 2
    nmodesRB = 3 ;
end
rRB =zeros(nmodesRB,nelem) ;
fextDOMred =zeros(nmodes.DISPLACEMENT,nelem)  ;
...
for iface =1:nfaces  % loop over all faces of the domain
    ...
    if  ~isempty(LOADF)
        ...
        %  ndimSP = size(COOR_FACE,1); % Number of spatial dimensions
        ...
        if isempty(COOR_DATA) %|| (iscell(COOR_DATA) & isempty(COOR_DATA{1}))
            ...
            if ISLOCAL(iface) == 0
                ...
                Tnodes_loc = repmat(Tuniform,nGAUSSbnd,1) ;
                ...
                Tnodes_loc = zeros(ndimSP,nGAUSSbnd) ;
                for igaus = 1:ngausELEM
                    for idim = 1:ndimSP
                        Tnodes_loc(idim,igaus:ngausELEM:end) = LOADF(1)*NORMALS(idim,:) ;
                    end
                end
                FORCES_2_PRINT_local{iface} =  Tnodes_loc(:,1:ngausELEM:end) ;  % Force for each element
                Tnodes = repmat(Tnodes_loc(:),1,nelem) ;
               % Tnodes
            end
            %      Tnodes = Tnodes(:) ;
        else
