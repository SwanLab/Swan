...
% 
% 
% %WHAT_TO_PRINT = ones(size(IND_ELEM_TYPE)) ; % Print inner elements
% %%  Lateral surfaces
% % --------------------
...
% IND_ELEM_TYPE_INNER = IND_ELEM_TYPE ;  % Interior elements
% IND_ELEM_TYPE = IND_SLICES_INTER ;
% nmaxDOM = max(IND_SLICES_INTER) ;
% WHAT_TO_PRINT = -ones(nmaxDOM,1) ; % If = 1, then 3D
% %  WHAT_TO_PRINT([1,end]) = 1;
% WHAT_TO_PRINT(IND_SLICES_INTER) = 0;
% 
% WHAT_TO_PRINT(IND_ELEM_TYPE_INNER) = 1;
% 
...
% %     MESH3D.SLICES(itypeSLICE).DATA3D.NAME_SLICE  = 'LATERALS' ;
% %
% %     MESH3D.SLICES(itypeSLICE).DATA3D.CN = cell2mat(DISP3D_lateral.CONNECTb') ;  % Connect. matrix, lateral s.
% %      MESH3D.SLICES(itypeSLICE).DATA3D.CNb = [] ;
% %     MESH3D.SLICES(itypeSLICE).NEW_COORDINATES = 0 ;
% 
% % else
...
% % if  nDOM_lat == 0
% %     % No lateral surfaces
% %     IND_ELEM_TYPE_reference = IND_ELEM_TYPE_INNER ;
% % else
% %     % All domains are incluced. HEnce
% %     IND_ELEM_TYPE_reference = IND_SLICES_INTER ;
% % end
% coorINT = MESH1D.COOR ;
...
% 
% for e = 1:length(WHAT_TO_PRINT)
%     if WHAT_TO_PRINT(e) < 0
%         
%     else
%         eGLO = e ;
%         if SIGNxLOCAL == 1
