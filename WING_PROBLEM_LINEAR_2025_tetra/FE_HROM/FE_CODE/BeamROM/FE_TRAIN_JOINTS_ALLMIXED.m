function FE_TRAIN_JOINTS_ALLMIXED(NameFileMeshLOC,NameFileMeshLOC_coarse,NAME_DATA_REF,...
    NAME_ROOT_FE,EXECUTABLE_FOLDER,MESH3D,MESH1D,ijoint,NUMBER_OF_SLICES,...
    VALUE_FORCES,VALUE_MOMENT)
...
FOLDER = cd ;
TYPE_BOUNDARY_CONDITIONS =  'PRESCRIBED_FLUCmixed_JOINTS_2faces';  % Fluctuations are given
...
save(FLUCTUATIONS_BOUNDARY_NameWS,'FLUCTUATION_MATRIX','RIGID_BODY_MATRIX','MASS_MATRIX_GEOMETRIC','FLUCTUATION_STIFF_COEFF')
...
GENERALIZED_FORCES_ENDS_BEAM.RIGHT_END(iproj,1)= VALUE_FORCES;
NUMBER_OF_DOMAINS(iproj) = ndom	;
%---------------------------------------
iproj = 2; % Torsion test
NAMEPROJECTS{iproj} = [NAME_ROOT_FE,NAME_PROJ_LOC{iproj}];
GENERALIZED_FORCES_ENDS_BEAM.RIGHT_END(iproj,4)= VALUE_FORCES;
NUMBER_OF_DOMAINS(iproj) = ndom	;
iproj = 3;  % Pure bending test -y
NAMEPROJECTS{iproj} =  [NAME_ROOT_FE,NAME_PROJ_LOC{iproj}];;
GENERALIZED_FORCES_ENDS_BEAM.RIGHT_END(iproj,5)= VALUE_FORCES;
%GENERALIZED_FORCES_ENDS_BEAM.LEFT_END(iproj,5)= -VALUE;
NUMBER_OF_DOMAINS(iproj) = ndom	;
%---------------------------------------
iproj = 4; % Pure bending test -z
NAMEPROJECTS{iproj} =  [NAME_ROOT_FE,NAME_PROJ_LOC{iproj}];;
GENERALIZED_FORCES_ENDS_BEAM.RIGHT_END(iproj,6)= VALUE_FORCES;
%GENERALIZED_FORCES_ENDS_BEAM.LEFT_END(iproj,6)= -VALUE;
NUMBER_OF_DOMAINS(iproj) = ndom	;
%------- Simple bending test y
iproj = 5;
NAMEPROJECTS{iproj} =  [NAME_ROOT_FE,NAME_PROJ_LOC{iproj}]; ;
GENERALIZED_FORCES_ENDS_BEAM.RIGHT_END(iproj,3)= VALUE_FORCES;
GENERALIZED_FORCES_ENDS_BEAM.RIGHT_END(iproj,5)= VALUE_MOMENT;
NUMBER_OF_DOMAINS(iproj) = ndom	;
%----%------- Simple bending test z
iproj = 6;
NAMEPROJECTS{iproj} =  [NAME_ROOT_FE,NAME_PROJ_LOC{iproj}]; ;
GENERALIZED_FORCES_ENDS_BEAM.RIGHT_END(iproj,2)= VALUE_FORCES;
