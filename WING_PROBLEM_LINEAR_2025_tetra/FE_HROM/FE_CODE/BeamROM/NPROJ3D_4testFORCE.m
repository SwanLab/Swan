...
NAME_PROJ_LOC = {'axial','sheary','shearz','torsion'} ;
NAMEPROJECTS = [] ;
NUMBER_OF_DOMAINS = [] ;
IMPOSED_DISP = [] ;
BoundaryConditions = [] ;
...
if ~isempty(PRESCRIBED_VALUE)
    ...
    PRESCRIBED_VALUE = DefaultField(PRESCRIBED_VALUE,'GENERALIZED_FORCE',[1,1,1,1,0,0]) ; 
    ...
    for iproj = 1:nprojects
      ...
      %  IMPOSED_DISP.RIGHT_END(iproj,:)= PRESCRIBED_VALUE.GENERALIZED_DISPLACEMENTS;
        %     VAL = 1e3 ;
             GENERALIZED_FORCES_ENDS_BEAM{2}(iproj,iproj) = PRESCRIBED_VALUE.GENERALIZED_FORCE(iproj)   ;
        %    % Generalized forces
