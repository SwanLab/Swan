function [K_axial,K_bending_z,K_bending_y,K_torsion,MSG] = PropStiffBeam3D(Kskel_11,Kskel_12,Kskel,L,MSG)
   ...
   MSG{end+1}='----------------------------', 
        ...
        EI = abs(FORCES(6))*L/2 ;  % Notice that this coincide with Kskel(6,6)
        K_bending_z = EI ;
        MSG{end+1}=['K_bending-z = EI_z =',num2str(K_bending_z,5)];
        MSG{end+1}='----------------------------';
        %MSG{end+1}='Bending stiffness -y (E*I_y)';
        DISP = zeros(nrows,1);
        DISP(5) = 1;
        DISP(11) = -1;
        FORCES = Kskel*DISP ;
        %FORCES(6) Should be equal to 4EI/L-2EI/L  = 2EI/L
        EI = abs(FORCES(5))*L/2 ;  % Notice that this coincide with Kskel(6,6)
        K_bending_y =EI ;
        MSG{end+1}=['K_bending-y = EI_y =',num2str(K_bending_y,5)];
        MSG{end+1}='----------------------------';
        %MSG{end+1}='Torsion stiffness   (G*J)';
        K_torsion = Kskel(4,4)*L ;
        MSG{end+1}=['K_torsion = G J =',num2str(K_torsion,5)];
        %%% What about shear stiffness. In principle
        % K(2,2) = 12*EI_z/L^3 + G*A_r/L ... But the first term is way too high
        % DISP = zeros(12,1);
        % DISP(2) = 1;
        % FORCES = Kskel*DISP ;
        % %FORCES(6) Should be equal to 4EI/L-2EI/L  = 2EI/L
