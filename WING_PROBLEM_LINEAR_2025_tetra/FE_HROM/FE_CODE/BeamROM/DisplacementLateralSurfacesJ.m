function [DISP3D,DATAIN ]= DisplacementLateralSurfacesJ(DATAIN,qDEF,DATA_REFMESH,DISP3D,SELECTED_DOMAINS,...
    qRB,DATAROM,MESH1D)
        ...
        for idom = 1:length(ELEMS)
            ...
            if SELECTED(idom) == 1
                ...
                if ISROTATED == 1
                    ...
                    finIND =  ndim*ISEL ;
                    iniIND = ndim*ISEL - ndim+1 ;
                    R = MESH1D.ROTATIONS(:,iniIND:finIND) ;
                    dispLOC =  reshape(DISPLOC,ndim,[]) ;
                    DISPLOC = R*dispLOC ;
                end
