function [NameFile_msh,NameFile_res,COORprint,CNprint,NAME_INPUT_DATA,...
    TypeElementPRINT,MaterialTypegloPRINT,NAMEMESH,DATAIN]=...
    Construct3DstructureROM(MESH1D,MESH3D,DATAIN,MESH_LATERAL,FOLDER,DATA_REFMESH)
...
% 1D mesh ------------------------------
% --------------------------------------
iacumNODES =  size(MESH1D.COOR,1) ; % Connectivities index (cumulative, for printing global 3D mesh)
...
MaterialTypegloPRINT = {MESH1D.MaterialType} ; % {ones(size(MESH1D.CN,1),1)} ; % Type of element global GID mesh
NAMEMESH =   {'1D'} ;
iMESH = 1;  % Number of meshes constructed so far
%imatACUM = 1 ;  % Number of GID materials defined so far
imatACUM = length(unique(MESH1D.MaterialType)) ;
    ...
    switch MESH1D.PROP(iENTITY).TYPE ;  % Type of entity
        case {'BEAM'}
            ...
            for iLOC = 1:length(itypeGLO)  % Loop over types of slices (mixed beams)
                ...
                [MESHcluster] =MeshCluster3D(MESH1D,MESH3D,iENTITY,itypeSLICE,DATAIN) ;  % Gathering COORDINATES and CONNECTIVITIES
                ...
                TypeElementPRINT{iMESH} =  MESH3D.SLICES(itypeSLICE).DATA3D.TypeElement ;
                MaterialTypegloPRINT{iMESH} = MESHcluster.MaterialType + imatACUM ;
                
                
                NAME_LOC_MSH = [MESH3D.SLICES(itypeSLICE).DATA3D.NAME_SLICE] ;
                ...
                if ~isempty(MESHcluster.CNlateral)
                    ...
                    if ~isempty(MESH3D.SLICES(itypeSLICE).DATA3D.TypeElementB_coarse )
                        TypeElementPRINT_LAT{iMESH_LAT} =  MESH3D.SLICES(itypeSLICE).DATA3D.TypeElementB_coarse ;
                    else
                        TypeElementPRINT_LAT{iMESH_LAT} =  MESH3D.SLICES(itypeSLICE).DATA3D.TypeElementB  ;
                    end
...
% PLOT FICTITIOUS INTERFACES
 [TypeElementPRINT,MaterialTypegloPRINT,CNprint,COORprint,NAMEMESH] = ...
...
NAME_INPUT_DATA = [FOLDER,filesep,DATAIN.NAME_LOAD_DATA] ;
