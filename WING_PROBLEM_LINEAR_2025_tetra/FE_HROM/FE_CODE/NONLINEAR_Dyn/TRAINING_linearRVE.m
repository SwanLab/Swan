function DATAFE_OUT = TRAINING_linearRVE(EXECUTABLE_FOLDER,FOLDER,DATA,NAMEPROJECTS,NAME_INPUT_DATA_MATERIAL,IMPOSED_FORCEbnd,...
    NUMBER_OF_DOMAINS,BoundaryConditions,IMPOSED_DISP,NameFileMeshLOC_coarse,NameFileMeshLOC,DATARUN)
...
%IMPOSED_FORCE = [] ;
 ...
 %=DATA_TYPESOLVER; % Conjugated gradient if = 1.
...
FUNinput.INPUTS.NEUMANN_BOUNDARY_CONDITIONS = 'UNIFORM_LOAD_PLATE_STRUCTURES' ; 
FUNinput.NAME ='INPUTS_GENERAL' ;  'INPUTS_GENERAL_NONLINEAR' ; % NAME OF THE FUNCTION THAT PREPARES THE INPUT DATA FOR THE SOLVER
...
for iprojects = 1:length(NAMEPROJECTS)
    ...
    if isempty(NAMEPROJECTS{iprojects})
        ...
        run(NAME_INPUT_DATA_MATERIAL) ;
         ...
         DATA = DefaultField(DATA,'TYPESOLVER',0) ; %= 0 ;  % Direct if = 0. Iterative if =1  
  DATA = DefaultField(DATA,'niterCONJG',[]) ; % = [] ;  % Direct 
...
%         
    ...
    %    DATA.niterCONJG  = DATA_niterCONJG; % Conjugated gradient if = 1.
       ...
       DATAFE_OUT =  FE_ELASTOSTATIC(FUNinput,DATA) ;
        
        AAAA = toc(AAAA) ;
        disp(['Total time FE training simulation = ',num2str(AAAA),' s'])
        DATAFE_OUT.timeSIMULATION = AAAA ; 
        cd(FOLDER) ;
