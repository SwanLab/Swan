function [stressDOM,reactDOM,Ndom,wDIAG_RHS,Bdom,Wdom,K,CgloDOM] = ...
    ReactionsDOMAIN(Nst,fNOD,ListGauss,ndim,DOFs,Fpnt,CNb,NODESrve,Tnod,...
    TypeElementB,CONNECTb,...
    Bst,dLOC,wSTs_RHS,wSTs,Cglo,COOR,nnodeDOM,faceDOFS,BasisUrb,nstrain,idom,iproject,...
    DATAIN,stressGLO,DATA_INPUT_FE,timestepINCLUDE,ListGauss_RHS)
...
if (~isempty(fNOD) && any(fNOD)) || (iproject ==1 && idom ==1)
    ...
    ListGaussDOF_RHS =  small2large(ListGauss_RHS,ndim) ;
    %   if iproject ==1 && idom ==1
    if ~isempty(Nst)
        Ndom = Nst(ListGaussDOF_RHS,DOFs) ;
    else
...
DATA_INPUT_FE = DefaultField(DATA_INPUT_FE,'STEPS_TO_STORE',1) ;
