function  [P,b]=  AssemblyPsimple(BasisRdef,NODESbound,reactDOMrbGLO,betaBC,COL,COLloc,...
...
Cov_f1f1 = BasisR(f1,:)'*BasisU(f1,:) ; 
Cov_f1f2 = BasisR(f1,:)'*BasisU(f2,:) ; 
Cov_f2f2 = BasisR(f2,:)'*BasisU(f2,:) ; 
...
for iface = 1:nDOM+1
    if iface == 1
        ...
        VECT=  [ ALPHA(1)*Cov_f1f1  ] ;
        indROWS = 1:size(VECT,1) ;
        indCOLS = 1:size(VECT,2) ;
        P(indROWS,indCOLS) = VECT ;
        b(indROWS) = ALPHA(1)*BasisR(f1,:)'*(uBAR{1,1}) ; 
        iROWS = length(indROWS) ; iCOLS = 0 ;
    elseif iface == (nDOM+1)
        VECT=  [ALPHA(2)*Cov_f2f2  ] ;
        indROWS = iROWS + (1:size(VECT,1)) ;
        indCOLS = iCOLS + (1:size(VECT,2))  ;
        P(indROWS,indCOLS) =  VECT ;
         b(indROWS) = ALPHA(2)*BasisR(f2,:)'*(uBAR{nDOM,3}) ; 
    else
