function [BASES, DATA_REFMESH,nBASES_BEAM,DATAsnap,MSG]= BasisU_R_def_computation(DATA_TRAINING,DATAIN,MSG)
...
% nstepsNODES = zeros(1,length(DATA_TRAINING))' ;  % For nonlinear analysis
% nstepsGAUSS = zeros(1,length(DATA_TRAINING))' ;  % For nonlinear analysis


% Check whether some projects are repeated
% Why is this necessary ?  To avoid reading the same project twice
% Disabled on  28-May-2019
...
for iproject = 1:length(DATA_TRAINING)
    ...
    %  if isempty(dDOM{iproject})  % Disabled 28-May-2019
    [dDOM{iproject},reactDOM{iproject},stressDOM{iproject},DATAREF,REMOVE_FIRST_DOMAIN,timestepINCLUDE ]= ...
        ExtractDisplReacMatrix_project(DATA_TRAINING{iproject},DATAIN,iproject) ;
    %  end
    ...
    %     %%% Boundary interface displacements  % DISABLED, 28-MAY-2019
    ...
    % DomdDOMains to be included in the SVD --- Disabled on 28-May-2019
    ...
    % Now we remove the first columns, because they belong to the first
    % domain,
...
if DATAIN.COMBINED_METHOD == 1
    error('Option no longer available (Aug-2019)')
...
if all(DATAIN.IS_BEAM_MODE)
    ...
    DATAIN.BeamProjectsLOC = DATAIN.BeamProjects ;
    ...
    %  nmodesR = min(nmodesR,nmodesU) ; % We remove this line( 8-Dec-2018)
