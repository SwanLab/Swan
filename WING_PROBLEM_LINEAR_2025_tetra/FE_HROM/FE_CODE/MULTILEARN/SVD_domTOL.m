function  [U,S,V,h,h2] = SVD_domTOL(SNAP,nfigure,LEGENDG,NMODES_SHOW,COLOR,DATA,...
    RELATIVE_TOLERANCE)
...
DATA = DefaultField(DATA,'METHOD_SVD_ORTHOGONAL_COMPLEMENT',1) ;  %

if DATA.METHOD_SVD_ORTHOGONAL_COMPLEMENT == 1
    % Method based on the orthogonal complement of each submatrix
    epsilon = zeros( size(RELATIVE_TOLERANCE)) ;
    for iproj = 1:length(epsilon)
        if ~isempty(RELATIVE_TOLERANCE{iproj})
            epsilon(iproj) = RELATIVE_TOLERANCE{iproj} ;
        end
    end
    DATALOC  = [] ;
    ...
    if length(SNAP) == 1
        ...
        DATALOC.EPSILON_GLO = epsilon(1) ; 
    end
    
    [U,S,V,ETIME,eSVD,RankMatrix,DATAOUT] = RSVDqp(SNAP,epsilon,DATALOC) ;
else
    
    DATA = DefaultField(DATA,'SVD_RANDOM',1) ;
    DATA = DefaultField(DATA,'TOL_LOC',0) ;
    DATA = DefaultField(DATA,'TOLERANCE_SVD_PROJECT',[] ) ;
    DATA = DefaultField(DATA,'TOLERANCE_SVD_SLICE',RELATIVE_TOLERANCE ) ;
    
    
    DATALOC.TOL_BLOCK_SVD =RELATIVE_TOLERANCE ;
    DATALOC.RELATIVE_SVD = 1;
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    DATA = DefaultField(DATA,'TOL_SVD_GLO',0) ;
    DATALOC.TOL_SVD_GLO = DATA.TOL_SVD_GLO ;
    NMODES = cell(size(RELATIVE_TOLERANCE)) ; 
    % Standard partitioned method
