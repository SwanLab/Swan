function [Gb,dR,DOFr,DOFm] = PLATES_BORDERS_FUN(PRESCRIBED_VALUES,DOMAINVAR,COOR,CONNECTb,TypeElementB,...
    DATA)
...
[x,y,z,COOR_FACE,nodesf,IDOMX,IDOMY] =  NodesFacesCorners(DOMAINVAR,COOR)  ;

for iface = 1:length(nodesf)
    if ~isempty(PRESCRIBED_VALUES{iface})
        nodesfA = nodesf{iface} ;
        DOFA = small2large(nodesfA,ndim) ;   % DOFS face 1
        COOR_FACE = COOR(nodesfA,:) ; %
        % Local connectivities
        % ----------------------
        LocalConnect =  (CONNECTb(IDOMX{iface},IDOMY{iface},iface)) ;
        ...
        %%%
        dR = [dR ; R*PRESCRIBED_VALUES{iface}] ;
        DOFr = [DOFr; DOFA] ;
    end
end

[DOFr III] = unique(DOFr) ; 
dR = dR(III) ; 
...
% % ---------------------------------------------------------------------------
% 
% USE_QUADRATIC_BENDING_MODES = 0 ;  % Rather than linear modes 
% 
...
% %%%% FREEING SIDE DOFS 
% % ---------------------
%  BNODES_all = [cell2mat(NODES_CORNERS');cell2mat(NODES_SIDES')] ;   
% switch  NAMEPROJECT
%     case {'StrainXX','StrainXY','StrainXZ','TorsionX','BendingXZ','BendingXY'}
%            
%             % DOFS of SIDES PARALLEL TO PLANE Y = 0  are set free
%             isides = [1,3] ; 
%             BNODES_selected = [cell2mat(NODES_CORNERS');cell2mat(NODES_SIDES(isides)')] ;   
%             [~, INDICES_SELECTED ]= intersect(BNODES_all,BNODES_selected) ; 
%             %  Hence 
%              DOFsSELECTED = small2large(INDICES_SELECTED,ndim) ; 
%             dR = dR(DOFsSELECTED) ; 
%             DOFr = DOFr(DOFsSELECTED) ; 
%             
%     case {'StrainY','StrainYZ','BendingYZ','TorsionY','BendingYX'}
%          isides = [2,4] ; 
%             BNODES_selected = [cell2mat(NODES_CORNERS');cell2mat(NODES_SIDES(isides)')] ;   
%             [~, INDICES_SELECTED ]= intersect(BNODES_all,BNODES_selected) ; 
%             %  Hence 
...
% 
% 
function [x,y,z,COOR_FACE,nodesf,IDOMX,IDOMY] =  NodesFacesCorners(DOMAINVAR,COOR)
