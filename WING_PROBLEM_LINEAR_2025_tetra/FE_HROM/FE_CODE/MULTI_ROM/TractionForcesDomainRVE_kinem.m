function [FtractionE,Rast,rRB,fextDOMred,FORCES_2_PRINT_local ]= ...
    TractionForcesDomainRVE_kinem(UPSILONforceTRAC,LOADS,...
...
%         end
        ...
        if  any(ndimGLO(1)-ndimGLO)
           % error('Implement this part of the code....')
            F_face_loc = zeros(nnodeE*ndim,1) ;
            COLUMNS_ini = 1; 
            iniROWS =1 ; 
            for inode = 1:nnodeE
                
                COLUMNS_fin = COLUMNS_ini +  ndimGLO(inode)-1 ;
                COLUMNS = COLUMNS_ini:COLUMNS_fin ;
                ifinROWS = iniROWS + ndim-1 ; 
                ROWS = iniROWS:ifinROWS ; 
                ROWS = ROWS(1:ndimGLO(inode)) ; 
                 F_face_loc(ROWS) = F_face(COLUMNS) ;
               COLUMNS_ini = COLUMNS_fin + 1 ; 
               iniROWS = ifinROWS + 1; 
            end
            FtractionE = FtractionE +  F_face_loc ;
