function [BasisINT,RotationMatrixLOC,TEXTP,Vrb,Vdef,Vdef_complementary ]...
    =  DeformModesInterface_AlignmentMethod(BasisRdef,f1,f2,...
    Vrb,M,DATAIN,BasisUdef,SinvVal_Udef,SinvVal_Rdef,iface,TEXTP,Vdef_Morth)
if nargin == 0
    load('tmp.mat')
  %  DATAIN.TOL_DeformationalInterfaceModes_ANGLES_INTERSECTION_RU  = 5 ; 
end
...
if DATAIN.INCLUDE_SINGULAR_VALUES_COMPUTATION_OF_INTERSECTIONS ==1
    ...
    % 28-May-2019
    %  BasisUdef1 =
    %  bsxfun(@times,BasisUdef1',SinvVal_Udef(1:size(BasisUdef1,2)))' ;  28
...
% !!!
%DATAIN = DefaultField(DATAIN,'AlignmentMethodComputeIntersectionDisp',1) ; % 28-May-2019 

DATAIN = DefaultField(DATAIN,'AlignmentMethodComputeIntersectionDisp',0) ;
%% *** 8-Jan-2020. See Discussion in /home/joaquin/Desktop/CURRENT_TASKS/POTENTIAL_RESEARCH_TOPICS/COMBINING_MULTISCALE_REDUCTIONMODELS/REPORT_MULTIS_REDUC_MODEL/PAPER_multiLEARN
% ExplanationLocalEffects.tex
if DATAIN.AlignmentMethodComputeIntersectionDisp == 1
    error('This option was  disabled in Jan 2020 ---it appears to be non-robust. Besides it is not in the paper')
    ...
    TEXTP{end+1} = ['Number of deformational modes (no intersection, just WSVD) = ', num2str(size(Vdef,2)) ];
