...
% ---------------------------------
%dbstop('5')
LOADglo = INPUTS_LOC.PLATELOADS.LOAD_UNIFORM ;  % LOAD{iface}(idom) = [t_x,t_y,t_z], iface = 3, 4...  LAteral surfaces
%
ISLOCAL_DEF = cell(size(LOADglo)) ; % If is empty, or zero ---> Global coordinates
INPUTS_LOC.PLATELOADS = DefaultField(INPUTS_LOC.PLATELOADS,'ISLOCAL',ISLOCAL_DEF) ;
ISLOCAL = INPUTS_LOC.PLATELOADS.ISLOCAL ;

CNb = cell(1,ndim) ; Tnod= cell(1,ndim) ;
nfaces = min(size(CONNECTb,3),length(LOADglo)) ;
for iface = 1:nfaces
    % Loop over faces
    ...
    nDOMconnect = size(CONNECTb,1)*size(CONNECTb,2) ;
    nslices = min(nDOMconnect,size(LOADface,1) );
    ...
    for idom = 1:nslices
        ...
        [IDOMX,IDOMY ]= ind2sub([size(CONNECTb,1),size(CONNECTb,2)],idom);
        CNbLOC = CONNECTb{IDOMX,IDOMY,iface} ;
...
if  isempty(INLOC.GENERALIZED_FORCES_ENDS_BEAM)
    ...
    Fpnt =  PointLoadsBeams(DOMAINVAR,COOR,CONNECTb,TypeElementB,FORCES)  ;
end
