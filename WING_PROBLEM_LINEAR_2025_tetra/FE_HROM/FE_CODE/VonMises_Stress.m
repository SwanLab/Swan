function [ sigmaVONMISES ] =  VonMises_Stress(stressEL) ; 
...
% stressEL = Stress_Elements_Average(DATAOUT.wSTs,DATAOUT.stress,ngausE,nstress,nelem) ; 
 ...
 STRESSm = zeros(3,3,size(stressEL,2)) ; 
% IND = cell(6,1) ; 
% for i=1:nstress
...
STRESSm(2,1,:) =  stressEL(6,:) ;  
 elseif size(stressEL,1) ==4
     % Plane strain 
  STRESSm(1,1,:) =  stressEL(1,:) ; 
STRESSm(2,2,:) =  stressEL(2,:) ; 
STRESSm(3,3,:) =  stressEL(4,:) ; 
STRESSm(1,2,:) =  stressEL(3,:) ; 
STRESSm(2,1,:) =  stressEL(3,:) ;     
     
 else
     error('Plane stress or plane strain with 3 entries have not been programmed yet')
 end
