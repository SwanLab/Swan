function BasisRrb = ConstructWarpingMode_double(COORrel)
...
if ndim == 2
   error('Option not valid')
    ...
    BasisRrb =  zeros(prod(size(COORrel)),2) ;
%     BasisRrb(1:3:end) = COORrel(:,3).*COORrel(:,2) ;
%     BasisRrb(2:3:end) = 0;
%     BasisRrb(3:3:end) = 0;
    
    BasisRrb(1:3:end,1) = -COORrel(:,3).*COORrel(:,2);
    BasisRrb(3:3:end,1) = +COORrel(:,1).*COORrel(:,2);
    
    BasisRrb(2:3:end,2) = +COORrel(:,3).*COORrel(:,2);;
    BasisRrb(3:3:end,2) =  -COORrel(:,2).*COORrel(:,2);
%     % Rotation modes
