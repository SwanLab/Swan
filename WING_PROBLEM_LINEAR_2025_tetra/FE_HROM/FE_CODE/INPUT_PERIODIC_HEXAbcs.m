function [COOR,CN,TypeElement,TypeElementB, celasglo,  DOFr,dR,...
    ...
    DATA,MaterialType] = INPUT_PERIODIC_HEXAbcs(INPUTS_LOC,DATA) ;
...
DATA_namew = ['DATAWS/',DATA.NAMEDATA,'_WS.mat'] ; % To retrieve already read data

DATA = DefaultField(DATA,'nameWORKSPACE',DATA_namew)
...
[celasglo celasgloINV typePROBLEM]  = AssignMatProp_ElastIso(ndim,MATERIAL,nelem,MaterialType,DATA) ;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if ndim == 3
    [DOFr DOFm Gb dR dispMACRO DATA] = Periodic_BoundaryCONDhexaMINbcs(COOR,CN,MACRODEF,CONNECTb,DATA) ;
else
    [DOFr DOFm Gb dR dispMACRO DATA] = Periodic_BoundaryCONDhexaMINbcs_2D(COOR,CN,MACRODEF,CONNECTb,DATA) ;
end
...
DATA = DefaultField(DATA,'NOVOIDS',1) ;
