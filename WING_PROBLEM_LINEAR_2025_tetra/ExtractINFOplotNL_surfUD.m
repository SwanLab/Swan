function [max_disp,STRAINenergy,max_disp_hist,STRAINenergy_hist,DATA_STEPS] = ExtractINFOplotNL_surfUD(BOUNDARY_CONDIDITONS,DATALOC)
if nargin == 0
    load('tmp3.mat')
end
% PLOT REACTIONS
...
max_disp_hist = abs(SNAPdisp(DOF,:)) ;

 disp(['MAX. DISPL.   =    ',num2str(max_disp),' m'])
...
xlabel('Time (s)')
ylabel('Max disp.  (m)')
h = plot(DATA.STEPS,max_disp_hist) ; 
