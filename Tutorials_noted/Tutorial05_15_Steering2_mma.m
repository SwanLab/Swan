classdef Tutorial05_15_Steering2_mma < handle
    ...
    methods (Access = public)
        ...
        function obj = Tutorial05_15_Steering2_mma()
                
                obj.Kp_bar_vector = [0.001, 0.0008, 0.0005];
                obj.width = 1;
                obj.height = 1;
    ...
    methods (Access = private)
        ...
        function [isInput, isOutputL, isOutputR, isWallL, isWallR] = nonDesignRegionsCoords(obj)
            ...
            isWallL = coords(:,1) < 1e-8 & coords(:,2) < 0.7*obj.height;
            isWallR = coords(:,1) > 1 - 1e-8 & coords(:,2) < 0.7*obj.height;
        ...
        function [isInput, isOutputL, isOutputR, isWallL, isWallR] = nonDesignRegionsAnonymous(obj)
            ...
            isWallL =@(coor) coor(:,1) < 1e-8 & coor(:,2) < 0.7*obj.height;
            isWallR =@(coor) coor(:,1) > 1 - 1e-8 & coor(:,2) < 0.7*obj.height;
        end
        ...
        function printFinalDisplacement_v3(obj)
            ...
            namePrint = sprintf('SteeringV_FinalDispl_J_%g',num_case);
        ...
        function saveFigures(obj)
            ...
            name_design = sprintf('SteeringV_DesignMap_Case_%g.png', num_case );
            name_monitor = sprintf('SteeringV_Monitoring_Case_%g.png', num_case);
        ...
        function printFinalDesignVariable(obj)
            num_case = find(obj.Kp_bar_vector == obj.Kp_bar);
            namePrint = sprintf('SteeringV_DesignVariable_kCase_%g',num_case);
