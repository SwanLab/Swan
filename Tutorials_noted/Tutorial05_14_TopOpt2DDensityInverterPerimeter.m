classdef Tutorial05_14_TopOpt2DDensityInverterPerimeter < handle
    ...
    methods (Access = private)
        ...
        function bc = createBoundaryConditionsAdjoint(obj) % must be modified
            ...
            sPL{1}.domain    = @(coor) isPLTop(coor);
            sPL{1}.direction = 2;
            sPL{1}.value     = +obj.k_case; % dummy load at the output

            sPL{2}.domain    = @(coor) isPLBottom(coor);
            sPL{2}.direction = 2;
            sPL{2}.value     = +1; % upward force on the input
