classdef Tutorial05_15_Steering2_mma_2MP < handle
    ...
    methods (Access = public)
        ...
        function obj = Tutorial05_15_Steering2_mma_2MP()
                
                obj.Kp_bar_vector = [0.01, 0.005];
                obj.width = 1;
                obj.height = 1;
    ...
    methods (Access = private)
        ...
        function [isInput, isOutputL, isOutputR, isWallL, isWallR] = nonDesignRegionsCoords(obj)
            ...
            isOutputL = coords(:,1) < 1e-8 & coords(:,2) > 0.8*obj.height;
            isOutputR = coords(:,1) > 1 - 1e-8 & coords(:,2) > 0.8*obj.height;
        ...
        function [isInput, isOutputL, isOutputR, isWallL, isWallR] = nonDesignRegionsAnonymous(obj)
            ...
            isOutputL =@(coor) coor(:,1) < 1e-8 & coor(:,2) > 0.8*obj.height;
            isOutputR =@(coor) coor(:,1) > 1 - 1e-8 & coor(:,2) > 0.8*obj.height;
            isWallL =@(coor) coor(:,1) < 1e-8 & coor(:,2) < 0.5*obj.height;
            isWallR =@(coor) coor(:,1) > 1 - 1e-8 & coor(:,2) < 0.5*obj.height;
        end

        function createDesignVariable(obj)
            s.fHandle = @(x) 0.4*ones(size(x(1,:,:)));
            s.ndimf   = 1;
            s.mesh    = obj.mesh;
        ...
        function bc = createBoundaryConditionsLoadBased(obj, gdi_index) 
            ...
            if gdi_index == 1 % Horizontal MP
                sPL{1}.domain = isInput;
                sPL{1}.direction = [1];
                sPL{1}.value = 1/nNodesInput;
            elseif gdi_index == 2 % Vertical MP
                sPL{1}.domain = isInput;
                sPL{1}.direction = [2];
                sPL{1}.value = 1/nNodesInput;
            elseif gdi_index == 3 % Output
                ...
                sPL{1}.direction = [2];
                sPL{1}.value = -0.001/nNodesOutputR;
            elseif gdi_index == 4
                sPL{1}.domain = isOutputL;
                sPL{1}.direction = [2];
                sPL{1}.value = -0.001/nNodesOutputL;
            else
        ...
        function printFinalDisplacement_v3(obj)
            ...
            % namePrintH = sprintf('SteeringH_FinalDispl_J_%g',num_case);
            % uFun = obj.physicalProblemMotionBased{1}.uFun;
            % uFun.print(namePrintH);

            namePrintV = sprintf('SteeringV_FinalDispl_J_%g',num_case);
            uFun = obj.physicalProblemMotionBased{2}.uFun;
            uFun.print(namePrintV);
        ...
        function saveFigures(obj)
            ...
            name_design = sprintf('SteeringH_DesignMap_Case_%g.png', num_case );
            name_monitor = sprintf('SteeringH_Monitoring_Case_%g.png', num_case);
        ...
        function printFinalDesignVariable(obj)
            num_case = find(obj.Kp_bar_vector == obj.Kp_bar);
            namePrint = sprintf('SteeringH_DesignVariable_kCase_%g',num_case);
