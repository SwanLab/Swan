classdef LHSintegrator_Curl_N < LHSintegrator
    ...
    methods (Access = protected)
        ...
        function lhs = computeElementalLHS(obj)
            curlShapes = obj.fun.computeCurlShapeFunctions(obj.quadrature);
            
            dVolu  = obj.mesh.computeDvolume(obj.quadrature);
            nGaus  = obj.quadrature.ngaus;
            nElem  = size(dVolu,2);
            nDimf  = obj.fun.ndimf;
            nNodE  = size(curlShapes,1);
            nDofE  = nNodE*nDimf;
                    ...
                    for jnode= 1:nNodE
                        for iunkn= 1:nDimf
                            for junkn= 1:nDimf
                                ...
                                dvol = dVolu(:,igauss);
                                Jd = Jdet(:,igauss);
                                Ni = pagemtimes(squeeze(curlShapes(inode,igauss,:))',JGlob);
                                Nj = pagemtimes(squeeze(curlShapes(jnode,igauss,:))',JGlob);
