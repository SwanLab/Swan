classdef OptimizationEIFEMTutorial2param < handle
    ...
    methods (Access = public)
        ...
        function obj = OptimizationEIFEMTutorial2param()
            obj.init()
            obj.createMesh();
...
%             obj.r= (1e-6 - 1e-6) * rand(obj.nSubdomains(2),obj.nSubdomains(1)) + 1e-6;
    ...
    % 
    % % Top Right Corner
    % mask = abs(coord0(:,1) - obj.xmax) < geo_tol & abs(coord0(:,2) - obj.ymax) < geo_tol;
    % coord0(mask, :) = coord0(mask, :) + [-delta, -delta];
    % 
    % % Bottom Right Corner
    % mask = abs(coord0(:,1) - obj.xmax) < geo_tol & abs(coord0(:,2) - obj.ymin) < geo_tol;
    % coord0(mask, :) = coord0(mask, :) + [-delta, +delta];
    % 
    % % Top Left Corner
    % mask = abs(coord0(:,1) - obj.xmin) < geo_tol & abs(coord0(:,2) - obj.ymax) < geo_tol;
    % coord0(mask, :) = coord0(mask, :) + [+delta, -delta];
    % 
    % % Bottom Left Corner
    % mask = abs(coord0(:,1) - obj.xmin) < geo_tol & abs(coord0(:,2) - obj.ymin) < geo_tol;
    % coord0(mask, :) = coord0(mask, :) + [+delta, +delta];
                    % coord0 = refMesh.coord;
