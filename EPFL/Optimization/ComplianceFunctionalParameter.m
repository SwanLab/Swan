classdef ComplianceFunctionalParameter < handle
    ...
    methods (Access = public)
        function obj = ComplianceFunctionalParameter(cParams)
            obj.init(cParams);
...
%              J = u(:)'*obj.stateProblem.FextDisc(:);
        ...
        function dJ = computeGradient(obj,dK,u,mu)
            ...
            [nDof, nElem] = size(uL);
            nparam = mu.fun.ndimf;
            uPages = reshape(uL, nDof, 1, nElem);
            % dj = zeros(mu.fun.mesh.nelem,nparam);
            for i= 1:nparam
                Ku = pagemtimes(squeeze(dK(:,:,i,:)), uPages);
                dj = -reshape(sum(uPages .* Ku, 1), nElem, 1);
                s.fValues = dj;
                dJ{i} = LagrangianFunction(s);
            end
            % dj = obj.computeGrad(dK,u,mu);
             % uL = obj.stateProblem.EIFEMsolver.global2local(u);
% %             nelem = size(uL,2);
%             [nDof, nElem] = size(uL);
%             uPages = reshape(uL, nDof, 1, nElem);
% 
%             Ku = pagemtimes(dK, uPages);
%             dj = -reshape(sum(uPages .* Ku, 1), nElem, 1);
% %             for ielem = 1:nelem
...
% %             end
        ...
        function dj = computeGrad(obj,dK,u,mu)
            uL = obj.stateProblem.EIFEMsolver.global2local(u);
            [nDof, nElem] = size(uL);
            nparam = mu.fun.ndimf;
            uPages = reshape(uL, nDof, 1, nElem);
            ...
            for i= 1:nparam
                Ku = pagemtimes(squeeze(dK(:,:,i,:)), uPages);
                dj(:,i) = -reshape(sum(uPages .* Ku, 1), nElem, 1);
            end
            %             for ielem = 1:nelem
            %                 dj(ielem,1) = -uL(:,ielem)'*dK(:,:,ielem)*uL(:,ielem);
            %             end
