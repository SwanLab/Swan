classdef OptimizationEIFEMTutorial_3D < handle
    ...
    methods (Access = public)
        ...
        function obj = OptimizationEIFEMTutorial_3D()
            obj.init()
            obj.createMesh();
...
%             obj.createCompliance();
    ...
    methods (Access = private)
        ...
        function init(obj)
            close all;
            obj.nSubdomains = [3,3,3]; %50 15
            muMax = 0.385;
            % obj.r = muMax*ones(obj.nSubdomains)'; 
%             obj.r= (1e-6 - 1e-6) * rand(obj.nSubdomains(2),obj.nSubdomains(1)) + 1e-6;
            obj.xmax=1; obj.xmin=-1; obj.ymax = 1; obj.ymin=-1; obj.zmax = 1; obj.zmin=-1; 
             % obj.xmax=2; obj.xmin=0; obj.ymax = 2; obj.ymin=0; 
            obj.length = 2;
            obj.tFrame = 0.385;
            ...
            % obj.fileNameEIFEM = './EPFL/parametrizedEIFEMLagrange_2paramsFrame_levelSet.mat';
        ...
        function [mD,mSb,iC,lG,iCR,discMesh] = createMeshDomain(obj,mR)
            ...
            s.geometryType = mR.geometryType;
        ...
        function createMesh(obj)
            ...
            obj.mR = obj.createReferenceMesh();
            [mD,mSb,iC,lG,iCR,discMesh] = obj.createMeshDomain(obj.mR);
            obj.mesh = mD;
         ...
         function  mSbd = createSubDomainMeshes(obj)
            ...
            for jDom = 1:nY
                for iDom = 1:nX
                    ...
                    % refMesh =  TriangleMesh(2,2,50,50);
        ...
        function cMesh = createReferenceCoarseMesh(obj)
            xmax = max(obj.mR.coord(:,1));
            xmin = min(obj.mR.coord(:,1));
            ymax = max(obj.mR.coord(:,2));
            ymin = min(obj.mR.coord(:,2));
            zmax = max(obj.mR.coord(:,3));
            zmin = min(obj.mR.coord(:,3));
...
%             uMesh.compute(levelSet);
        ...
        function createVolumeConstraint(obj)
            ...
            s.volume  = @(mu) sum(8*mu.fValues(:,1) - 4*mu.fValues(:,1).^2 ...
             ...
             s.rhoCell  = @(mu) (8*mu.fValues(:,1) - 4*mu.fValues(:,1).^2 ...
                               +4*sqrt(2)*(1-mu.fValues(:,1)).*mu.fValues(:,2)-3/2*mu.fValues(:,2).^2)/4;
            % s.gradJ   =  @(mu) [4*sqrt(2)*(1-mu.fValues(:,1))-3*mu.fValues(:,2), ...
                                % 8-8*mu.fValues(:,1)-4*sqrt(2)*mu.fValues(:,2) ];
            s.gradJ = @(mu) [8 - 8*mu.fValues(:,1) - 4*sqrt(2)*mu.fValues(:,2), ...
                 4*sqrt(2)*(1 - mu.fValues(:,1)) - 3*mu.fValues(:,2)];
        ...
        function M = createMassMatrix(obj)
            test   = LagrangianFunction.create(obj.coarseMesh, 2, 'P0');
            trial  = LagrangianFunction.create(obj.coarseMesh, 2, 'P0');
            f = @(u,v) DP(v,u);
...
%             obj.optimizer = opt;
        ...
        function bc = createBoundaryConditions(obj)
            ...
            %% Arch
              ...
              isDir = @(coor) ( ...
                             (abs(coor(:,1)-xMin) < 2e-9 & abs(coor(:,2)-yMin) < 2e-9) | ...
                             (abs(coor(:,1)-xMax) < 2e-9 & abs(coor(:,2)-yMin) < 2e-9) );
               isForce = @(x)  abs(x(2,:,:)-yMax) < 2e-9;

            %% MBB beam
            % % isDir   = @(coor)  abs(coor(:,1)-xMin) < 1e-12 | abs(coor(:,1)-xMax) < 1e-12;
            % % isForce = @(x)  abs(x(2,:,:)-yMax) < 1e-12 & x(1,:,:)>=xMin+0.45*Lx & x(1,:,:)<=xMin+0.55*Lx;
             % isDir  = @(coor) ( (abs(coor(:,1)-xMin) < 1e-9  & abs(coor(:,2)-yMin) < 1e-9 ));
             % isDir2 = @(coor) ( (abs(coor(:,1)-xMax) < 1e-9  & abs(coor(:,2)-yMin) < 1e-9));
             % isForce = @(x)  abs(x(2,:,:)-yMax) < 1e-9 & (x(1,:,:)>=xMin+0.495*Lx & x(1,:,:)<=xMin+0.505*Lx);
%%
             % % % isForce = @(x)  abs(x(2,:,:)-yMax) < 1e-9 & abs(x(1,:,:)-xMin) < 0.01*Lx;
             % % 
