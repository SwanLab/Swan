function [K,f] = system_GLS_p2(tau,tau_c,a,nu,xnode,problem)
% [K,f] = system_GLS_p2(a,nu,xnode)
% System obtained by discretizing the weak form associated to
% the convection-diffusion equation
%               a ux - nu uxx = f
% with the stabilized GLS method using parabolic interpolation.
...
for i=1:numel
    ...
    weigth = wpg*h;
    isp = [2*i-1 2*i 2*i+1]; % Global number of the nodes in the current element
    ...
    for ig=1:ngaus
        ...
        Nxx= Nxxi_mef(ig,:)/(h^2);
        w_ig = weigth(ig);
        x = xnode(2*i) + h*xipg(ig); % x-coordinate of the Gauss point
        % Assembly
