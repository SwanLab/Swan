classdef tempDataGenerationPoissonFetiDP < handle    
    ...
    methods (Access = public)
        ...
        function obj = tempDataGenerationPoissonFetiDP()
            close all;
            
            % 1. Initialization Parameters
            obj.numSubdomains = [6 6]; 
            obj.nodeTol       = 1e-10;
            ...
            obj.fetiSolver = tempFetiDPPoisson(obj.globalMesh, obj.localMeshes, ...
    ...
    methods (Access = private)
        ...
        function [uMono, uFeti] = runConvergenceComparison(obj)
            ...
            % lambdaMaxK = eigs(kFree, 1, 'largestabs');
            % lambdaMinK = eigs(kFree, 1, 'smallestabs');
            kappaK = 1; %lambdaMaxK / lambdaMinK;
            ...
            % E_F = real(eig(fMat));
            kappaF =1;% max(E_F) / min(E_F);
            nDofsFeti    = length(dBar);

            % Dummy variables to prevent plotting/table errors
            residual1 = []; timeMono     = 0;
            residual2 = []; timeFetiDual = 0;
