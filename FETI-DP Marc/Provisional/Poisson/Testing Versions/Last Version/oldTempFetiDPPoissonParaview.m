classdef tempFetiDPPoissonParaview < handle
    % FETI-DP algebraic solver for the scalar Poisson problem on a domain decomposition.
    % Generalized format handling DOFs dynamically without boolean assembly matrices (Ap).
    ...
    methods (Access = public)
        ...
        function obj = tempFetiDPPoissonParaview(globalMesh, subMeshes, stiffness, forces, tol, dofsNode, boundaryConditions)
        ...
        function extractFetiDofs(obj)
            ...
            for i = 1:nSub
                ...
                for b = 1:length(bMeshes)
                    allN = bMeshes{b}.globalConnec(:);
                    deg = accumarray(allN, 1, [max(allN), 1]);
                    idxPrimal = [idxPrimal; find(deg == 1)];
                end
