classdef oldTempFetiDPElasticity < handle
    ...
    methods (Access = public)
        ...
        function obj = oldTempFetiDPElasticity(globalMesh, subMeshes, stiffness, forces, tol, dofsNode, boundaryConditions)
    ...
    methods (Access = private)
        ...
        % function dirDofs = getDirichletDofs(obj)
        %     dirDofs = obj.boundaryConditions.dirichlet_dofs;
        % end
        
        % function isDirNode = getDirichletNodeMask(obj)
        %     isDirNode = false(size(obj.meshCoords, 1), 1);
        %     dirDofs   = obj.getDirichletDofs();
        %     if isempty(dirDofs)
        %         return;
        %     end
        %     dirNodes = unique(ceil(dirDofs / obj.dofsPerNode));
        ...
        %     for i = 1:nSub
        %         [~, gNodes] = ismembertol(obj.localMeshes{i}.coord, obj.meshCoords, tol, 'ByRows', true);
        %         nodeMultiplicity(gNodes) = nodeMultiplicity(gNodes) + 1;
        %     end
        % 
        %     isDirNode = obj.getDirichletNodeMask();
        % 
        %     for i = 1:nSub
        ...
        % 
        %         isShared       = nodeMultiplicity(globalNodes) > 1;
        %         localDualNodes = setdiff(find(isShared), localPrimalNodes);
        %         localRemNodes  = setdiff(1:size(coords, 1), [localPrimalNodes; localDualNodes]);
        % 
        %         pGlobal{i} = obj.nodesToDofs(globalNodes(localPrimalNodes));
        %         dGlobal{i} = obj.nodesToDofs(globalNodes(localDualNodes));
        ...
        % 
        %         minCoords = min(coords); maxCoords = max(coords);
        %         isPrimal = (abs(coords(:,1) - minCoords(1)) < tol | abs(coords(:,1) - maxCoords(1)) < tol) & ...
        %                    (abs(coords(:,2) - minCoords(2)) < tol | abs(coords(:,2) - maxCoords(2)) < tol);
        % 
        %         isShared = nodeMultiplicity(gNodes) > 1;
        %         isDual   = isShared & ~isPrimal;
        %         isRem    = ~isShared & ~isPrimal;
        % 
        %         idxPrimal = find(isPrimal);
        %         idxDual = find(isDual);
        %         idxRem = find(isRem);
        % 
