classdef DataGenerationElasticity2DMonoPrecond < handle
    ...
    properties (Access = public)
        ...
        monolithicPreconditioner = 'amg'  % Options: 'direct', 'ilu', 'amg'
    ...
    methods (Access = public)
        ...
        % CONSTRUCTOR / MAIN
        % =================================================================
        function obj = DataGenerationElasticity2DMonoPrecond(selectedPrecond, runSolver)
            if nargin > 0
                obj.monolithicPreconditioner = selectedPrecond;
            end
            ...
            % Display selected preconditioner for monolithic system
            fprintf('\n========================================\n');
            fprintf('MONOLITHIC SOLVER PRECONDITIONER: %s\n', upper(obj.monolithicPreconditioner));
            fprintf('FETI-DP: Dirichlet preconditioner (standard)\n');
            fprintf('========================================\n');
            ...
            if runSolver
                ...
                if obj.enablePlots
                    ...
                    obj.visualizeDeformedMesh(results.uFeti, 1, 'FETI-DP (Dirichlet preconditioner)');
                end
                ...
                if obj.exportParaview
                    ...
                    obj.exportToParaview(results.uFeti, 'feti_dp_dirichlet');
                end
            end
        end
    end
    ...
    methods (Access = public)
        ...
        function results = solveFetiDP(obj)
            ...
            results.monolithicPrecondType   = obj.monolithicPreconditioner;
            ...
            % --- Case 1: Monolithic (with preconditioner for CG) ---------
            if obj.computeMonolithic
                tic;
                ...
                % Direct solve (reference solution for error calculation)
                tic;
                s.stiffness = kGlobal;
                s.forces    = fGlobal;
                ...
                % Preconditioned CG on reduced system
                ...
                % Select preconditioner for monolithic system
                switch lower(obj.monolithicPreconditioner)
                    case 'direct'
                        % Direct solver (no preconditioner needed for CG reference)
                        Pmono = @(r) r;
                        fprintf('Monolithic: Using unpreconditioned CG (for iteration count)\n');

                    case 'ilu'
                        fprintf('Monolithic: Building ILU preconditioner (size: %d)...\n', length(fRed));
                        try
                            iluParams.LHS = kRed;
                            iluPrecond = PreconditionerILU(iluParams);
                            Pmono = @(r) iluPrecond.apply(r);
                            fprintf('  ILU preconditioner ready\n');
                        catch ME
                            warning('ILU failed: %s. Falling back to unpreconditioned.', ME.message);
                            Pmono = @(r) r;
                        end

                    case 'amg'
                        fprintf('Monolithic: Building AMG preconditioner (size: %d)...\n', length(fRed));
                        try
                            % 1. Configurar el BCApplier igual que el tutorial
                            sB.mesh = obj.globalMesh;
                            sB.boundaryConditions = obj.boundaryConditions;
                            BCAp = BCApplier(sB);

                            % 2. Calcular modos de sólido rígido usando las funciones del framework
                            % Usamos el centro geométrico o un punto de referencia [1.0, 0.1]
                            refPoint = [max(obj.globalMesh.coord(:,1))/2, max(obj.globalMesh.coord(:,2))/2];
                            Rfull = obj.computeRigidBodyModes(refPoint);

                            % 3. Reducir el Null Space para que coincida con las dimensiones de kRed
                            R = zeros(length(fRed), size(Rfull,2));
                            for i = 1:size(Rfull,2)
                                R(:,i) = BCAp.fullToReducedVectorDirichlet(Rfull(:,i));
                            end

                            % 4. Configurar el struct tal y como exige el tutorial
                            s_amg.type      = 'ELASTIC';
                            s_amg.nullSpace = R;
                            s_amg.nLevels   = 5;
                            s_amg.tol       = 1e-8; % Tolerancia original del tutorial
                            s_amg.maxIter   = 1;

                            % Si tu wrapper pyAMG requiere la matriz en la creación (asegúrate de este paso):
                            s_amg.matrix    = kRed;

                            pyAmgPrec = pyAMG.create(s_amg);
                            Pmono = @(r) pyAmgPrec.solve(kRed, r);
                            fprintf('  AMG preconditioner ready\n');

                        catch ME
                            warning('AMG failed: %s. Falling back to unpreconditioned.', ME.message);
                            Pmono = @(r) r;
                        end

                    otherwise
                        error('Unknown monolithic preconditioner: %s. Options: ''direct'', ''ilu'', ''amg''', ...
                            obj.monolithicPreconditioner);
                end

                tic;
                [~, residualMono] = PCG.solve(@(x) kRed * x, fRed, x0, Pmono, tol, uRed);
            ...
            % --- Assemble FETI-DP interface problem ----------------------
            tic;
            ...
            % --- Case 3: Preconditioned FETI-DP (Dirichlet only) ---------
            fprintf('FETI-DP: Using Dirichlet preconditioner (standard)\n');
            ...
            if obj.computeKappa
                % Build preconditioner matrix for condition number estimation
        ...
        function R = computeRigidBodyModes(obj,refPoint)
            rigModes = RigidBodyFunction.create(obj.globalMesh, refPoint); 
        ...
        function printResults(obj, r)
            ...
            if r.computeKappa
                ...
                if r.computeMonolithic
                    fprintf('| Monolithic CG + %-9s | %5d | %12.2f | %6d | %10.4f | %10.4f |\n', ...
                        upper(r.monolithicPrecondType), r.nIterMono, r.kappaK, r.nDofsMono, r.timeSetupMono, r.timeSolveMonoCG);
                end
                ...
                fprintf('+------------------------------+-------+--------+------------+------------+\n');
                if r.computeMonolithic
                    fprintf('| Monolithic CG + %-9s | %5d | %6d | %10.4f | %10.4f |\n', ...
                        upper(r.monolithicPrecondType), r.nIterMono, r.nDofsMono, r.timeSetupMono, r.timeSolveMonoCG);
                end
                if r.computeUnpreconditioned
        ...
        function plotConvergence(obj, r, tol)
            figure('Name', 'Convergence Comparison', 'Color', 'w', 'Position', [100 100 750 480]);
            hold on;

            if r.computeMonolithic && ~isempty(r.residualMono)
                % Color según el preconditioner del monolítico
                switch lower(r.monolithicPrecondType)
                    case 'direct'
                        monoColor = [0.00 0.45 0.74];
                    case 'ilu'
                        monoColor = [0.93 0.69 0.13];
                    case 'amg'
                        monoColor = [0.64 0.08 0.18];
                    otherwise
                        monoColor = [0.00 0.45 0.74];
                end
                semilogy(1:length(r.residualMono), r.residualMono, '-o', ...
                    'Color', monoColor, 'LineWidth', 1.8, 'MarkerSize', 4, ...
                    'DisplayName', sprintf('Monolithic CG (%s)', upper(r.monolithicPrecondType)));
            end
            ...
            % FETI-DP con Dirichlet (siempre igual)
            ...
            title(sprintf('CG Convergence - 2D Elasticity (Monolithic: %s)', upper(r.monolithicPrecondType)), 'FontSize', 13);
