...
%         end
        ...
        function extractFETIdofs(obj, mSb, ~)
            ...
            for i = 1:nSub
                ...
                isOnBoundary = abs(coords(:,1) - minX) < tol | abs(coords(:,1) - maxX) < tol | abs(coords(:,2) - minY) < tol | abs(coords(:,2) - maxY) < tol;
                
                localBoundaryNodes = find(isOnBoundary);
                
                localDualNodes = setdiff(localBoundaryNodes, localPrimalNodes);
                
                localRemNodes = setdiff(1:size(coords,1), localBoundaryNodes);
