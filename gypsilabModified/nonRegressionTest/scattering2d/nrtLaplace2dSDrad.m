...
% Spherical sources
SW      = @(X) femGreenKernel(X,X0,'[log(r)]',[]);
dxSW{1} = @(X) femGreenKernel(X,X0,'gradx[log(r)]1',[]);
dxSW{2} = @(X) femGreenKernel(X,X0,'gradx[log(r)]2',[]);
dxSW{3} = @(X) femGreenKernel(X,X0,'gradx[log(r)]3',[]);
...
% Single Layer
tic
S   = -1/(2*pi) .* integral(radiat.vtx,sigma,Gxy,u);
...
% Double layer
tic
D  = -1/(2*pi) .* integral(radiat.vtx,sigma,dyGxy,ntimes(u));
...
% Single Layer
tic
S  = -1/(2*pi) .* integral(sigma,sigma,u,Gxy,u);
...
% Double layer
tic
D  = -1/(2*pi) .* integral(sigma,sigma,u,dyGxy,ntimes(u));
