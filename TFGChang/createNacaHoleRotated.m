function fV = createNacaHole(x,y,s)
    ...
    xu = (xNaca).*cos(AoA) - (yu - y0).*sin(AoA) + x0/c;
    yu = (xNaca).*sin(AoA) + (yu - y0).*cos(AoA) + y0;
    xl = (xNaca).*cos(AoA) - (yl - y0).*sin(AoA) + x0/c;
    yl = (xNaca).*sin(AoA) + (yl - y0).*cos(AoA) + y0;

    x0Rotated    = min([xu,xl]);
    xNacaRotated = (x-x0Rotated)/c;
    f(:,:,:,1)   = yl*c - y;
    f(:,:,:,2)   = y - yu*c;
    f(:,:,:,3)   = xNacaRotated - 1*cos(AoA); 
    f(:,:,:,4)   = -xNacaRotated;
    fV           = -max(f,[],4);
