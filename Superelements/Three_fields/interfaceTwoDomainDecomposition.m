...
testConnectionLeftMesh = P1Function.create(boundaryConnectionLeftMesh.mesh,2);
s.type       = 'MassMatrix';
s.mesh       = boundaryConnectionLeftMesh.mesh;
s.test       = testConnectionLeftMesh;
s.trial      = trialConnectionMesh;
lhs          = LHSintegrator.create(s);
cConnectionLeft  = lhs.compute();

testConnectionRightMesh = P1Function.create(boundaryConnectionRightMesh.mesh,2);
s.type       = 'MassMatrix';
s.mesh       = boundaryConnectionLeftMesh.mesh;
s.test       = testConnectionRightMesh;
s.trial      = trialConnectionMesh;
lhs          = LHSintegrator.create(s);
cConnectionRight  = lhs.compute();

% Interface matrix computation
InterfaceMesh = boundaryConnectionLeftMesh.mesh;
trialInterfaceMesh = P0Function.create(InterfaceMesh,2);
s.type       = 'MassMatrix';
s.mesh       = boundaryConnectionLeftMesh.mesh; % We are using the same mesh as the subdomains' boundaries.
s.test       = testConnectionLeftMesh;
s.trial      = trialInterfaceMesh;
lhs          = LHSintegrator.create(s);
dInterfaceLeft  = lhs.compute();

s.type       = 'MassMatrix';
s.mesh       = boundaryConnectionRightMesh.mesh; % We are using the same mesh as the subdomains' boundaries.
s.test       = testConnectionRightMesh;
s.trial      = trialInterfaceMesh;
lhs          = LHSintegrator.create(s);
dInterfaceRight  = lhs.compute();
...
if isa(testConnectionLeftMesh,"P0Function") 
    nDofLagrangeConnectionLeft = boundaryConnectionLeftMesh.mesh.nelem*testConnectionLeftMesh.ndimf;
elseif isa(testConnectionLeftMesh,"P1Function") 
    nDofLagrangeConnectionLeft = boundaryConnectionLeftMesh.mesh.nnodes*testConnectionLeftMesh.ndimf;
end
DofsLagrangeConnectionLeft = 1:1:nDofLagrangeConnectionLeft;
DofsLagrangeConnectionLeft = DofsLagrangeConnectionLeft + nDofTotal;
nDofTotal = max(DofsLagrangeConnectionLeft);

if isa(testConnectionRightMesh,"P0Function") 
    nDofLagrangeConnectionRight = boundaryConnectionRightMesh.mesh.nelem*testConnectionRightMesh.ndimf;
elseif isa(testConnectionRightMesh,"P1Function") 
    nDofLagrangeConnectionRight = boundaryConnectionRightMesh.mesh.nnodes*testConnectionRightMesh.ndimf;
end
DofsLagrangeConnectionRight = 1:1:nDofLagrangeConnectionRight;
DofsLagrangeConnectionRight = DofsLagrangeConnectionRight + nDofTotal;
nDofTotal = max(DofsLagrangeConnectionRight);

% Interface
if isa(trialInterfaceMesh,"P0Function") 
    nDofInterface = InterfaceMesh.nelem*InterfaceMesh.ndim;
elseif isa(trialInterfaceMesh,"P1Function") 
    nDofInterface = InterfaceMesh.nnodes*InterfaceMesh.ndim;
end
DofsInterface = 1:1:nDofInterface;
DofsInterface = DofsInterface + nDofTotal;
nDofTotal = max(DofsInterface);
...
GlobalLHS(DofsLeftConnection,DofsLagrangeConnectionLeft) = cConnectionLeft';
GlobalLHS(DofsRightConnection,DofsLagrangeConnectionRight) = -cConnectionRight';
GlobalLHS(DofsLagrangeDirichlet,DofsDirichlet) = cDirichlet;
GlobalLHS(DofsLagrangeConnectionLeft,DofsLeftConnection) = cConnectionLeft;
GlobalLHS(DofsLagrangeConnectionRight,DofsRightConnection) = -cConnectionRight;
GlobalLHS(DofsInterface,DofsLagrangeConnectionLeft) = - dInterfaceLeft';
GlobalLHS(DofsInterface,DofsLagrangeConnectionRight) = dInterfaceRight';
GlobalLHS(DofsLagrangeConnectionLeft,DofsInterface) = -dInterfaceLeft;
GlobalLHS(DofsLagrangeConnectionRight,DofsInterface) = dInterfaceRight;
...
lambdaConnectionLeft = Globalu(nDofLeftDomain+nDofRightDomain+nDofLagrangeDirichlet+1:nDofLeftDomain+nDofRightDomain+nDofLagrangeDirichlet+nDofLagrangeConnectionLeft);
lambdaConnectionRight = Globalu(nDofLeftDomain+nDofRightDomain+nDofLagrangeDirichlet+nDofLagrangeConnectionLeft+1:nDofLeftDomain+nDofRightDomain+nDofLagrangeDirichlet+nDofLagrangeConnectionLeft+nDofLagrangeConnectionRight);
uInterface = Globalu(nDofLeftDomain+nDofRightDomain+nDofLagrangeDirichlet+nDofLagrangeConnectionLeft+nDofLagrangeConnectionRight+1:nDofLeftDomain+nDofRightDomain+nDofLagrangeDirichlet+nDofLagrangeConnectionLeft+nDofLagrangeConnectionRight+nDofInterface);

% Maximum error calculation
ErrorConnectionDisp = max(max(abs(GlobalLHS(DofsLagrangeConnectionLeft,:)*Globalu)))
