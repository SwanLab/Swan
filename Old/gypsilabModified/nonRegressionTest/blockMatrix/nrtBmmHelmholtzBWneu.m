...
spmd
    ...
    for j = 1:Nlab   
        ...
        % Hypersingular
        H = 1/(4*pi) .* (k^2 ...
            .* integral( sigmaLab{labindex} , sigmaLab{j} , ...
            ntimes(uLab{labindex}) , Gxy , ntimes(uLab{j}) , tol) ...
            - integral( sigmaLab{labindex} , sigmaLab{j} , ...
            nxgrad(uLab{labindex}) , Gxy , nxgrad(uLab{j}) , tol));
        Hr = 1/(4*pi) .* (k^2 ...
            * regularize( sigmaLab{labindex} , sigmaLab{j} , ...
            ntimes(uLab{labindex}) , '[1/r]' , ntimes(uLab{j})) ...
            - regularize( sigmaLab{labindex} , sigmaLab{j} , ...
            nxgrad(uLab{labindex}) , '[1/r]' , nxgrad(uLab{j})));
        
        % Double layer transpose : switch indices for block transposition
        Dt  = 1/(4*pi) .* integral( sigmaLab{j} , sigmaLab{labindex} , ...
            uLab{j} , gradyGxy , ntimes(uLab{labindex}) , tol).';
        Dtr = 1/(4*pi) .* regularize( sigmaLab{j} , sigmaLab{labindex} , ...
            uLab{j} , 'grady[1/r]' , ntimes(uLab{labindex})).';
