...
spmd
    ...
    for j = 1:Nlab   
        ...
        % Single layer
        S  = oprIntegral('S',k,sigmaLab{labindex},uLab{labindex},sigmaLab{j},uLab{j},tol);
        Sr = 1/(4*pi).*regularize(sigmaLab{labindex},sigmaLab{j},uLab{labindex},'[1/r]',uLab{j});
        
        % Double layer
        D  = oprIntegral('D',k,sigmaLab{labindex},uLab{labindex},sigmaLab{j},uLab{j},tol);
        Dr = 1/(4*pi).*regularize(sigmaLab{labindex},sigmaLab{j},uLab{labindex},'grady[1/r]',ntimes(uLab{j}));
...
Sinf = 1/(4*pi) .* integral(nu,sigma,Ginf,k,u,tol);
Dinf = 1/(4*pi) .* integral(nu,sigma,gradxGinf,k,ntimes(u),tol);
...
tic
Sbnd = 1/(4*pi) .* (integral(sigma,sigma,u,Gxy,k,u,tol) + ...
    regularize(sigma,sigma,u,'[1/r]',u));
toc

% Finite element boundary operator --> \int_Sx \int_Sy psi(x)' dny G(x,y) psi(y) dx dy 
tic
Dbnd = 1/(4*pi) .* (integral(sigma,sigma,u,gradyGxy,k,ntimes(u),tol) + ...
    regularize(sigma,sigma,u,'grady[1/r]',ntimes(u)));
toc
...
tic
Sdom = 1/(4*pi) .* (integral(square.vtx,sigma,Gxy,k,u,tol) + ...
    regularize(square.vtx,sigma,'[1/r]',u));
toc

% Finite element radiative operator --> \int_Sx \int_Sy psi(x)' grady(G(x,y)) ny.psi(y) dx dy 
tic
Ddom = 1/(4*pi) .* ( integral(square.vtx,sigma,gradyGxy,k,ntimes(u),tol) + ...
    regularize(square.vtx,sigma,'grady[1/r]',ntimes(u)) );
toc
