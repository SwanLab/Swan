function FE_TRAIN_JOINTS_OD_FIXED(NameFileMeshLOC,NameFileMeshLOC_coarse,NAME_DATA_REF,...
    NAME_ROOT_FE,EXECUTABLE_FOLDER,MESH3D,MESH1D,ijoint,NUMBER_OF_SLICES,...
    PRESCRIBED_VALUE,FIXED_FACE)
...
FOLDER = cd ;
TYPE_BOUNDARY_CONDITIONS =  'PRESCRIBED_FLUCTUATIONS_J1face';  % Fluctuations are given
...
AREJOINTS = find(MESH1D.TYPE.ISBEAM ==0);  % Indices of all joints 
% Which elements have index ijoint (either joint or beam) ?
ISINDEX = find(MESH1D.TYPE.INDEX3D ==ijoint);
ELEM_JOINTS = intersect(AREJOINTS,ISINDEX) ;  % Indicies of the joint under consideration
...
% One of the faces of the joint has no slices attached to it --> FIXED FACE
%  
ISZZ = find(INDEX_MESH_SLICES == 0) ; 
INDEX_MESH_SLICES(ISZZ) = [] ;  
% 
...
for iprojects = 1:length(NAMEPROJECTS)
    ...
    if isempty(NAMEPROJECTS{iprojects})
        ...
        FUNinput.INPUTS.FIXED_FACE = FIXED_FACE ;
