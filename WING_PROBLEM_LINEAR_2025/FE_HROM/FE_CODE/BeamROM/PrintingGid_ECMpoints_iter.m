function [DATAIN] = PrintingGid_ECMpoints_iter(DATAIN,DATA_REFMESH,DATA_GENGAUSS,HISTORY_ITERATIONS) ;

if nargin == 0
    load('tmp1.mat')
    DATA_GENGAUSS.PLOT_SELECTED_ELEMENTS_ALONG_ITERATIONS =3  ; 
end


if DATA_GENGAUSS.PLOT_SELECTED_ELEMENTS_ALONG_ITERATIONS == 2
    NAMEMESH = {} ;
    for  i = 1:length(HISTORY_ITERATIONS.ELEMENTS_CONTAINING_POINTS)-1
        NAMEMESH{i} = ['ITER',num2str(i)] ;
        NAMEMESH{end+1} = 'FULL' ;
    end    
elseif DATA_GENGAUSS.PLOT_SELECTED_ELEMENTS_ALONG_ITERATIONS == 3
    ELEMS_ini = HISTORY_ITERATIONS.ELEMENTS_CONTAINING_POINTS{1} ; 
    ELEMS_FIN = HISTORY_ITERATIONS.ELEMENTS_CONTAINING_POINTS{end} ;
...
for i = 1:length(CNred)
    CNred{i}  = CNref(HISTORY_ITERATIONS.ELEMENTS_CONTAINING_POINTS{i},:) ;
    MatRED{i} = i*ones(size(DATA_REFMESH.MaterialType(HISTORY_ITERATIONS.ELEMENTS_CONTAINING_POINTS{i})))  ;
end
...
COORprint = COOR ;
%CNprint = {CNred,CNref} ;
CNprint = CNred ; CNprint{end+1} = CNref;
NAME_INPUT_DATA =NAMEFILEinput ;
MaterialTypegloPRINT = MatRED ; MaterialTypegloPRINT{end+1} = DATA_REFMESH.MaterialType;
...
for  i = 1:length(NAMEMESH)
   % NAMEMESH{i} = ['ITER',num2str(i)] ;
    TypeElementPRINT{i} = DATA_REFMESH.TypeElement  ;
end
% NAMEMESH{end} = ['FULL'] ;
...
[NameFileRES ]= GidPostProcess_MMESH_weights(COORprint,CNprint,TypeElementPRINT, ...
    posgp,NameFile_res,MaterialTypegloPRINT,IND_ELEM_MESHES,...
    DATA_REFMESH,DATA,WEIGHTS);
