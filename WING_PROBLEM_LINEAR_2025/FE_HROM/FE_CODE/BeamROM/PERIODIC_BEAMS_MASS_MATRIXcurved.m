function [Gb,dR,DOFr,DOFm,AREA,R,DOFA,DOFB] = ...
    PERIODIC_BEAMS_MASS_MATRIXcurved(a_A,a_B,DOMAINVAR,COOR,CONNECTb,TypeElementB,...
    DATA,Q)

% COpy of PERIODIC_BEAMS_MASS_MATRIXfun for curved domains, 15-Jan-2019
if nargin == 0 
    load('tmp.mat')
end
...
% Rorth = zeros(size(R')) ; 
% for idim =1:ndim
%     INDLOC =idim:ndim:size(R,1) ; % Rorth = zeros(size(R')) ; 
% for idim =1:ndim
...
[Gb,dR,DOFr,DOFm] = BCs_BEAMS_PERIODICcurved(DOFA,DOFB,R,a_A,a_B,Rbar,Q) ; 
