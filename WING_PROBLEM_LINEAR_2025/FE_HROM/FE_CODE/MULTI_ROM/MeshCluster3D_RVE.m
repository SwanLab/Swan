function [MESHcluster,DATAmeshR] =MeshCluster3D_RVE(MESH2D,MESH3D,iENTITY,itypeSLICE,DATAIN,DATA_REFMESH)
...
% DATAIN.LengthCube_ECMpointsPRINT = 0.01; %   % POST-PROCESSIN ECM ELEMENTS AS POINTS (LENGTH PARAMETER,  CUBE  SURROUNDING THE ELEMENT)
% DATAIN.PRINT_MESH_ECM_POINTS_AS_POINTS  = 1;  % POST-PROCESSIN ECM ELEMENTS AS POINTS  
DATAIN = DefaultField(DATAIN,'LengthCube_ECMpointsPRINT',0.01) ; 
DATAIN = DefaultField(DATAIN,'PRINT_MESH_ECM_POINTS_AS_POINTS',0) ; % If this option is on, then the .res file cannot be printed  


if    ~isempty(DATAIN.setReducedElements)
    
    if  DATAIN.PRINT_MESH_ECM_POINTS_AS_POINTS == 0
    RVE.CN = RVE.CN(DATAIN.setReducedElements,:) ; % Only reduced set of elements 
    RVE.MaterialType = RVE.MaterialType(DATAIN.setReducedElements,:) ;
    else
        % ECM elements are shrinked so that they look like "points" for
        % post-processing purposes. 
        CNred = RVE.CN(DATAIN.setReducedElements,:)  ; 
         [RVE.COOR,RVE.CN]=  PrintMeshECMPointsAsPoints(RVE.COOR,CNred,DATAIN,...
            DATA_REFMESH{iENTITY},DATAIN.setReducedElements);
         RVE.MaterialType = RVE.MaterialType(DATAIN.setReducedElements,:) ;
    end
...
DATAmeshR.POINTS_TRANSLATION = cell(nDOM,1) ; 
DATAmeshR.ROTATIONS = cell(nDOM,1) ; 

for e = 1:nDOM
  %  e
    eGLO = IND_ELEM_TYPE(e) ;
    
    % Relative coordinates of the RVE  with respect to the midside point of
    % face 1
    COORrel  = zeros(size(RVE.COOR')) ;
    for idim=1:ndim
        COORrel(idim,:) = RVE.COOR(:,idim) - RVE.COOR_MIDSIDE_FACE{1}(idim) ;
    end
    
    
    if isempty(MESH2D.rotDOM)
        % What is the midside node corresponding to face 1 ?
        nodoINI = MESH2D.CN(eGLO,:) ;
        ...
        xINI = MESH2D.COOR(nodoINI,:)' ;
        
        
        % Translation  + ROTATION 
    
%       if eGLO == 31 | eGLO ==71 | eGLO ==111 | eGLO == 151
%           
%                   MESH2D.rotDOM{eGLO} = 0.5*eye(3); 
%  
%       end
       COORrel = MESH2D.rotDOM{eGLO}*COORrel ; 
     % end
        for idim = 1:ndim
              COORrel(idim,:) = COORrel(idim,:) + xINI(idim) ;
        end
          DATAmeshR.ROTATIONS{e} = MESH2D.rotDOM{eGLO} ; 
    end
    
    DATAmeshR.POINTS_TRANSLATION{e} = xINI ; 
