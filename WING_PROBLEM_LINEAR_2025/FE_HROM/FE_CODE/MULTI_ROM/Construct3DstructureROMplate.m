function [NameFile_msh,NameFile_res,COORprint,CNprint,NAME_INPUT_DATA,...
    TypeElementPRINT,MaterialTypegloPRINT,NAMEMESH,DATAIN]= Construct3DstructureROMplate(MESH2D,MESH3D,DATAIN,MESH_LATERAL,FOLDER)
...
% 2D mesh ------------------------------
% --------------------------------------
iacumNODES =  size(MESH2D.COORvert,1) ; % Connectivities index (cumulative, for printing global 3D mesh)
    ...
    switch MESH2D.PROP(iENTITY).TYPE ;  % Type of entity
        case {'RVE'}
            ...
            for iLOC = 1:length(itypeGLO)  % Loop over types of RVES (mixed beams)
                ...
                [MESHcluster] =MeshCluster3D_PLATE(MESH2D,MESH3D,iENTITY,itypeSLICE,DATAIN) ;  % Gathering COORDINATES and CONNECTIVITIES
