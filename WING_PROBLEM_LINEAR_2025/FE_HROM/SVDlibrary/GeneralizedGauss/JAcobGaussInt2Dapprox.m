function [D,POLYINFO]= JAcobGaussInt2Dapprox(PHI_der,xNEW,wNEW,PHIk_y,xINT,DATALOC,VAR_SMOOTH_FE,POLYINFO,dPHIk_y)

if nargin == 0
    load('tmp2.mat')
    DATALOC.FUN_GRADIENT_DIRECTLY.NAME_FUN = 'Poly2Dfun';
    DATALOC.FUN_GRADIENT_DIRECTLY.NAME_GRAD= 'dPoly2Dfun';
end
...
%     else
     ...
     %   [dummy, dPHIk_y,POLYINFO]= EvaluateBasisFunctionAtX_approx(xNEW, DATALOC.APPROX_FUN__DERI,VAR_SMOOTH_FE,POLYINFO)  ;
   ...
   % end
    ...
    DerPHIt_x = bsxfun(@times,dPHIk_y{1},wNEW)' ;
    DerPHIt_y = bsxfun(@times,dPHIk_y{2},wNEW)' ;
    D = [DerPHIt_x DerPHIt_y] ; 
%end
...
%end
% Multiplication by diagonal matrix of weights
