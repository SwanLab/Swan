function [xGAUSS,wGAUSS,DATAapprox] = GeneralizedGauss3D(J,W,xMAT,z,w,DATALOC,MPOINTS,S,V,XY)
...
if   DATALOC.LOAD_PREVIOUSLY_COMPUTED_POINTS ==0
    ...
    while CONVERGENCE ==1 & length(xOLD)>1
        ...
        [xNEW,wNEW,CONVERGENCE,DATALOC] = Iter_RemovePoint2D(xOLD,wOLD,xINT,PHI,PHI_der,b,DATALOC,...
            VSinv) ;
        if CONVERGENCE == 1
            xOLD = xNEW ;
            wOLD = wNEW ;
...
if DATALOC.SHOW_FIGURES==1
    ...
    %         plot3(xMAT(:,1),xMAT(:,2),xMAT(:,3),'.','Color',[0 1 0])
    %     end
    ...
    h2 = plot3(xOLD(:,1),xOLD(:,2),xOLD(:,3),'x','Color',[1 0 0],'MarkerSize',8);
     hseed = plot3(xMAT(zOLD,1),xMAT(zOLD,2),xMAT(zOLD,3),'*','Color',[1 0 1],'MarkerSize',8);
...
% Nearest points
% ----------------
STUDY_NEAREST_POINT = 0 ;
if STUDY_NEAREST_POINT ==1
    dt = DelaunayTri(xMAT(:,1),xMAT(:,2),xMAT(:,3));
    NP = nearestNeighbor(dt, xOLD(:,1),xOLD(:,2),xOLD(:,3));
