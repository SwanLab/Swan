function [z,w,ERROR_GLO,DATAOUT]= EmpiricalCubatureMethod_NoIter(G,W,DATA)
% This is a modification of EmpiricalCubatureMethod_CANDcompl, described
% below.
% In this version, negative weights make the ECM to stop and leave the
% comptutations (because in such cases BasisF is to be enlarged)
% See /home/joaquin/Desktop/CURRENT_TASKS/MATLAB_CODES/FE_HROM/LARGE_STRAINS/NonLinearManifolds/ManifoldAdaptWeightsECM.m
...
z=[] ; w = [] ; ERROR_GLO= [] ; DATAOUT = [] ;

% free memory
b = G*sqrt(W) ;
nbEXACT = norm(b) ;                          % norm used in error metric
...
DATA = DefaultField(DATA,'NITERATIONS_NO_MORE_POINTS_negative_iterationECM',10) ;
NITERATIONS_NO_MORE_POINTS = DATA.NITERATIONS_NO_MORE_POINTS_negative_iterationECM ;
ITER_MINIMUM               = 0 ;
MAX_NUMBER_POINTS          = 0 ;
...
while  (nerrorACTUAL > TOL) && (mPOS < m) && (k < NITERACIONES)
    ...
    %if length(y) == 1 && ObjFun(1) <= TOL_ObjFun
    %   ObjFunNotSingular = 1;
    if maxLOC <= TOL_ObjFun
        disp('ERROR: ObjFun is equal to zero (or negative for all candidates); ill-posed problem')
        ObjFunNotSingular = 1;
    else
        ObjFunNotSingular = 0 ;
        if k == 1 || isempty(z)
    ...
    %  if ITER_MINIMUM > NITERATIONS_NO_MORE_POINTS ||      (isempty(y) && ( length(z) < m  ) ) || ObjFunNotSingular == 1
    
    if   ( ObjFunNotSingular == 1  ) && length(yORIG) == m
        disp('--------------------------------------------------------------')
        disp('The algorithm cannot proceed with the current set of points')
        disp('We have to enlarge the basis matrix (outside this function)')
        return
    elseif  (isempty(y) && ( length(z) < m  ) ) || ( ( ObjFunNotSingular == 1 ) && length(yORIG) < m)
        ...
        if ~isempty(yCOMPL)
            ...
            disp(['No convergence is achieved with the current set of candidate points'])
            disp('SAW-ECM strategy has not worked out')
