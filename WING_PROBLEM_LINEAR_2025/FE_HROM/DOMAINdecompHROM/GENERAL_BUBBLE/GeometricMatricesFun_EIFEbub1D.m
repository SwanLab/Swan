...
[Bmat_allelem,Nmat_allelem,WEIGHTSinteg,TRANSF_COORD,MESH.CN,DATA,Vrot,Kelem,MESH.CN_SUPPORT  ]=...
    B_N_matricesEIFEbub1D(MESH.COOR,MESH.CN, PROPMAT,MESH.MaterialType,DATA,MESH.TypeElement,CN_SUPPORT ) ;
...
if DATA.NO_USE_Deformation_gradient_in_Small_Strains == 1 && DATA.SMALL_STRAIN_KINEMATICS ==1
 ...
 %   Bst_F = BstSmallStrains_EIFEbub1D(DATA.MESHextended,nstrain,Bmat_allelem) ;
...
ndim_FINE = size(posgp_RHS,1) ; 
Nst = AssemblyNGlobalBUB1D(Nshapeelem,DATA.MESHextended,ngaus_RHS,DATA,ndim_FINE ) ;
...
%  [BasisUrb,VOLUME,Rbar,CENTROID,INERTIA ]= ConstructBasisRigidBody_MASSM1D(MESH.COOR,Nst(:,DATA.MESHextended.DOFS_bLOC),wSTs_RHS,DATALOC) ; %
% % 
