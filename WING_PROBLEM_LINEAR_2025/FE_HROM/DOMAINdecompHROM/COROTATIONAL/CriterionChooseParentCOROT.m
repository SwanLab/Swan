function [TRANSF_COORD,PERMUT_chosen,CNnew] = CriterionChooseParentCOROT(TRANSF_COORD_perm,CNold,DATA,PERMUT)
...
%             Q11(iPERM) = TRANSF_COORD_perm{iPERM}.QrotINI(1,1) ;
%         end
%     end
...
for   iPERM = 1:length(TRANSF_COORD_perm)
    ...
    if ~isempty(TRANSF_COORD_perm{iPERM})
        if  size( TRANSF_COORD_perm{iPERM}.QrotINI,2) == 2
            Q11(iPERM) = atand(TRANSF_COORD_perm{iPERM}.QrotINI(1,2)/TRANSF_COORD_perm{iPERM}.QrotINI(1,1)) ;
            
            ang_cos = acosd(TRANSF_COORD_perm{iPERM}.QrotINI(1,1)) ;
            ang_sin = asind(TRANSF_COORD_perm{iPERM}.QrotINI(1,2)) ;
            ...
            Q11(iPERM) = TRANSF_COORD_perm{iPERM}.QrotINI(1,1) ;
            Q33(iPERM) = TRANSF_COORD_perm{iPERM}.QrotINI(3,3) ;
        end
    end
end
