function [VAR,CONVERGED,DATA] = NewtonRapshonStaticLarge_COROT(DATA,OPERFE,VAR,MATPRO,DOFl,dCqLOC,KcLINq,BstQ,D_QrotALL)
% Adaptation of NewtonRapshonStaticLarge.m, corotational approach
...
while  kiter <=DATA.NEWTON_RAPHSON.NMAXiter
    ...
    if isempty(celastST) && DATA.INTERNAL_FORCES_USING_precomputed_Kstiff == 0
        ...
        if CONVERGED == 1
            ...
            VAR = KineticAndStrainEnergyGet_COROT(VAR,OPERFE,DATA,MATPRO,FgradST,dCqLOC,D_QrotALL) ;   
            break
        else
