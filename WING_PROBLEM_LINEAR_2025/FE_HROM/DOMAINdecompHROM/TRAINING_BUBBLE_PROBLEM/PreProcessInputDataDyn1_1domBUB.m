function   [MESH,MATPRO,OPERFE,Fbody,Ftrac,DISP_CONDITIONS,INICOND,DATA,OTHER_data] ...
    =  PreProcessInputDataDyn1_1domBUB(DATA,PROPMAT,DIRICHLET,NEUMANN,INITIAL_CONDITIONS,OTHER_INPUTS,EIFEM_library,MESHdom)
% Adaptation of PreProcessInputDataDyn1.m to the problem of bubble boundary
% conditions
...
% Generated by training in the linear elastic range. See
% /home/joaquin/Desktop/CURRENT_TASKS/MATLAB_CODES/FE_HROM/DOMAINdecompHROM/MultiHROM/Train1dom_all.m
load(EIFEM_library,'EIFEoper','OPERFE','GEOMATRICES','MODES') ;
...
GEOproperties = MESH.GEOproperties  ;
...
[MATPRO,MESH,DATA,INICOND] = MaterialPropertiesIntegPoints(MESH,DATA,PROPMAT)  ;
...
DATA.MESH.ndofSTRESS =   DATA.MESH.ngausT*nstrain; %
...
% 5.  MASS MATRIX
% -----------------------
DATA.MESH.ngaus_RHS = OPERFE.ngaus_RHS ;
densGLO = repmat(MATPRO.dens',OPERFE.ngaus_RHS,1) ;
densGLO = densGLO(:) ;
...
mass_point =  OPERFE.wSTs_RHS.*densGLO;
mass = sum(mass_point) ;
%for idim = 1:3
COOR = MESH.COOR' ;
NstCOOR = OPERFE.Nst*COOR(:) ;
for idim = 1:ndim
...
[DISP_CONDITIONS] =  DirichletCONDtime_1domBUB(DIRICHLET,DATA,ndim,MESH, MODES) ; 
...
% -------------------------
% 9. INITIAL CONDITIONS  (not necessary )
% --------------------------
