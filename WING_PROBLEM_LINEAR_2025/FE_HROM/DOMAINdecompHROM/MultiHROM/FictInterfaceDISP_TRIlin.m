function [Vall,Mintf,MESH,DATA] = FictInterfaceDISP_TRIlin(PhiDEF,PsiDEFf,MESH,DATAcommon,DATA,Mintf,PhiRB,PsiRBf,DATAoffline)
...
COORtri = MESH.COOR(CORNERS,:);

% FOR COMPARISON PURPOSES WITH THE CECM INTEGRATION RULE
MESHLOC.COOR = COORtri  ;
MESHLOC.CN = [1,2,3];
DATAloc.MESH = MESHLOC ;
DATAloc.NumberOfGaussPointsPerElement = [2,2] ;
DATAloc.MESH.TypeElement = 'Triangle' ;
[~,DATA.weigthsGAUSSIAN,DATA.xGAUSSIAN] = GetMeshVariablesGAUSStri(DATAloc) ;
...
% ORDER_POLYNOMIALS =1 ;
DATAshape = ShapeFunCoefficientsTRI(COORtri) ;
...
FESHAPE_coarse_elem_transf_coord.COOR =COORtri ; % 
FESHAPE_coarse_elem_transf_coord.CN =1:size(COORtri,1) ; 
FESHAPE_coarse_elem_transf_coord.DATA_ShapeFunctionFE =DATAlocSHAPE ; 
FESHAPE_coarse_elem_transf_coord.ShapeFunction = 'ShapeFunctionFEtri' ; 
