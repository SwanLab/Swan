function [NameFile_res ]= GidPostProcessEIFE(COOR,CN,TypeElement,d, STRESSES_FINE,...
    posgp,NameFile_res,MaterialType,DATAIN,DATA,stressGLO_coarse,IND_ELEM_MESHES);
...
for imeshLOC  = 1:length(STRESSES_FINE)
    ...
    if ~isempty(CN{imesh})
        ...
        if ~isempty(STRESSES_FINE{imeshLOC})
...
GidResults2DFE_EIFE(NameFile_res,ndim,NODAL_VECTOR,NODAL_SCALAR,MESH,DATALOC);
