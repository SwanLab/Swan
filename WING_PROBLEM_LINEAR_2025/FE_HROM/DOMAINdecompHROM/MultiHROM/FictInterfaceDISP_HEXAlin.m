function [Vall,Mintf,MESH,DATA] = FictInterfaceDISP_HEXAlin(PhiDEF,PsiDEFf,MESH,DATAcommon,DATA,Mintf,PhiRB,PsiRBf,DATAoffline)
% Fictitious interface modes for hexahedra elements
...
%2.) Determination CORNERS 1, 2 3 to 8
...
if ~isempty(COMB_FACES) && isempty(DATAcommon.MESH_PARENT_DOMAIN_COARSE)
    ndim = 3;
    ncorners = 8;
    CORNERS = zeros(ncorners,1) ;
    disp(['If this function fails, set DATAoffline.CORNERS_DEFINING_MACRO_ELEMENT_AS_INTERSECTIONS_ENTITIES =[], and create in GID'])
    disp(['a mesh file with the H8 element containing the RVE (name of the mesh file DATAcommon.MESH_PARENT_DOMAIN_COARSE'])
    
    for icorner = 1:length(COMB_FACES)
        iface = COMB_FACES{icorner}(1) ;
        jface = COMB_FACES{icorner}(2) ;
        LINES =    intersect(MESH.BNDinterface(iface).NODES,MESH.BNDinterface(jface).NODES) ;
        kface =  COMB_FACES{icorner}(3) ;
        CORNERS(icorner) =    intersect(MESH.BNDinterface(kface).NODES,LINES) ;
    end
    
    COORhexa = MESH.COOR(CORNERS,:);
else
    ...
    if ~isempty(DATAcommon.MESH_PARENT_DOMAIN_COARSE)
        ...
        % This is the EIF element that encloses the domain under study
        
        COORgid = MESHparent.COOR(MESHparent.CN,:) ;
        
        CENT = MESH.GEOproperties.CENTROID ;
        ndim = size(CENT,2);
        for idim = 1:ndim
        ...
        error('MESH_QUADRATIC_PARENT_DOMAIN is required')
