function [VAR,CONVERGED,DATA] = NewtonRapshonStaticLarge_manifoldECMnonMD(DATA,OPERFE,VAR,MATPRO,DOFl)
%--------------------------------------------------------------------------
% This is a modification of NewtonRapshonStaticLarge_manifoldECMnonFST (described in the sequel)
% The goal is to introduce 2 latent variables (as done in 
%  NewtonRapshonStaticLarge_manifoldMD.m), while maintaining the adaptive
%  ECM 
% JAHO, 29th August 2025, Balmes 185, Barcelona 
...
while  kiter <=DATA.NEWTON_RAPHSON.NMAXiter
    ...
    [VAR,celastST,FgradST,detFgrad,fINTredNON_mst_DOFl,wMSTeq,ResREDeqMST] =...
        ResidualFromDisplacementsVARnonECMmd(OPERFE,VAR,MATPRO,DATA,VARint_n,tauNONallDER_q) ;
