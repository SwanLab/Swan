function [VAR,CONVERGED,DATA] = NewtonRapshonStatic_MHROM_LSearch_GROK_v1(DATA,OPERFE,VAR,MATPRO,DOFl)
    ...
    if isempty(celastST) && DATA.INTERNAL_FORCES_USING_precomputed_Kstiff == 0
        ...
        if CONVERGED == 1
            ...
            if ~converged_step
                ...
                disp('NR failed → switching to gradient descent');
                [VAR, normd, ~] = gradient_descent_step(VAR, Rl, DOFl, OPERFE, ...
                    VARint_n, DATA, MATPRO, kiter, m0);
                %  converged_step = true;   % prevent abort
