function OFFLINE_STAGE_nonECM_genFASTd2(DATAoffline,DATA_interp,...
    NAME_BASE,DATA_GENGAUSS,InputDataForForces,DATA_interp_ECM)
%%--------------------------------------------------------------------------
% Adaptation  of OFFLINE_STAGE_nonECM_genFASTd; case linear modes as master
% modes
% 
%a--------------------------------------------------------------------------%
%% VErsion of OFFLINE_STAGE_nonECM_genFAST adapted to cope with problems with 
% nonhomogeneous Dirichlet BCs
% 11th-Sept. 2025, 8:31,  T2 Airport Barcelona
%-----------------------------------------------------------------------------
%%
...
switch   DATA_interp.METHOD_SELECT_REFERENCE_MODE
    ...
    case 'FIRST_SVD_MODE'
        error('Option implemented in  OFFLINE_STAGE_nonECM_genFASTd.m ')
%         [PhiLIN,PhiNON,qINF,qSUP,UU,SS,VV,MESH,DATA,DOFl,DOFr,OPERFE,MATPRO,DISP_CONDITIONS,OTHER_output]...
...
%     case 'NN'
%         error('Option not implemented without anonymous functions')
...
SNAPstressPonePROJ = cell(1,length(CASES_orth)) ;
