function    [RESULTANTS,ANG,WEIGHT,Moment_mass ]= ReactionsResultant_rotating_body(SNAPreactions,DOFr,MESH,DATAINPUTloc,Fbody_U)
...
for istep = 1:nsteps
    ...
    if  isempty(DATAINPUTloc.AngleVersusTime_vector)
        t = DATAINPUTloc.TIME_STEPS(istep) ;
    ...
    if ndim == 2
        ...
        RESULTANTS(1:2,istep) = sum(REACTSloc_ROT,2) ;
        ...
        R= [cosd(ANG) -sind(ANG)  0
            ...
            0           0        1] ;
        REACTSloc_ROT = R'*REACTSloc ;
        RESULTANTS(1:3,istep) = sum(REACTSloc_ROT,2) ;
