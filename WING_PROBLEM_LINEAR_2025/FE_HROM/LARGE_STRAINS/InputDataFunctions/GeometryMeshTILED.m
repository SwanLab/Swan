function MESH = GeometryMeshTILED(DATA)
% JAHO, 28-May-2025
if nargin == 0
    load('tmp1.mat')
end
...
if DATA.READ_MATERIAL_COLUMN == 1
    if DATA.GID_VERSION_WITH_MESHES_DIFFERENT_SURFACES_SEPARATED == 0
        [MESHrve]= ReadMeshFileStr(DATA.NameFileMeshDATA,'READ_MATERIAL_COLUMN',1)  ;
    else
        [MESHrve]= ReadMeshFileStr_MULT(DATA.NameFileMeshDATA,'READ_MATERIAL_COLUMN',1)  ;
    end
    
else
    % 23-March-2024
     [MESHrve]= ReadMeshFileStr(DATA.NameFileMeshDATA,'READ_MATERIAL_COLUMN',0)  ;
end
 


nCOOR = size(MESHrve.COOR,1) ;
nCN = length(unique(MESHrve.CN));
if nCOOR ~= nCN
...
[MESHrve.NODES_FACES,MESHrve.NODES_LINES,MESHrve.NODES_DOM] = NodesFacesLinesGID(DATA.NameFileMeshDATA(1:end-4)) ;
%%%%%%
ndim = size(MESHrve.COOR,2);   %CH2D

if ndim==3
    error('Option not implemented yet')
elseif ndim == 2
    % TILING COPIES OF THE DOMAIN
    
    SizeArray = DATA.TILED_MESH.SizeArray ;
    ScaleFactor = 1;
    [MESH] = ConstructArrayMESH_2D(SizeArray,MESHrve ) ;
