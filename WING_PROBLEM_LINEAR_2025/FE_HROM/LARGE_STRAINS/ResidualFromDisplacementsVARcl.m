function  [VAR,celastST,FgradST,detFgrad] =  ResidualFromDisplacementsVARcl(OPERFE,VAR,MATPRO,DATA,VARint_n,wSTs) 
% Similar to ResidualFromDisplacementsVAR, described below, but with
% state-dependent weights (cluster)
%  JAHO, 8-Sept-2025, HGs, Pedralbes, Barcelona. 
...
if isempty(VAR.PK2STRESS) && DATA.INTERNAL_FORCES_USING_precomputed_Kstiff ==0
    ...
    if isempty(OPERFE.KinternalFORCES_given)
     %   if ~isfield(OPERFE,'BstW')           
        VAR.FINT = InternalForcesCL(OPERFE,VAR.PK1STRESS,VAR.PK2STRESS,DATA,VAR,wSTs) ;
      %  else 
