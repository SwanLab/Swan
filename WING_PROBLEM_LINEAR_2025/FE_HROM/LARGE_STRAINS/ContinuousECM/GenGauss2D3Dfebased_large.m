...
DATALOC = DefaultField(DATALOC,'FORCE_POINTS_TO_REMAIN_INSIDE',0); %   Force points to remain inside of the domain 
...
while CONVERGENCE ==1 && length(xOLD)>1
    ...
    [xNEW,wNEW,CONVERGENCE,DATALOC,POLYINFO] = RemovingPointsAlgLARGE(xOLD,wOLD,b,DATALOC,VAR_SMOOTH_FE) ;
    if CONVERGENCE == 1
...
disp('--------------------------------------------------')
disp(['Final integration rule with m =',num2str(length(xOLD)),' POINTS  (of ',num2str(size(coorECM,1)),'). ',...
...
[PHIk_y,~,POLYINFO]=     EvaluateBasisFunctionAtX_LARGE(xOLD,[],VAR_SMOOTH_FE,POLYINFO) ; 
