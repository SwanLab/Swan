function sol = UnsteadyConvectionProblem(meth,n_end,nstep,recording)

clc;

disp(' ')
disp('This program solves a transient pure convection problem')
disp('on a square domain [-0.5, 0.5] x [-0.5, 0.5] with 20x20 elements')
disp('and homogeneous Dirichlet boundary conditions')
disp('Initial condition is a product-cosine hill.')
disp('Convective velocity is v(x,y)=[-y,x].')
...
[X,T] = CreateMesh(x_lo,x_up,y_lo,y_up,nx,ny);
numnp = size(X,1);
...
% TIME INTEGRATION SCHEME
disp(' ')
disp('Spatial discretization is performed first using linear finite elements.')
disp('Then, transient response is computed using a time-stepping algorithm.')
disp('The following methods can be used to solve the problem:')
disp('[1]: TG2          Galerkin formulation + Lax-Wendroff method with consistent mass matrix')
disp('[2]: LW-FD        Galerkin formulation + Lax-Wendroff method with lumped mass matrix')
disp('[3]: TG3          Galerkin formulation + third-order explicit Taylor-Galerkin method')
disp('[4]: CN           Galerkin formulation + Crank-Nicolson with consistent mass matrix')
disp('[5]: CN-FD        Galerkin formulation + Crank-Nicolson with lumped mass matrix')
disp('[6]: CJ           Least-squares formulation + Crank-Nicolson method (Carey-Jiang)')
disp('[7]: TG3-2S       Galerkin formulation + third order two-step Taylor-Galerkin method')
