...
% DEFINE BOUNDARY CONDITIONS (Definició de les fronteres del domini)
...
% Original (no-slip condition) (Definició de condicions de contorn de
% velocitat (Dirichlet))
...
size_cutmesh = size(uMesh.boundaryCutMesh.mesh.coord,1);%done
dirDofspresscyl=zeros(2,size_cutmesh); %done
...
coor_occult=zeros(size_cutmesh,2); %done
for i = 1:1:size_cutmesh
...
% Pressure bc (Condicions de contorn per la pressió, Neumann)
...
for i = 1:1:4  %Assignació de les condicions de contorn
    if i == 1 || i == 2
