%% ACADEMIC PLOT TEST 1 %%
close all

load("NullSpaceVariablesAcademic1.mat");
cost.Null  = c;
...
load("AugmentedLagrAcademic1.mat");
k2 = v;

load("fminconSQPacademic1.mat");
k3 = v;

load("fminconIPOPTacademic1.mat");
k4 = v;

% c1 = @(x) 1/x(1) - x(2);
% c2 = @(x) x(1)+x(2)-3;

v = 0:0.001:3;  % plotting range from -5 to 5
[x,y] = meshgrid(v);  % get 2-D mesh for x and y
cond1 = 1./x - y < 0;  % check conditions for these values
cond2 = x+y-3 < 0;
...
plot(k(1,1),k(2,1),'o','Color','k','LineWidth',2)
plot(k(1,end),k(2,end),'p','Color','k','LineWidth',2)
grid on
