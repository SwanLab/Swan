classdef MINRES_Pol < Solver
    ...
    methods (Static)
        ...
        function x_k = solve(A,b)
            ...
            [Alpha, Beta, v_0, V, c, s, Gamma, Delta, Epsilon, phi, d, x0, t] = MINRES_Pol.init(k, n, b, A);   
            ...
            [Alpha(j), Beta(j+1), V(:,j+1)] = MINRES_Pol.lanczosProc(A, V(:,j), v_0, Beta(j));
            ...
            [c, s, Gamma(j)] = MINRES_Pol.computeH(Beta, Gamma(j), j);
            ...
            [Alpha(j), Beta(j+1), V(:,j+1)] = MINRES_Pol.lanczosProc(A, V(:,j), V(:,j-1), Beta(j));
...
%             for j = 3:k
            while (j<maxiter)&&(res>tol)
                ...
                [c, s, Gamma(j)] = MINRES_Pol.computeH(Beta, Gamma(j), j);            % R(:,j) is now completed    
        ...
        function [Alpha, Beta, v_0, V, c, s, Gamma, Delta, Epsilon, phi, d, x0, t] = init(k, n, b, A)
