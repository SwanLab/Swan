classdef MeshSymmetrizer < handle
    ...
    methods (Access = private)
        ...
        function sConnec = computeSymmetricConnec(obj)
            ...
            sConnec = obj.unifyNodesInSymLine(sConnec);
        end
        
        function sConnec = unifyNodesInSymLine(obj,sConnec)
            nnode  = obj.mesh.nnode;
            for iNode = 1:nnode
                nodesOriginal = obj.mesh.connec(:,iNode);
                coord         = obj.mesh.coord(nodesOriginal,:);
                isInLine      = obj.isNodeInSymmetricLine(coord);
                nodesInLine   = nodesOriginal(isInLine,1);                
