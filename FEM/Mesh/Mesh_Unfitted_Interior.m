classdef Mesh_Unfitted_Interior < Mesh_Unfitted
    methods
        function [subcell_coord_iso,subcell_coord_global,subcell_x_value,interior_subcell_connec] = computeSubcells(obj,fitted_cell_connec,subcell_cutPoints_iso,subcell_cutPoints_global)
            ...
            subcell_x_value = [obj.x_fitted(fitted_cell_connec); zeros(size(subcell_cutPoints_iso,1),1)]'; % !! RENAME !!
...
%         function plot(obj)
%             figure, hold on
%             unfitted_coord_global_x = obj.unfitted_coord_global(:,1);
%             unfitted_coord_global_y = obj.unfitted_coord_global(:,2);
%             plot(unfitted_coord_global_x(obj.unfitted_connec_global),unfitted_coord_global_y(obj.unfitted_connec_global));
%             axis equal off
%             hold off
%         end
    end
