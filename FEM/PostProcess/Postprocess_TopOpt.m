classdef Postprocess_TopOpt < Postprocess
    ...
    methods (Access = public)
        function obj = Postprocess_TopOpt(results)
           obj.Values = results;
        end
        % Export Mesh
        function ToGiD(obj,file_name,input,istep)
            ...
            msh_file = fullfile('Output','TOPOPT_RESULTS',strcat(file_name,'_',num2str(istep),'.flavia.msh'));
        ...
        function ToGiDpost(obj,file_name,input,istep)
            ...
            res_file = fullfile('Output','TOPOPT_RESULTS',strcat(file_name,'_',num2str(istep),'.flavia.res'));
            ...
            %% Print Results
            obj.PrintVector(fid,ndim,'Displacements','U','Elastic Problem','Vector',istep,'OnNodes','',input.variables.d_u);
            obj.PrintTensor(fid,ndim,'Stress','S','Elastic Problem','Vector',istep,'OnGaussPoints',gauss_points_name,input.variables.stress);
            obj.PrintTensor(fid,ndim,'Strain','E','Elastic Problem','Vector',istep,'OnGaussPoints',gauss_points_name,input.variables.strain);
