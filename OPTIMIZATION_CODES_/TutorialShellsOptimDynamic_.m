classdef TutorialShellsOptimDynamic_ < handle
    ...
    methods (Access = public)
        function obj = TutorialShellsOptimDynamic_(varargin)
            ...
            if isSymmetric && forceSymmetry
                ...
                constraint.cF{1} = @(h_half)  (-obj.dynamicProblem(T * h_half) + 10) / normalizef; % Min: 10 Hz
                constraint.gF{1} = @(h_half)  -T' * obj.GradDynamicProblem(T * h_half, nLayers) / normalizef;

                constraint.cF{2} = @(h_half)  (obj.dynamicProblem(T * h_half) - 20) / normalizef; % Max: 20 Hz
                constraint.gF{2} = @(h_half)  T' * obj.GradDynamicProblem(T * h_half, nLayers) / normalizef;
                ...
                constraint.cF{1} = @(h)  (-obj.dynamicProblem(h) + 10) / normalizef;   % Min: 10 Hz
                constraint.gF{1} = @(h)  -obj.GradDynamicProblem(h,nLayers) / normalizef;

                constraint.cF{2} = @(h)  (obj.dynamicProblem(h) - 20) / normalizef;   % Max: 20 Hz
                constraint.gF{2} = @(h)  obj.GradDynamicProblem(h,nLayers) / normalizef;
