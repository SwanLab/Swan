...
path = "NonLinearNull/02_NullSpace_HJ_Compl_VolPer_Original/"

exports = FreeFemRunner(path+"02_Mesh.edp").execute()
Th = exports['Th']
alpha = exports['alpha']
beta = exports['beta']
...
class TO_problem(EuclideanOptimizable):
    def __init__(self):
        ...
        self.ny = []
    def x0(self):
        runner = FreeFemRunner(path+"02_InitialGuess.edp")
        runner.import_variables(Th=Th)
        return runner.execute()['phi[]']

    def J(self, x):
        runner = FreeFemRunner(path+"02_Cost.edp")
...
runner = FreeFemRunner(path+"02_VolFracComputer.edp")
runner.import_variables(Th=Th,phiVal=x)
...
while normdx > params['tol'] and it <= params['maxit']:
    ...
    runner = FreeFemRunner(path+"02_BoundaryRefinement.edp")
    ...
    # Update step
    g = AJ*xiJ+AC*xiC
    runner = FreeFemRunner(path+"02_HJ.edp")
