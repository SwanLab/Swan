...
path = "NonLinearNull/06_NullSpace_HJ_Compl_VolMaxthick_MBB/"

exports = FreeFemRunner(path+"06_Mesh.edp").execute()
Th = exports['Th']
alpha = exports['alpha']
beta = exports['beta']
...
class TO_problem(EuclideanOptimizable):
    def __init__(self):
        ...
        self.ny = []
    def x0(self):
        runner = FreeFemRunner(path+"06_InitialGuess.edp")
        runner.import_variables(Th=Th)
        return runner.execute()['phi[]']

    def J(self, x):
        runner = FreeFemRunner(path+"06_Cost.edp")
        runner.import_variables(Th=Th,phiVal=x,labelDir1=labelDir1,labelDir2=labelDir2,
                                labelNeu=labelNeu,AchiVal=self.volFrac)
    ...
    def dJ(self, x):
        runner = FreeFemRunner(path+"06_CostGradient.edp")
        runner.import_variables(Th=Th,Th2=self.Th2,beta=beta,lsLab=lsLabel,
                                uxVal=self.ux,uyVal=self.uy)
        return runner.execute()['g[]']

    def H(self, x):
        runner = FreeFemRunner(path+"06_ConstraintEq.edp")
        runner.import_variables(Th=Th,AchiVal=self.volFrac)
        H1 =  [runner.execute()['C']]
    
        runner = FreeFemRunner(path+"06_ConstraintIneq.edp")
        runner.import_variables(Th=Th,Th2=self.Th2,phiVal=x,dmax=dmax,rInner=rInner,alpha=alpha,meshsiz=hmin)
        H2 = [runner.execute()['H']]

        return np.hstack((H1,H2))

    def dH(self, x):
        runner = FreeFemRunner(path+"06_ConstraintEqGradient.edp")
        runner.import_variables(Th=Th,Th2=self.Th2,phiVal=x,beta=beta,
                                lsLab=lsLabel,rInner=rInner)
        dH1 = runner.execute()['g[]']
    
        runner = FreeFemRunner(path+"06_ConstraintIneqGradient.edp")
...
group2 = ['normxiJ', 'eps', 'tolerance','dmax']
...
runner = FreeFemRunner(path+"06_VolFracComputer.edp")
runner.import_variables(Th=Th,phiVal=x)
problem._problem.volFrac = runner.execute()['Achi[]']

runner = FreeFemRunner(path+"06_BoundaryRefinement.edp")
...
while normdx > params['tol'] and it <= params['maxit']:
    ...
    # Update step
    g = AJ*xiJ+AC*xiC
    runner = FreeFemRunner(path+"06_HJ.edp")
...
axes[3].plot(iter, dmaxVec, color='b')
axes[3].set_xlabel('Iter')
axes[3].set_ylabel('dmax')
