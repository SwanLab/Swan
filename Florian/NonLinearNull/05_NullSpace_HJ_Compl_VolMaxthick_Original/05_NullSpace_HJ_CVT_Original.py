...
path = "NonLinearNull/05_NullSpace_HJ_Compl_VolMaxthick_Original/"

exports = FreeFemRunner(path+"05_Mesh.edp").execute()
Th = exports['Th']
alpha = exports['alpha']
beta = exports['beta']
...
class TO_problem(EuclideanOptimizable):
    def __init__(self):
        ...
        self.ny = []
    def x0(self):
        runner = FreeFemRunner(path+"05_InitialGuess.edp")
        runner.import_variables(Th=Th)
        return runner.execute()['phi[]']

    def J(self, x):
        runner = FreeFemRunner(path+"05_Cost.edp")
    ...
    def dH(self, x):
        runner = FreeFemRunner(path+"05_ConstraintEqGradient.edp")
        runner.import_variables(Th=Th,Th2=self.Th2,phiVal=x,beta=beta,
                                lsLab=lsLabel,rInner=rInner)
        dH1 = runner.execute()['g[]']
    
        runner = FreeFemRunner(path+"05_ConstraintIneqGradient.edp")
...
runner = FreeFemRunner(path+"05_VolFracComputer.edp")
runner.import_variables(Th=Th,phiVal=x)
problem._problem.volFrac = runner.execute()['Achi[]']

runner = FreeFemRunner(path+"05_BoundaryRefinement.edp")
...
while normdx > params['tol'] and it <= params['maxit']:
    ...
    # Update step
    g = AJ*xiJ+AC*xiC
    runner = FreeFemRunner(path+"05_HJ.edp")
