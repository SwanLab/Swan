...
class TO_problem(EuclideanOptimizable):
    ...
    def G(self, x):
        runner = FreeFemRunner(path+"VolumeConstraintValue.edp")
        runner.import_variables(Th=Th,AchiVal=self.volFrac)
        return [runner.execute()['C']]
    
    def dG(self, x):
        runner = FreeFemRunner(path+"VolumeConstraintGradient.edp")
...
runner.import_variables(Th=Th,phiVal=x,ers=0.001)
problem._problem.volFrac = runner.execute()['Achi[]']

runner = FreeFemRunner(path+"BoundaryComputations.edp")
runner.import_variables(Th=Th,phiVal=x,alpha=alpha)
exports = runner.execute()
...
while normdx > params['tol'] and it <= params['maxit']:
    ...
    nxOld = problem._problem.nx
    nyOld = problem._problem.ny
    kappaOld = problem._problem.kappa
    ...
    for k in range(params['maxtrials']):
        ...
        while itj<=maxItj:
            ...
            runner = FreeFemRunner(path+"VolFracComputer.edp") 
            runner.import_variables(Th=Th,phiVal=newx,ers=0.001)
            problem._problem.volFrac = runner.execute()['Achi[]']

            runner = FreeFemRunner(path+"BoundaryComputations.edp")
            runner.import_variables(Th=Th,phiVal=newx,alpha=alpha)
            exports = runner.execute()
