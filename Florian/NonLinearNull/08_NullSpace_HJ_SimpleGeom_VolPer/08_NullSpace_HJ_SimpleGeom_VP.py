...
path = "NonLinearNull/08_NullSpace_HJ_SimpleGeom_VolPer/"

exports = FreeFemRunner(path+"08_Mesh.edp").execute()
Th = exports['Th']
alpha = exports['alpha']
beta = exports['beta']
...
class TO_problem(EuclideanOptimizable):
    def __init__(self):
        ...
        self.ny = []
    def x0(self):
        runner = FreeFemRunner(path+"08_InitialGuess.edp")
        runner.import_variables(Th=Th)
        return runner.execute()['phi[]']

    def J(self, x):
        runner = FreeFemRunner(path+"08_Cost.edp")
        runner.import_variables(Th=Th,phiVal=x)
        exports = runner.execute()
        return exports['J']

    def dJ(self, x):
        runner = FreeFemRunner(path+"08_CostGradient.edp")
    ...
    def G(self, x):
        runner = FreeFemRunner(path+"08_ConstraintEq.edp")
        runner.import_variables(Th=Th,AchiVal=self.volFrac)
        return [runner.execute()['C']]
    
    def dG(self, x):
        runner = FreeFemRunner(path+"08_ConstraintEqGradient.edp")
        runner.import_variables(Th=Th,Th2=self.Th2,phiVal=x,beta=beta,
                                lsLab=lsLabel,rInner=rInner)
        return runner.execute()['g[]']

    def H(self, x):
        runner = FreeFemRunner(path+"08_ConstraintIneq.edp")
        runner.import_variables(Th=Th,phiVal=x)
        return [runner.execute()['H']]

    def dH(self, x):
        runner = FreeFemRunner(path+"08_ConstraintIneqGradient.edp")
...
runner = FreeFemRunner(path+"08_VolFracComputer.edp")
runner.import_variables(Th=Th,phiVal=x)
problem._problem.volFrac = runner.execute()['Achi[]']

runner = FreeFemRunner(path+"08_BoundaryRefinement.edp")
...
while normdx > params['tol'] and it <= params['maxit']:
    ...
    # Update step
    g = AJ*xiJ+AC*xiC
    runner = FreeFemRunner(path+"08_HJ.edp")
