...
def FunctionCase09(case,No,maxIter):
    ...
    class TO_problem(EuclideanOptimizable):
        def __init__(self):
            ...
            self.dydOmVal = []
        def x0(self):
            runner = FreeFemRunner(path+"09_InitialGuess.edp")
            runner.import_variables(Th=Th)
            return runner.execute()['phi[]']

        def J(self, x):
            runner = FreeFemRunner(path+"09_Cost.edp")
            runner.import_variables(Th=Th,phiVal=x)
            exports = runner.execute()
            return exports['J']

        def dJ(self, x):
            runner = FreeFemRunner(path+"09_CostGradient.edp")
            runner.import_variables(Th=Th,Th2=self.Th2,phiVal=x,alpha=alpha,beta=beta,lsLab=lsLabel,rInner=rInner)
            exports = runner.execute()

            self.wVal = exports['w[]']
            self.dOmVal = exports['dOm[]']
            self.dxdOmVal = exports['dxdOm[]']
            self.dydOmVal = exports['dydOm[]']
        
            return exports['g[]']

        def G(self, x):
            runner = FreeFemRunner(path+"09_ConstraintEq.edp")
            runner.import_variables(Th=Th,AchiVal=self.volFrac)
            return [runner.execute()['C']]

        def dG(self, x):
            runner = FreeFemRunner(path+"09_ConstraintEqGradient.edp")
            runner.import_variables(Th=Th,Th2=self.Th2,phiVal=x,beta=beta,
                                    lsLab=lsLabel,rInner=rInner)
            return runner.execute()['g[]']
        
        def H(self, x):
            runner = FreeFemRunner(path+"09_ConstraintIneq.edp")
            runner.import_variables(Th=Th,phiVal=x)
            return [runner.execute()['H']]

        def dH(self, x):
            runner = FreeFemRunner(path+"09_ConstraintIneqGradient.edp")
    ...
    runner = FreeFemRunner(path+"09_VolFracComputer.edp")
    runner.import_variables(Th=Th,phiVal=x)
    ...
    while normdx > params['tol'] and it <= params['maxit']:
        ...
        if case=="Original":
            runner = FreeFemRunner(path+"09_OriginalPrimal.edp")
        ...
        runner = FreeFemRunner(path+"09_HJ.edp")
