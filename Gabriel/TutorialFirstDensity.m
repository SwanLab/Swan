classdef TutorialFirstDensity < handle

    properties (Access = private)
        mesh
        filterPDE          % filtro PDE para b
    ...
    methods (Access = public)
        ...
        function obj = TutorialFirstDensity()
            obj.init()
            obj.createMesh();
        ...
        function plotMicrostructuresOnMesh(obj, nGridX, nGridY, scaleFactor)
            ...
            % ========== OBTER CAMPOS b e ρ (AMBOS VARIÁVEIS) ==========
            x = obj.designVariable;
            f = x.obtainDomainFunction();

            % Campo b (primeira variável)
            b_raw = f{1};

            % Campo ρ (segunda variável)
            rho_raw = f{2};

            % Aplicar filtros (mesmos usados na otimização)
            b_fun = obj.filterPDE.compute(b_raw, 1);
            rho_fun = obj.filterLUMP.compute(rho_raw, 1);

            b_vals = b_fun.fValues;
            ...
            for ix = 1:nGridX
                for iy = 1:nGridY
                    ...
                    % COR baseada em ρ (escala de cinza)
                    % ρ = 1 → preto (material sólido)
                    % ρ = 0 → branco (vazio)
                    gray = 1 - rho_local;
                    col = [gray, gray, gray];
            ...
            % Barra de cores para ρ
            colormap(gray);
            caxis([0, 1]);
            colorbar('Location', 'eastoutside', 'FontSize', 10);
            ylabel(colorbar, 'Densidade ρ', 'FontSize', 10);

            title('Microestruturas otimizadas - Simultâneo (b e ρ variáveis)', ...
    ...
    methods (Access = private)
        ...
        function createDesignVariable(obj)
            % b inicializado em 0, ρ inicializado em 1
            n = obj.mesh.nnodes;
        ...
        function createFilters(obj)
            % Filtro PDE para b (regularização)
            eOverhmin = 6;
            s.filterType = 'PDE';
            s.mesh = obj.mesh;
        ...
        function createVolumeConstraint(obj)
            ...
            v = VolumeConstraintMicroDensity(s);
            obj.volume = v;
        end
        ...
        function createMaterial(obj)
            % Material que combina C^h(b) com SIMP
        ...
        function bc = createBoundaryConditions(obj)
            ...
            % Apoio fixo no lado esquerdo (engaste)
            isDir = @(coor) abs(coor(:,1) - xMin) < 1e-12;

            % Força aplicada no lado direito, no centro
            isForce = @(coor) abs(coor(:,1) - xMax) < 1e-12 & ...
                coor(:,2) >= 0.4*yMax & ...
                coor(:,2) <= 0.6*yMax;

            % Condição de Dirichlet (engaste)
