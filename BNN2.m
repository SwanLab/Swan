classdef BNN2 < handle
    ...
    methods (Access = public)
        ...
        function obj = BNN2()
            ...
            [obj.meshDomain,obj.meshSubDomain,obj.interfaceConnec,obj.subdomainBoundary] = m.create();
...
%             obj.interiorDof        = obj.assingInteriorDof();
    ...
    methods (Access = private)
        ...
        function coarseSpace = computeCoarseSpace(obj)
            RBbasis = obj.computeRigidBodyBasis();
            RBbasis = obj.scaleInterfaceValues(RBbasis);
            obj.hExt    = obj.computeHarmonicExtension();
            coarseSpace = obj.computeCoarseSpaceBasis(RBbasis);
        end
        ...
        function cBasis = computeCoarseSpaceBasis(obj,basis)
            for idom = 1:obj.nSubdomains(1)
                ...
                boundaryDof = obj.boundaryDof{idom};
                interiorDof = obj.interiorDof{idom};
                for ibasis = 1:nbasis
                    boundaryDisp = basis{idom}(boundaryDof,ibasis);
                    interiorDisp = epsilon*boundaryDisp;
                    cBasis{idom}(interiorDof,ibasis)  = interiorDisp;
                    cBasis{idom}(boundaryDof,ibasis) = boundaryDisp;
                end
            end
        end
        ...
        function [interiorDof,boundaryDOF] = assingInteriorBoundaryDof(obj)
            for idom=1:obj.nSubdomains(1)
                ndof  = obj.displacementFun{idom}.nDofs;
                ndimf = obj.displacementFun{idom}.ndimf;
                boundaryDOF{idom} = obj.assignBoundaryDOF(obj.subdomainBoundary{idom},ndimf);                
                alldofs = [1:1:ndof]';
                interiorDof{idom}  = setdiff(alldofs,boundaryDOF{idom});
            end
        end

        function bdDof = assignBoundaryDOF(obj,subdomainBd,ndimf)
            nbound = size(subdomainBd);
            bdDof = 0;
            for ibound = 1:nbound
                nodes = reshape(unique(subdomainBd{ibound}.globalConnec),[],1);
                for iunkn=1:ndimf
                    bdDof = [bdDof ;ndimf*(nodes - 1) + iunkn];
                end
            end
            bdDof = unique(bdDof(2:end));
        end
