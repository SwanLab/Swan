function [mesh, pdecoef, matprop, params, bc, psi] = viga2x1_3m_40volfrac
    ...
    % penalty parameter
    params.penalty = [0.0 0.0];   % method 1,2 and 3
    params.volfrac = [0.2 0.2];   % method 2 and 3
    params.voleps  = [0.01 0.01];  % method 2 and 3
    params.auglag  = [0.1 0.1];   % method 3
    params.epsilon = 0.01;  % method 3       
    
    params.beta = 1e-3; %perimetric constraint penalty parameter
    params.e_h = 4;  %perimetric constraint regularizing parameter ratio     
    %% mesh generation
    prec = 1e6;
    n = 40;
...
%-------------------------------------------------------------------------%
    ...
    psi(:,2) = (p(1,:)'-0.8).^2+(p(2,:)'-0.25).^2-0.1.^2;
