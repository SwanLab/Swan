function [mesh, pdecoef, matprop, params, bc, psi] = placa_pilar_3m
    %% load g and b 
        %% Distributed Neumann boundary conditions    
    load placa_pilar.mat
    %% parameters
    ...
    params.volfrac = [0.25 0.25];   % method 2 and 3
    params.voleps  = [0.01 0.01];  % method 2 and 3
    params.auglag  = [0.05 0.05];   % method 3
    params.epsilon = 0.01;  % method 3      
   
    %% mesh generation  
    [p,e,t] = initmesh(g,'hmax',0.1,'jiggle','on','JiggleIter',100);
%     n = 40;
%     [p,e,t] = poimesh(g,n,n); 
    t(4,:) = 1;
    ghold  = [];
