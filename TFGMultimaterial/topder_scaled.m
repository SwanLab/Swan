...
function dt = topder_scaled(mesh,U,volume,matprop,psi,params,pdecoef)
    ...
    elseif size(pdecoef.f,1)==2 % vectorial second order pde
        ...
        % Lame's coefficients
        la0 = pdecoef.la(1); mu0 = pdecoef.mu(1);
        gamma  = matprop.E./E1; 
        % element characteristic function: 1 if psi>0 and gama if psi<=0 
        [~,tfi] = charfunc( p,t,psi);        
...
%                 + coef2.*((s(1,:)+s(3,:)).*(s(1,:)+s(3,:))) ;
                ...
                %new version for scaled problem
                gamma  = matprop.E(j)/matprop.E(i);
                
                p1 = (1-gamma)*(1+alpha)/(1+gamma*alpha); p2 = ((1-gamma)/2)*((beta-alpha)/(1+gamma*alpha));
                
                TD{i,j} = p1.*((s(1,:).*e(1,:)+2*s(2,:).*e(2,:)+s(3,:).*e(3,:))) ...
                    + p2.*((s(1,:)+s(3,:)).*(e(1,:)+e(3,:))) ;
            end
        end
