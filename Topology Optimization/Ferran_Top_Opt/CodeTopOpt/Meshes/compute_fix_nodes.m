...
switch ftype
    case {'ELASTIC'}
        switch BC
            case 'PERIODIC'
                switch file_name
                    case {'RVE04N3','RVE04N3Fine','RVE04N3FineFine'}
                        pnods = compute_periodic_boundary_nodes_square(coordinates);
                    case 'RVE04N3Hexagonal'
                        pnods = compute_periodic_boundary_nodes_hexagonal(coordinates);
                    otherwise
    ...
    case {'THERMAL'}
        switch BC
            ...
            case 'PERIODIC'
                ...
                %eval(fname);
                pnods = compute_periodic_boundary_nodes(coordinates);
                % prescription of the corners
                ifix=1;
                for i=1:size(corners,2)
                   fixnodes(ifix,1)=corners(i); % node
