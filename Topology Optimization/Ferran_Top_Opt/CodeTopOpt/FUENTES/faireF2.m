% Assemble the force vector for a characteristic function defined by level-set 
% P1 finite elements
...
%     A=0.5*abs((x2-x1)*(y3-y1)-(x3-x1)*(y2-y1));
%     psi1=psi(p1); psi2=psi(p2); psi3=psi(p3);
%     if (psi1>=0 & psi2>=0 & psi3>=0)
%         % rien a faire
%     elseif (psi1<0 & psi2<0 & psi3<0)
%         F(p1)=F(p1)+A/3; F(p2)=F(p2)+A/3; F(p3)=F(p3)+A/3;
%         aire=aire+A;
%     else
%         [psis,is]=sort([psi1,psi2,psi3]);
%         pl=[p1,p2,p3]; ps=pl(is);
%         if (psis(2)>=0)
%             x1=p(1,ps(1)); y1=p(2,ps(1)); x2=p(1,ps(2)); y2=p(2,ps(2)); x3=p(1,ps(3)); y3=p(2,ps(3));
%             x12=(psis(1)*x2-psis(2)*x1)/(psis(1)-psis(2));
...
%             y13=(psis(1)*y3-psis(3)*y1)/(psis(1)-psis(3));
%             A=0.5*abs(((x12-x1)*(y13-y1)-(x13-x1)*(y12-y1)));
%             F(ps(1))=F(ps(1))+(1+psis(2)/(psis(2)-psis(1))+psis(3)/(psis(3)-psis(1)))*A/3;
%             F(ps(2))=F(ps(2))+(psis(1)/(psis(1)-psis(2)))*A/3;
%             F(ps(3))=F(ps(3))+(psis(1)/(psis(1)-psis(3)))*A/3;
%             aire=aire+A;
%         else
%             F(p1)=F(p1)+A/3; F(p2)=F(p2)+A/3; F(p3)=F(p3)+A/3;
%             aire=aire+A;
%             is=fliplr(is); psis=fliplr(psis); ps=pl(is);
%             x1=p(1,ps(1)); y1=p(2,ps(1)); x2=p(1,ps(2)); y2=p(2,ps(2)); x3=p(1,ps(3)); y3=p(2,ps(3));
%             x12=(psis(1)*x2-psis(2)*x1)/(psis(1)-psis(2));
%             y12=(psis(1)*y2-psis(2)*y1)/(psis(1)-psis(2));
%             x13=(psis(1)*x3-psis(3)*x1)/(psis(1)-psis(3));
