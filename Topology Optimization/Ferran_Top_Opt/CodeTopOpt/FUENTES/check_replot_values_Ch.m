function check_replot_values_Ch
...
[First_last,C12,C13,matCh] = get_last_first(Path_mesh);

% if  (C12 == C13) %&& ~strcmp(First_last,'SNAP:')
%     casos_malos(k) = isnap;
...
function [First_last,C12,C13,matCh] = get_last_first(Path)

fid = fopen([Path,'/Ch.txt'],'r');     %# Open the file as a binary
lastline = call_last_line(fid);
fclose(fid);  %# Close the file
C = textscan(lastline, '%s%s%s%f%s%f%s%f%s%f%s%f%s%f');
% k = 1;
% for i = 28:2:28
%     Datos{k} = [C{i}];
%     k = k + 1;
...
matCh = [C11 C12 C13; C12 C22 C23; C13 C23 C33];
