...
%             data = {[obj.constraint.getEigenModes{2,1}.fValues];
%                     [obj.constraint.getEigenModes{2,2}.fValues];
%                     [obj.constraint.getEigenModes{2,3}.fValues];  
%                     [obj.constraint.getEigenModes{2,4}.fValues]}';
...
%                     [obj.constraint.getDesignVariable(2).project('P1',obj.designVariable.fun.mesh).fValues]}';
...
%             obj.obtainGIF('Dv connec', obj.designVariable.fun);
        ...
        function updateMonitoringGradients(obj)
            data = {[obj.constraint.getDesignVariable(2).fValues];
                    ...
                    [obj.constraint.getGradientUN(2).fValues]}';
...
%             titles        = [{'Cost'};titlesF;titlesConst;{'Norm L2 x'};{'Compliance Beta'}];
        ...
        function createMonitoringWithEigenValueConstraint(obj, cParams) 
            ...
            nSFCost       = length(titlesF);
            nSFConstraint = length(titlesConst);
            titles        = [{'Cost'};titlesF;titlesConst;{'Lambda 1'};{'Norm L2 x'};{'Connectivity EigenValue'};{'Target Eigenvalue'}];
%             titles        = [{'Cost'};titlesF;titlesConst;{'Norm L2 x'};{'Connectivity Beta'};{'Target Eigenvalue'}];
%             titles        = [{'Cost'};titlesF;titlesConst;{'Norm L2 x'};{'Compliance Beta'}];
            chConstr      = cell(1,nSFConstraint);
            for i = 1:nSFConstraint
                chConstr{i}   = 'plot';
            ...
            chartTypes = [{'plot'},chCost,chConstr,{'plot'},{'logy'},{'plot'},{'plot'},{'plot'}];
%             chartTypes = [{'plot'},chCost,chConstr,{'logy'},{'plot'},{'plot'}];
            s.shallDisplay = cParams.monitoring;
            s.maxNColumns  = 5;
            s.titles       = titles;
            s.chartTypes   = chartTypes;
        ...
        function createMonitoringGradients(obj,cParams)
            titles        = [{'Design Variable Connectivity'}; {'First EigenMode'}; {'Eigenvalue Gradient'}; {'Eigenvalue Unfiltered Gradient'}];
...
%             funs = [{field1},{field1},{field1}];
        ...
        function checkInitial(obj,x0)
            if isempty(obj.x)
                ...
                obj.c = 1000*ones(obj.m,1); % VAR1    1000
                obj.d = 0*ones(obj.m,1); % VAR 2       0
                obj.a0 = 1;
