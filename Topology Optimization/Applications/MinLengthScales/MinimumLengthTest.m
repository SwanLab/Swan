...
gradRhoe = diff(rhoe)./diff(x);
gradRhoe = interp1(xc,gradRhoe,x);
Isolid = rhoP.*exp(-c*gradRhoe.^2);
Ivoid = (1-rhoP).*exp(-c*gradRhoe.^2);

Isolid(isnan(Isolid))=0;
Ivoid(isnan(Ivoid))=0;

gSolid = sum(Isolid.*min((rhoe-etaMax),0).^2)/(N+1);
gVoid = sum(Ivoid.*min((etaMin-rhoe),0).^2)/(N+1);